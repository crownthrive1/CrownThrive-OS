...
# These actions may be analyzed/prepared by the owning Penta, but execution is
# consequential and therefore requires explicit authority/human/provider gates.
CONSEQUENTIAL_ACTIONS = {
    "penta.compliance": {
        "submit_attestation",
        "file_compliance_response",
        "close_material_exception",
    },
    "penta.privacy": {
        "disclose_personal_data",
        "erase_personal_data",
        "restrict_processing",
        "change_processing_purpose",
        "respond_to_rights_request",
    },
    "penta.identity": {
        "grant_role",
        "revoke_role",
    ...
    "penta.data": {
        ...
        "share_restricted_dataset",
        "change_data_classification",
        "change_schema_contract",
    },
    "penta.records": {
        "place_hold",
        "release_hold",
...
# Actions that violate the subsystem's authority boundary regardless of packet
# completeness. These are held rather than merely routed for approval.
FORBIDDEN_ACTIONS = {
...
def receipt_sha256(value: Dict[str, Any]) -> str:
    """Return a deterministic evidence hash for a request/result object."""
...
def build_control_request(
    *,
    ...
    """Build a portable institutional-control request and validate it."""
    ...
    packet = {
        ...
        or {"chlom_ref": None, "dail_ref": None, "accountable_owner": None},
        "human_gate": human_gate
        or {"required": False, "satisfied": False, "approver_refs": [], "separation_of_duties": False},
...
def validate_control_request(packet: Dict[str, Any]) -> None:
    ...
    if not isinstance(trace, dict):
        raise InstitutionalControlError("authority_trace must be an object")
    if not {"chlom_ref", "dail_ref", "accountable_owner"}.issubset(trace):
    ...
    if packet["provider_binding_ref"] is not None and (
        not isinstance(packet["provider_binding_ref"], str) or not packet["provider_binding_ref"].strip()
    ):
        raise InstitutionalControlError("provider_binding_ref must be non-empty or null")
    if packet["readback_strategy"] is not None and (
        not isinstance(packet["readback_strategy"], str) or not packet["readback_strategy"].strip()
    ):
...
def required_controls(packet: Dict[str, Any]) -> Dict[str, Any]:
    ...
    separation_required = (system, action) in INDEPENDENCE_ACTIONS
    return {
        ...
        "separation_of_duties_required": separation_required,
...
def evaluate_control_request(packet: Dict[str, Any]) -> Dict[str, Any]:
    """Evaluate a control request without performing provider effects."""
    ...
    if forbidden:
        result = {"disposition": "hold_fail_closed", "reasons": [forbidden], "controls": controls}
    ...
    if packet["member_maturity"] in {"hold", "retired"}:
        result = {
            ...
            "reasons": [f"member maturity is {packet['member_maturity']}"],
            "controls": controls,
        }
        result["receipt_sha256"] = receipt_sha256(result)
        return result

    # Domain-specific hard safety invariants.
    ...
    if system == "penta.records" and action in {"dispose_record", "destroy_record"} and metadata.get("active_hold"):
        result = {
            "disposition": "hold_fail_closed",
            "reasons": ["record disposition is prohibited while an active hold exists"],
            "controls": controls,
        }
        result["receipt_sha256"] = receipt_sha256(result)
        return result
    ...
    if system == "penta.privacy" and action == "change_processing_purpose" and not metadata.get("purpose_review_ref"):
        result = {
            "disposition": "hold_fail_closed",
            "reasons": ["processing-purpose change requires a purpose/privacy review reference"],
            "controls": controls,
        }
        result["receipt_sha256"] = receipt_sha256(result)
        return result
    ...
    if system == "penta.quality" and action == "change_acceptance_criteria" and metadata.get("after_failure"):
        result = {
            "disposition": "hold_fail_closed",
            "reasons": ["acceptance criteria cannot be lowered after failure to manufacture a pass"],
            "controls": controls,
        }
        result["receipt_sha256"] = receipt_sha256(result)
        return result
    ...
    if controls["execution_maturity_required"] and packet["member_maturity"] not in EXECUTION_ELIGIBLE:
        reasons.append(
            f"execution requires certified/production maturity; current maturity is {packet['member_maturity']}"
        )
    ...
    if system == "penta.quality" and action in {"close_nonconformance", "approve_capa"}:
        if not metadata.get("retest_evidence_ref"):
            reasons.append("quality closure requires re-test/effectiveness evidence")

    if reasons:
        result = {"disposition": "governance_required", "reasons": reasons, "controls": controls}
        result["receipt_sha256"] = receipt_sha256(result)
        return result

    if packet["requested_effect"] == "analyze":
        disposition = "advisory_ready"
    elif packet["requested_effect"] in {"prepare", "route"}:
        disposition = "workflow_ready"
    else:
        disposition = "execution_ready"

    result = {"disposition": disposition, "reasons": [], "controls": controls}
    result["receipt_sha256"] = receipt_sha256(result)
    return result


def self_test() -> Dict[str, Any]:
    """Small deterministic self-test used by CI and operator smoke checks."""

    base = build_control_request(
        system="penta.data",
        action="classify_dataset",
        requested_effect="analyze",
        evidence_refs=["evidence:data-catalog:1"],
        member_maturity="implemented",
    )
    assert evaluate_control_request(base)["disposition"] == "advisory_ready"

    identity = build_control_request(
        system="penta.identity",
        action="grant_role",
        requested_effect="execute",
        evidence_refs=["evidence:role-request:1"],
        member_maturity="implemented",
        authority_trace={"chlom_ref": "chlom:role:1", "dail_ref": None, "accountable_owner": "role:owner"},
        human_gate={
            "required": True,
            "satisfied": True,
            "approver_refs": ["human:approver:1"],
            "separation_of_duties": True,
        },
        provider_effect=True,
        provider_binding_ref="provider-binding:iam:1",
        readback_strategy="read effective role binding",
    )
    assert evaluate_control_request(identity)["disposition"] == "governance_required"

    forbidden = build_control_request(
        system="penta.privacy",
        action="fabricate_consent",
    ...
    return {
        "ok": True,
        "systems": sorted(KNOWN_SYSTEMS),
        "system_count": len(KNOWN_SYSTEMS),
        "execution_eligible_maturities": sorted(EXECUTION_ELIGIBLE),
    }
