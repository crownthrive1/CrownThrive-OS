...
def test_implemented_member_cannot_execute_even_with_other_gates() -> None:
    packet = build_control_request(
        ...
        evidence_refs=["evidence:access-request:1"],
        member_maturity="implemented",
        authority_trace=authority(),
        human_gate=approved_gate(separation=True),
        provider_effect=True,
        provider_binding_ref="provider-binding:iam:1",
        readback_strategy="read effective role membership",
    )
    result = evaluate_control_request(packet)
...
def test_certified_identity_execution_requires_separation_of_duties() -> None:
    packet = build_control_request(
        ...
        action="grant_role",
        requested_effect="execute",
        evidence_refs=["evidence:access-request:2"],
        member_maturity="certified",
        authority_trace=authority(),
        human_gate=approved_gate(separation=False),
        provider_effect=True,
        provider_binding_ref="provider-binding:iam:1",
        readback_strategy="read effective role membership",
    )
...
def test_certified_identity_execution_can_become_execution_ready() -> None:
    packet = build_control_request(
        ...
        action="grant_role",
        requested_effect="execute",
        evidence_refs=["evidence:access-request:3", "evidence:sod:1"],
        member_maturity="certified",
        authority_trace=authority(),
        human_gate=approved_gate(separation=True),
        provider_effect=True,
        provider_binding_ref="provider-binding:iam:1",
        readback_strategy="read effective role membership and audit event",
    )
...
def test_forbidden_privacy_consent_manufacture_holds() -> None:
    packet = build_control_request(
        ...
        evidence_refs=["evidence:privacy-request:1"],
    )
    result = evaluate_control_request(packet)
...
def test_record_under_hold_cannot_be_disposed() -> None:
    packet = build_control_request(
        system="penta.records",
        action="dispose_record",
        requested_effect="execute",
        evidence_refs=["evidence:records:1"],
        member_maturity="production",
        authority_trace=authority(),
        human_gate=approved_gate(),
        provider_effect=True,
        provider_binding_ref="provider-binding:records:1",
        readback_strategy="verify disposition receipt",
        metadata={
            "active_hold": True,
            "retention_authority_ref": "records-schedule:1",
            "hold_clearance": False,
        },
    )
    result = evaluate_control_request(packet)
    assert result["disposition"] == "hold_fail_closed"
...
def test_contract_signature_requires_legal_and_signatory_evidence() -> None:
    packet = build_control_request(
        system="penta.contracts",
        action="sign_contract",
        requested_effect="execute",
        evidence_refs=["evidence:contract:1"],
        member_maturity="certified",
        authority_trace=authority(),
        human_gate=approved_gate(),
        provider_effect=True,
        provider_binding_ref="provider-binding:esign:1",
...
def test_contract_signature_can_become_execution_ready_with_full_evidence() -> None:
    packet = build_control_request(
        ...
        action="sign_contract",
        requested_effect="execute",
        evidence_refs=["evidence:contract:2", "evidence:legal:2", "evidence:signatory:2"],
        member_maturity="production",
        authority_trace=authority(),
        human_gate=approved_gate(),
        provider_effect=True,
        provider_binding_ref="provider-binding:esign:2",
        readback_strategy="verify signed envelope and exact executed hash",
        metadata={
            "legal_review_ref": "penta-legal:review:2",
            "signatory_authority_ref": "chlom:signatory:2",
        },
    )
    assert evaluate_control_request(packet)["disposition"] == "execution_ready"
...
def test_procurement_commitment_requires_spend_and_terms() -> None:
    packet = build_control_request(
        system="penta.procure",
        action="place_order",
        requested_effect="execute",
        evidence_refs=["evidence:requisition:1"],
        member_maturity="certified",
        authority_trace=authority(),
        human_gate=approved_gate(),
        provider_effect=True,
        provider_binding_ref="provider-binding:procurement:1",
...
def test_quality_cannot_lower_criteria_after_failure() -> None:
    packet = build_control_request(
        system="penta.quality",
        action="change_acceptance_criteria",
        requested_effect="prepare",
        evidence_refs=["evidence:test-failure:1"],
        metadata={"after_failure": True},
    )
    result = evaluate_control_request(packet)
    assert result["disposition"] == "hold_fail_closed"
...
def test_quality_closure_requires_independent_retest() -> None:
    packet = build_control_request(
        system="penta.quality",
        action="close_nonconformance",
        requested_effect="execute",
        evidence_refs=["evidence:ncr:1"],
        member_maturity="certified",
        authority_trace=authority(),
        human_gate=approved_gate(separation=True),
        readback_strategy="confirm NCR closure and quality ledger state",
    )
    result = evaluate_control_request(packet)
    assert result["disposition"] == "governance_required"
...
def test_compliance_attestation_requires_source_and_evidence_review() -> None:
    packet = build_control_request(
        ...
        authority_trace=authority(),
        human_gate=approved_gate(separation=True),
        readback_strategy="verify attestation receipt and stored evidence hash",
    )
    result = evaluate_control_request(packet)
    assert result["disposition"] == "governance_required"
