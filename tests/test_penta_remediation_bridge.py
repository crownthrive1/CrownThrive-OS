...
class PentaRemediationBridgeTests(unittest.TestCase):
    def test_flows_fans_one_governed_gate_into_multiple_root_cause_domains(self):
        ...
        self.assertEqual({plan.route_id for plan in plans}, {"family-interoperability", "provider-convergence"})
        self.assertEqual({plan.owner_penta for plan in plans}, {"PentaInterOps", "PentaBind"})
        self.assertEqual(len({plan.fingerprint for plan in plans}), 2)

    def test_route_attempts_are_durable_by_exact_head_and_route(self):
        attempts = {}
        ...
        self.assertEqual(changed[0].attempt, 1)

    def test_unclassified_failure_routes_to_pentatriage_without_arbitrary_edit(self):
        plans, escalations = PentaFlows().route(
            repo="crownthrive1/CrownThrive-OS",
            number=1,
            head_sha="a" * 40,
        ...
        self.assertEqual(escalations, [{"check": "Novel Gate", "reason": "unclassified", "owner": "PentaTriage"}])

    def test_source_convergence_uses_expected_head_and_never_force_pushes(self):
        gh = FakeGH()
    ...
    def test_cross_repository_pr_is_never_mutated_by_source_convergence(self):
        gh = FakeGH()
        helper = PentaHelper(gh)
        pull = {
            "number": 551,
        ...
        result = helper.converge_source(pull, current_base_sha="n" * 40)
        self.assertEqual(result, {"action": "source_convergence", "status": "HOLD", "reason": "fork_or_cross_repo"})
        self.assertEqual(gh.puts, [])

    def test_provider_refresh_dispatches_authoritative_control_plane_once_per_head_signal(self):
        gh = FakeGH()
        helper = PentaHelper(gh)
        pull = {"base": {"ref": "main"}}
    ...
    def test_crawler_keeps_only_latest_check_context_and_failed_conclusions(self):
        gh = FakeGH()
        sha = "a" * 40
        ...
        self.assertEqual(failures[0].name, "Governed Merge Gate")
