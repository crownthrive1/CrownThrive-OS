...
BASE = {
    ...
    "release_execution": receipt("ct.penta.release", semantic_stage="execution"),
    "token_model": "UNRESOLVED",
}


class ReleaseTopologyCertifierTests(unittest.TestCase):
    def test_complete_independent_topology_is_release_eligible(self):
        decision, checks = verify(BASE)
        self.assertEqual(decision, "RELEASE_ELIGIBLE")
    ...
    def test_missing_pentacertifier_fails_closed(self):
        ...
        self.assertTrue(any(x["check"] == "penta_certifier" for x in checks if x["status"] == "FAIL"))
    ...
    def test_originator_cannot_self_certify(self):
        ...
        decision, checks = verify(evidence)
        self.assertEqual(decision, "RELEASE_HOLD")
        self.assertTrue(any(x["check"] == "penta_certifier.independence" for x in checks if x["status"] == "FAIL"))
    ...
    def test_exact_head_drift_fails_closed(self):
        ...
        decision, checks = verify(evidence)
        self.assertEqual(decision, "RELEASE_HOLD")
        self.assertTrue(any(x["check"] == "stage.tests.exact_version" for x in checks if x["status"] == "FAIL"))

    def test_certifier_cannot_also_execute_release(self):
        evidence = copy.deepcopy(BASE)
        evidence["release_execution"]["actor_id"] = "ct.penta.certifier"
        decision, checks = verify(evidence)
        self.assertEqual(decision, "RELEASE_HOLD")
        self.assertTrue(any(x["check"] == "release_execution.independence" for x in checks if x["status"] == "FAIL"))
    ...
    def test_d3_requires_exact_human_reserved_approval(self):
        ...
        decision, checks = verify(evidence)
        self.assertEqual(decision, "RELEASE_HOLD")
        self.assertTrue(any(x["check"] == "d3.approval" for x in checks if x["status"] == "FAIL"))
    ...
    def test_dail_semantic_stage_is_not_token_model(self):
        ...
        decision, checks = verify(evidence)
        self.assertEqual(decision, "RELEASE_HOLD")
        self.assertTrue(any(x["check"] == "token_model" for x in checks if x["status"] == "FAIL"))
