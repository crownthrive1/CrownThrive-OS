-- Rollback for 20260904034000_chlom_dail_compact_verification_sequence_gap_safe_v4_1.sql
-- Restores the exact v4.0 dense-sequence verifier semantics. This rollback does
-- not mutate immutable DAIL events or delete compact verification evidence.
...
begin
  ...
  v_start:=v_cursor.verified_through_sequence_id+1;
  if v_start>v_head_sequence then
    ...
    where cursor_key='canonical';
    return jsonb_build_object('ok',true,'state','CAUGHT_UP','verified_through_sequence_id',v_cursor.verified_through_sequence_id,
      'head_sequence_id',v_head_sequence,'sequence_span_lag',0,'authority_created',false,'observed_at',clock_timestamp());
  end if;

  v_end:=least(v_head_sequence,v_start+p_max_events-1);
  v_expected_count:=(v_end-v_start+1)::integer;

  with source_rows as (
    select e.*,
           lag(e.event_hash) over(order by e.sequence_id) as prior_stored_hash
    from chlom_runtime.dail_events e
    where e.sequence_id between v_start-1 and v_end
  ), payload_calc as (
    select s.*,
  ...
  if v_count>0 then v_root:=chlom_runtime.dail_merkle_root_v2(v_hashes); end if;
  ...
  v_canonical:=jsonb_build_object(
    ...
    'verification_mode','bounded_set_based_no_membership_rewrite'
  );
  ...
  insert into chlom_runtime.dail_verification_segments_v4(
    ...
    v_duration_ms,jsonb_build_object('canonical',v_canonical,'content_address','sha256:'||v_payload_sha,
      'mass_dail_event_update',false,'per_event_membership_write',false,'external_chain_transaction',false)
  ...
  if v_state='PASS' then
    ...
    set cursor_state='HOLD_INTEGRITY',
        last_error=jsonb_build_object('segment_id',v_segment_id,'segment_key',v_segment_key,
          ...
          'contiguous_sequence',v_contiguous,'boundary_hash_match',v_boundary),
  ...
  return jsonb_build_object(
    ...
    'event_count',v_count,'head_at_start_sequence_id',v_head_sequence,
