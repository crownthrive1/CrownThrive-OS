...
begin
  ...
  if v_receipt_id is null then return new; end if;
  ...
  v_body:=concat_ws(E'\n',
    ...
    'V/R,','Kavonte Executive Twin','Founder Office Executive Twin','CrownThrive, LLC'
  );

  v_work_id:=gen_random_uuid();
  v_work_dedupe:=left('founder-send-receipt:'||new.message_id::text,240);
  insert into crm.penta_marketer_work_queue_v1(
    ...
    v_work_id,v_work_dedupe,'penta-mail-founder-receipt',new.message_id::text,'email','founder_office',
    'founder_send_receipt',left(v_subject,1000),v_founder,
    'ct.pentamarketer.agent.executive-twin','ct.persona.crownthrive.executive-twin.kavonte.v1',0,55,'D1',false,'routed',
    jsonb_build_object('source_message_id',new.message_id,'source_provider_message_id',new.provider_message_id,'cold_reach',v_cold)
  );

  v_receipt_message:=public.penta_mail_enqueue_v1(
    'founder_send_receipt','INFO',v_subject,v_body,v_work_dedupe,
    jsonb_build_object(
  ...
  update public.penta_mail_outbox_v1
     set metadata=metadata||jsonb_build_object('recipient_scope','governed_external','recipient_authority','PentaMarketer'),
         updated_at=clock_timestamp()
   where message_id=v_receipt_message;

  update crm.penta_marketer_work_queue_v1
     set penta_mail_message_id=v_receipt_message,state='queued',updated_at=clock_timestamp()
...
begin
  ...
  v_founder:=public.penta_mail_notification_recipient_v1();
  if p_recipient is not null and lower(btrim(p_recipient))<>lower(v_founder) then raise exception 'founder_test_recipient_required'; end if;
  if coalesce(p_spacing_minutes,20) not between 15 and 60 then raise exception 'spacing_minutes_must_be_15_to_60'; end if;

  select count(*) into v_count
  from crm.penta_marketer_personas_v1 p join crm.penta_marketer_agents_v2 a using(persona_id)
  ...
  for r in
    select row_number() over(order by p.display_name,p.persona_id)::integer as seq,
           p.persona_id,p.display_name,p.role_title,p.brand,p.organization,p.disclosure,
           a.agent_id,a.lane,a.primary_channel,a.risk_ceiling
    from crm.penta_marketer_personas_v1 p join crm.penta_marketer_agents_v2 a using(persona_id)
    where p.state='approved' and a.enabled and a.autonomous and a.state='active'
  ...
  loop
    ...
    v_authority:=case when r.risk_ceiling='D0' then 'D0' else 'D1' end;
    ...
    v_body:=concat_ws(E'\n',
      ...
      'Agent ID: '||r.agent_id,'',
      'V/R,',r.display_name,r.role_title,coalesce(nullif(r.organization,''),'CrownThrive, LLC')
    );
    ...
    insert into crm.penta_marketer_work_queue_v1(
      ...
      r.agent_id,r.persona_id,0,case when r.seq=1 then 60 else 20 end,v_authority,false,'routed',
      jsonb_build_object('test_run_id',v_run_id,'sequence_no',r.seq,'scheduled_at',v_scheduled,'lane',r.lane,'primary_channel',r.primary_channel,'risk_ceiling',r.risk_ceiling)
    );
    ...
    v_message:=public.penta_mail_enqueue_v1(
      ...
      jsonb_build_object(
        ...
        'origin_penta','PentaMarketer','delivery_penta','PentaMail',
        'assigned_agent_id',r.agent_id,'assigned_persona_id',r.persona_id,
        'assigned_display_name',r.display_name,'assigned_role_title',r.role_title,
        'work_id',v_work_id,'persona_test_run_id',v_run_id,'persona_test_sequence',r.seq,
        'reply_expected',true,'reply_to','contact@crownthrive.com','send_kind','persona_intro_test',
    ...
    update public.penta_mail_outbox_v1
       set available_at=v_scheduled,
           metadata=metadata||jsonb_build_object(
             'scheduled_persona_test_at',v_scheduled,
             'recipient_scope','governed_external',
             'recipient_authority','PentaMarketer'
           ),
           updated_at=clock_timestamp()
    ...
    insert into integration_control.penta_marketer_persona_mail_test_v1(
      ...
      jsonb_build_object('transport_owner','PentaMail','control_plane','PentaMarketer','copy_policy','ct.pentamailer.policy.universal-copy.v1','reply_to','contact@crownthrive.com','dispatch_scope','send_marketer')
    );
  end loop;

  return jsonb_build_object(
    'ok',true,'run_id',v_run_id,'persona_count',v_count,'recipient',v_founder,'spacing_minutes',p_spacing_minutes,
    ...
    'transport_owner','PentaMail','communication_control_plane','PentaMarketer',
    'dispatch_scope','send_marketer','copy_policy','ct.pentamailer.policy.universal-copy.v1',
