-- Brilliant Directories list-users pagination uses an opaque next_page token in the page parameter.
-- Preserve the opaque token, bind it to its embedded page size, and permit one safe stale-token reset.

create or replace function crm.bd_opaque_page_descriptor_v2(p_cursor text)
returns jsonb
language plpgsql
immutable
strict
...
create or replace function crm.validate_bd_provider_page_cursor_v2()
returns trigger
language plpgsql
security definer
set search_path to 'pg_catalog','crm'
...
begin
  ...
  v_descriptor:=crm.bd_opaque_page_descriptor_v2(new.provider_page_cursor);
  if coalesce((v_descriptor->>'valid')::boolean,false) is not true then
    raise exception using
      errcode='22023',
      message='bd_provider_opaque_page_cursor_rejected:'||coalesce(v_descriptor->>'reason','unknown');
  end if;
...
drop trigger if exists validate_bd_provider_page_cursor_v2
on crm.penta_marketer_queue_policy_v1;
create trigger validate_bd_provider_page_cursor_v2
before insert or update of provider_page_cursor
on crm.penta_marketer_queue_policy_v1
for each row execute function crm.validate_bd_provider_page_cursor_v2();

-- Remove the temporary numeric cursor. The next bounded cycle starts at page 1 and stores a fresh opaque token.
update crm.penta_marketer_queue_policy_v1
set provider_page_cursor=null,
    provider_current_page=null,
    last_provider_ingest_state='CURSOR_RESET_FOR_OPAQUE_REPLAY_V3',
...
declare
  ...
  v_requested_limit integer:=greatest(1,least(coalesce(p_limit,100),100));
  ...
  v_reason text;
  v_cursor text;
  v_cursor_descriptor jsonb;
  v_next_cursor text;
  v_next_descriptor jsonb;
  v_url text;
  v_current_page integer;
  v_total bigint;
  v_attempt integer:=0;
  v_failovers integer:=0;
  v_cursor_resets integer:=0;
...
begin
  ...
  if nullif(v_cursor,'') is not null then
    v_cursor_descriptor:=crm.bd_opaque_page_descriptor_v2(v_cursor);
    if coalesce((v_cursor_descriptor->>'valid')::boolean,false) is not true then
      return jsonb_build_object(
        'state','PROVIDER_HOLD','reason','stored_provider_cursor_invalid',
        'cursor_reason',v_cursor_descriptor->>'reason',
        'endpoint_id',v_endpoint.endpoint_id,'credential_exposed',false
      );
    end if;
    v_effective_limit:=(v_cursor_descriptor->>'page_size')::integer;
  end if;
  ...
  loop
    ...
    v_url:='https://www.locticians.com/api/v2/user/get?property=verified&property_value=0&limit='||v_effective_limit::text;
    if nullif(v_cursor,'') is not null then
      v_url:=v_url||'&page='||
    ...
    begin
      ...
      perform public.integration_record_request(
        ...
        '; failovers='||v_failovers::text||'; cursor_resets='||v_cursor_resets::text||
    ...
    exception when others then
      ...
      insert into integration_control.provider_capability_health_v1(
        ...
        jsonb_build_object('attempts',v_attempt,'failovers',v_failovers,
          'cursor_resets',v_cursor_resets,'raw_secret_exposed',false)
      )
      ...
      return jsonb_build_object(
        ...
        encode(extensions.digest(convert_to(v_error,'UTF8'),'sha256'),'hex'),
        'attempts',v_attempt,'failovers',v_failovers,
        'cursor_resets',v_cursor_resets,'credential_exposed',false
      );
    end;

    -- A page token is query-bound and may become stale after query/provider changes.
    -- One read-only reset to page 1 is safe because prospect writes are idempotent upserts.
    if v_resp.status=400 and nullif(v_cursor,'') is not null and v_cursor_resets=0 then
      v_cursor:=null;
      v_effective_limit:=v_requested_limit;
      v_cursor_resets:=1;
    ...
    if v_resp.status=429 then
      insert into integration_control.provider_capability_health_v1(
        ...
        jsonb_build_object('switch_key',false,'attempts',v_attempt,
          'failovers',v_failovers,'cursor_resets',v_cursor_resets,'raw_secret_exposed',false)
      )
      on conflict(service_id,capability_key) do update set
      ...
      return jsonb_build_object(
        ...
        'failovers',v_failovers,'cursor_resets',v_cursor_resets,'credential_exposed',false
      );
    end if;

    if v_resp.status in (401,403) and v_attempt=(1+v_cursor_resets) and v_failovers=0 then
    ...
    insert into integration_control.provider_capability_health_v1(
      ...
      jsonb_build_object('attempts',v_attempt,'failovers',v_failovers,
        'cursor_resets',v_cursor_resets,'switch_key',false,'raw_secret_exposed',false)
    )
    on conflict(service_id,capability_key) do update set
    ...
    return jsonb_build_object(
      ...
      'attempts',v_attempt,'failovers',v_failovers,'cursor_resets',v_cursor_resets,
      'switch_key',false,'credential_exposed',false
    );
  ...
  if coalesce(v_body->>'status','')<>'success' then
    return jsonb_build_object(
      'state','PROVIDER_HOLD','reason','provider_response_not_success',
      'http_status',v_resp.status,'response_sha256',v_response_sha,
      'attempts',v_attempt,'failovers',v_failovers,'cursor_resets',v_cursor_resets,
      'credential_exposed',false
    );
  ...
  if v_next_cursor is not null then
    v_next_descriptor:=crm.bd_opaque_page_descriptor_v2(v_next_cursor);
    if coalesce((v_next_descriptor->>'valid')::boolean,false) is not true then
      return jsonb_build_object(
        'state','PROVIDER_HOLD','reason','provider_next_page_token_invalid',
        'cursor_reason',v_next_descriptor->>'reason','http_status',v_resp.status,
        'response_sha256',v_response_sha,'credential_exposed',false
      );
    end if;
  end if;
  ...
  with source_rows as (
    select value as r from jsonb_array_elements(v_records) limit v_effective_limit
  ), normalized as (
    select
      r,
      coalesce(
  ...
  select
    ...
    jsonb_strip_nulls(jsonb_build_object(
      ...
      'provider_cursor_reset_count',v_cursor_resets,
  ...
  insert into integration_control.provider_capability_health_v1(
    ...
    jsonb_build_object(
      'attempts',v_attempt,'failovers',v_failovers,'cursor_resets',v_cursor_resets,
      'initial_lane_id',v_initial_lane_id,'provider_records',jsonb_array_length(v_records),
      'prospects_touched',v_touched,'current_page',v_current_page,
      'next_page_present',v_next_cursor is not null,'provider_total',v_total,
      'effective_limit',v_effective_limit,'raw_secret_exposed',false,'raw_payload_archived',false
    )
  )
  on conflict(service_id,capability_key) do update set
    endpoint_id=excluded.endpoint_id,http_method=excluded.http_method,
  ...
  return jsonb_build_object(
    ...
    'failovers',v_failovers,'max_auth_failovers',1,
    'cursor_resets',v_cursor_resets,'max_cursor_resets',1,
    'key_hop_on_429',false,'credential_exposed',false,
