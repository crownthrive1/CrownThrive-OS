...
-- Originator: PentaBuild/PentaCredentials/PentaTime/PentaSecurity.
...
as $fn$
begin
  raise exception 'append_only_runtime_review_receipt';
end
$fn$;
...
begin
  ...
  if v_ref='' then
    v_disposition:='HOLD_RUNTIME_REF_MISSING';
  elsif v_ref like 'function:%' then
    v_target:=substring(v_ref from 10);
  elsif v_ref like 'rpc:%' then
    v_target:=substring(v_ref from 5);
  elsif v_ref like 'postgres:%' then
    v_target:=substring(v_ref from 10);
  else
    v_target:=split_part(v_ref,';',1);
  end if;
  ...
  if v_disposition is null and v_target ~ '^[A-Za-z_][A-Za-z0-9_]*\.[A-Za-z_][A-Za-z0-9_]*$' then
    ...
    if v_count=0 then
      v_disposition:='HOLD_RUNTIME_NOT_FOUND';
    elsif v_security_definer and (v_public or v_anon or v_authenticated) then
      v_disposition:='HOLD_SECURITY_DEFINER_EXECUTE_EXPOSURE';
    else
      v_disposition:='PASS';
    end if;
  ...
  v_payload:=jsonb_build_object(
    'contract','ct.penta.security.runtime-review.v1',
    'system_key',v_row.system_key,
    'canonical_name',v_row.canonical_name,
    'system_version',v_row.version,
    'runtime_ref',v_row.runtime_ref,
    'runtime_target',v_target,
    'risk_ceiling',v_row.risk_ceiling,
    'did_uri',v_row.metadata->>'did_uri',
    'fingerprint_id',v_row.metadata->>'fingerprint_id',
    'function_count',v_count,
    'security_definer_present',v_security_definer,
    'public_execute',v_public,
    'anon_execute',v_anon,
    'authenticated_execute',v_authenticated,
    'service_role_execute',v_service,
  ...
  v_dail:=public.chlom_append_dail_event(
    p_event_type=>'penta.security.runtime-review.completed.v1',
    p_entity_type=>'penta_security_review',
    p_entity_id=>v_row.system_key,
    ...
    p_correlation_id=>'penta-security-review:'||v_row.system_key,
    p_causation_id=>null,
    p_authority_basis=>'Bounded D1 read-only runtime/ACL security review; certification remains independent',
    p_approval_id=>null,
  ...
  insert into penta_security.runtime_review_receipts_v1(
    system_key,runtime_ref,disposition,evidence,evidence_sha256,dail_event_id,dail_event_hash
...
stable
security definer
set search_path='pg_catalog','penta_security','public'
as $fn$
...
-- Source custody for the already-deployed hardened credential-continuity v3 contract.
...
begin
  ...
  v_result:=integration_control.credential_continuity_cycle_v2();
  v_completed:=clock_timestamp();
  v_payload:=jsonb_build_object(
    'contract','ct.penta.credentials.continuity-cycle.v3','execution_key',v_key,'execution_kind',p_execution_kind,
    'scheduler_jobname',p_scheduler_jobname,'function_ref','integration_control.credential_continuity_cycle_v3()',
  ...
  v_sha:=encode(extensions.digest(convert_to(v_payload::text,'UTF8'),'sha256'),'hex');
  v_dail:=public.chlom_append_dail_event(
    p_event_type=>'penta.credentials.continuity-cycle.completed.v3',p_entity_type=>'credential_continuity_execution',
    p_entity_id=>v_key,p_payload=>v_payload,p_actor_ref=>'PentaCredentials/PentaTime',p_actor_did=>null,
    p_agent_id=>'penta.credentials',p_entity_version=>'3.0.0',p_correlation_id=>v_key,p_causation_id=>null,
  ...
  return jsonb_build_object('state','SUCCEEDED','execution_key',v_key,'result',v_result,'result_sha256',v_sha,'dail_event_id',v_event_id,'dail_event_hash',v_event_hash,'started_at',v_started,'completed_at',v_completed,'credential_values_exposed',false,'authority_expansion',false);
end
$fn$;
...
language plpgsql
security definer
set search_path='pg_catalog','pentatime','integration_control'
as $fn$
declare
  v_role text:=coalesce((nullif(current_setting('request.jwt.claims',true),'')::jsonb->>'role'),'');
begin
...
values('credential_continuity','pentatime.executor_credential_continuity_v3()'::regprocedure,'D2',true,
       jsonb_build_object('family','PentaCredentials/PentaTime/PentaCrons','contract','ct.penta.credentials.continuity-cycle.v3','provider_write',false,'credential_change',false,'authority_expansion',false),now())
on conflict(operation_key) do update set executor_regprocedure=excluded.executor_regprocedure,authority_ceiling=excluded.authority_ceiling,enabled=true,metadata=pentatime.operation_executors_v3.metadata||excluded.metadata,updated_at=now();
...
declare
  ...
  v_v2_public boolean;
  v_v2_anon boolean;
  v_v2_authenticated boolean;
  v_v2_service boolean;
  v_v3_public boolean;
  v_v3_anon boolean;
  v_v3_authenticated boolean;
  v_v3_service boolean;
  v_executor text;
  v_enabled boolean;
  v_pass boolean;
begin
...
-- Canonical scheduler stays one clock; ownership moves behind PentaTime's guarded executor.
do $do$
...
-- Keep scheduler permanence pointed at the same single native clock.
update integration_control.scheduler_desired_jobs_v2
set schedule='23 */6 * * *',
    command='select pentatime.execute_guarded_v3(''credential_continuity'');',
    generation=greatest(generation,2026083023),
    source_ref='ct.penta.credentials.continuity-cycle.v3',
    active=true,
    allow_auto_restore=true,
    metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('owner','PentaCredentials/PentaTime/PentaCrons','contract','ct.penta.credentials.continuity-cycle.v3','single_native_clock',true,'authority_expansion',false),
    updated_at=now()
where jobname='crownthrive_credential_continuity_daily';

-- PentaSecurity becomes runtime-present but remains pending independent certification.
update public.penta_system_registry
set runtime_ref='function:penta_security.review_system_v1(text)',
    metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object(
      'runtime_contract','ct.penta.security.runtime-review.v1',
      'implementation_state','runtime_active_pending_independent_certification',
      'bounded_runtime_risk','D1',
      'provider_write',false,
      'credential_change',false,
      'money_movement',false,
