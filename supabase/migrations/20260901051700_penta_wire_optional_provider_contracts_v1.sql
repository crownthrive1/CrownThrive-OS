...
-- No provider writes, credential operations, money movement, rights, or D3 effects.
...
BEGIN
  ...
  IF v_before <> v_expected_sha THEN
    RAISE EXCEPTION 'penta_wire_scan_v1 predecessor digest mismatch: expected %, got %', v_expected_sha, v_before;
  END IF;
  ...
  SET metadata = coalesce(metadata,'{}'::jsonb) || jsonb_build_object(
      ...
      'penta_wire_optional_contract_classified_at', clock_timestamp()
    ),
    updated_at = now()
  WHERE service_id IN (
  ...
  IF v_old::int <> 5 THEN
    RAISE EXCEPTION 'expected exactly 5 optional provider services, updated %', v_old;
  END IF;

  v_def := pg_get_functiondef('integration_control.penta_wire_scan_v1()'::regprocedure);
  ...
  IF EXISTS (
    ...
    WHERE service_id IN ('google_analytics_ga4','google_maps_javascript','meta_facebook_login','openai_crownthrive_api','unsplash_crownthrive_studios')
      AND coalesce((metadata->>'penta_wire_certification_required')::boolean,true)
  ) THEN
