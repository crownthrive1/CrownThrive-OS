-- CrownThrive COS V1.0.0 source convergence preflight guard
--
...
-- Passing this preflight does not prove that the current Git commit was accepted,
-- deployed or read back. Exact Git/source and production-SHA binding remain
-- external release-certification gates.
...
select chlom_runtime.append_dail_event(
  'cos.v1.source_convergence_preflight.pass',
  ...
  jsonb_build_object(
    ...
    'release_promotion_created',false,
    'observed_registry_source_sha',(
      select r.source_sha
      from integration_control.cos_release_registry_v1 r
      where r.release_id='ct.cos.release.1.0.0'
      limit 1
    ),
    'source_git_readback_state','external_required',
    'exact_git_and_production_sha_readback','external_required',
    'pentafabric_production_canary_state','external_activation_hold',
    'pentafabric_canary_durable_ledger','public.pentafabric_events',
    'pentafabric_canary_consumer','external_registry_verifier_required',
    'caller_boolean_can_promote',false,
