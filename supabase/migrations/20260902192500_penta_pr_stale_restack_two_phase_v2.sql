-- ct.penta.pr-terminalization-policy.v2
-- Allow a stale/conflicting predecessor to be classified before its successor is created,
-- but require successor + handoff as an atomic pair before any successful terminalization.
...
select jsonb_build_object(
  ...
  'classifications',jsonb_build_array(
    ...
    jsonb_build_object('class','STALE_RESTACK_REQUIRED','action','RESTACK','rule','unique work still needed but base/head is stale or conflicting; classify predecessor first, preserve it, then create/route exactly one successor on current main and record successor + handoff before predecessor terminalization'),
  ...
  'stale_predecessor_classification_guard','successor_pr_number and handoff_receipt_ref may both be omitted during initial STALE_RESTACK_REQUIRED classification',
  'stale_predecessor_close_guard','successor_pr_number AND handoff_receipt_ref required before predecessor terminalization',
...
declare
  ...
  v_disposition text;
  v_meta jsonb;
  v_has_successor boolean:=p_successor_pr_number is not null;
  v_has_handoff boolean:=coalesce(trim(p_handoff_receipt_ref),'')<>'';
begin
  ...
  if v_class='STALE_RESTACK_REQUIRED' and v_has_successor<>v_has_handoff then
    raise exception 'stale_restack_successor_and_handoff_must_be_supplied_together';
  end if;
  ...
  v_meta:=jsonb_strip_nulls(jsonb_build_object(
    ...
    'lifecycle_policy_version','2.0.1',
    'classification',v_class,
    'owner',p_owner,
    'next_predicate',p_next_predicate,
    'successor_pr_number',p_successor_pr_number,
    'handoff_receipt_ref',nullif(trim(p_handoff_receipt_ref),''),
  ...
  set disposition=v_disposition,
      reason=p_reason,
      metadata=(case
        when v_class='STALE_RESTACK_REQUIRED' and not v_has_successor and not v_has_handoff
          then coalesce(metadata,'{}'::jsonb)-'successor_pr_number'-'handoff_receipt_ref'
  ...
  return jsonb_build_object(
    ...
    'successor_pr_number',p_successor_pr_number,
    'handoff_receipt_ref',nullif(trim(p_handoff_receipt_ref),''),
    'restack_pair_complete',case when v_class='STALE_RESTACK_REQUIRED' then v_has_successor and v_has_handoff else null end,
    'authority_created',false
  );
end;
$function$;
...
declare
  ...
  v_handoff text;
begin
  if upper(trim(coalesce(p_state,'')))='SUCCEEDED'
     and upper(trim(coalesce(p_classification,'')))='STALE_RESTACK_REQUIRED' then
    select metadata->>'successor_pr_number', metadata->>'handoff_receipt_ref'
      into v_successor, v_handoff
    from penta_pr.lifecycle
    where lower(repo)=lower(trim(coalesce(p_repo,'')))
      and pr_number=p_pr_number
      and head_sha=p_head_sha
    order by first_seen_at desc
    limit 1;

    if coalesce(trim(v_successor),'')='' or coalesce(trim(v_handoff),'')='' then
      raise exception 'stale_predecessor_terminalization_requires_successor_and_handoff';
    end if;
  end if;

  insert into penta_pr.terminal_reconciliation_v3(
    repo,pr_number,head_sha,classification,terminal_action,decision_state,reason,evidence,
    provider_http_status,provider_state,provider_merged,provider_merge_commit_sha,provider_readback,
    terminal_at,updated_at
  )
  values(
    p_repo,p_pr_number,p_head_sha,p_classification,p_action,p_state,p_reason,coalesce(p_evidence,'{}'::jsonb),
    p_http_status,p_provider_state,coalesce(p_merged,false),p_merge_commit_sha,coalesce(p_readback,'{}'::jsonb),
    case when p_state='SUCCEEDED' then now() end,now()
  )
  on conflict(repo,pr_number,head_sha,terminal_action) do update set
    classification=excluded.classification,
    decision_state=excluded.decision_state,
    reason=excluded.reason,
    evidence=penta_pr.terminal_reconciliation_v3.evidence||excluded.evidence,
    provider_http_status=excluded.provider_http_status,
    provider_state=excluded.provider_state,
    provider_merged=excluded.provider_merged,
    provider_merge_commit_sha=excluded.provider_merge_commit_sha,
    provider_readback=excluded.provider_readback,
    terminal_at=case when excluded.decision_state='SUCCEEDED' then coalesce(penta_pr.terminal_reconciliation_v3.terminal_at,now()) else penta_pr.terminal_reconciliation_v3.terminal_at end,
    updated_at=now()
...
begin
  ...
  if v_class='STALE_RESTACK_REQUIRED' then
    select metadata->>'successor_pr_number', metadata->>'handoff_receipt_ref'
      into v_successor, v_handoff
    from penta_pr.lifecycle
