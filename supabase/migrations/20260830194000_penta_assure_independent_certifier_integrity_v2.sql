...
begin
  ...
  -- A check only passes when it is an object whose explicit `passed` value is true.
  -- Malformed or missing check state therefore fails closed rather than raising from a
  -- permissive boolean cast.
  v_all :=
  ...
  if p_risk_class = 'D3' then
    -- D3 remains human-reserved. Independent identity evidence cannot auto-certify it.
    v_disp := 'hold';
    v_ind := 'not_satisfied';
