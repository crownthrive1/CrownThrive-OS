...
-- Exact-head provider/Edge source security review extension.
-- Additive to the canonical penta_security runtime introduced by
-- 20260830233000_penta_security_and_credential_continuity_contract_v1.sql.
--
-- Authority boundary:
-- * PentaSecurity issues a bounded security disposition only.
-- * This does not certify, merge, deploy, grant CHLOM rights, create credentials,
--   move money, expand authority, or perform D3/sovereign actions.
-- * Source is fetched read-only from the canonical public GitHub repository at
--   an exact immutable commit SHA. Policy controls are version-pinned and
--   append-only. Raw source is never stored in receipts or DAIL.
...
as $fn$
begin
  raise exception 'append_only_provider_source_policy';
end
$fn$;

drop trigger if exists penta_security_provider_source_policies_immutable_v1
  on penta_security.provider_source_policies_v1;
...
as $fn$
begin
  raise exception 'append_only_provider_source_review_receipt';
end
$fn$;

drop trigger if exists penta_security_provider_source_review_receipts_immutable_v1
  on penta_security.provider_source_review_receipts_v1;
...
create or replace function penta_security.review_github_provider_source_v1(
  p_policy_key text,
...
begin
  ...
  if p_policy_key is null or btrim(p_policy_key)='' then
    raise exception 'policy_key_required';
  end if;
  ...
  if v_policy.state<>'active' then
    v_disposition:='HOLD_POLICY_NOT_ACTIVE';
  elsif coalesce(p_exact_head_sha,'') !~ '^[0-9a-f]{40}$' then
    v_disposition:='HOLD_EXACT_HEAD_INVALID';
  end if;
  ...
  if v_disposition is null then
    ...
    begin
      v_http:=extensions.http_get(v_url);
    exception when others then
      v_disposition:='HOLD_SOURCE_FETCH_FAILED';
    end;
    ...
    if v_disposition is null then
      if v_http.status<>200 then
        ...
        v_bytes:=octet_length(v_source);
        if v_bytes=0 then
          v_disposition:='HOLD_SOURCE_EMPTY';
        elsif v_bytes>v_policy.max_source_bytes then
          v_disposition:='HOLD_SOURCE_TOO_LARGE';
        end if;
      end if;
    end if;
  end if;
  ...
  if v_source is not null then
    ...
    foreach v_required in array v_policy.required_literals loop
      if strpos(v_source,v_required)=0 then
        v_missing:=array_append(v_missing,v_required);
      end if;
    end loop;

    foreach v_forbidden in array v_policy.forbidden_literals loop
      if strpos(v_source,v_forbidden)>0 then
        v_forbidden_present:=array_append(v_forbidden_present,v_forbidden);
      end if;
    end loop;

    if v_disposition is null and cardinality(v_missing)>0 then
      v_disposition:='HOLD_REQUIRED_SECURITY_CONTROL_MISSING';
    elsif v_disposition is null and cardinality(v_forbidden_present)>0 then
      v_disposition:='HOLD_FORBIDDEN_SECURITY_PATTERN_PRESENT';
  ...
  v_payload:=jsonb_build_object(
    ...
    'provider_system',v_policy.provider_system,
    'resource_type',v_policy.resource_type,
    'resource_id',v_policy.resource_id,
    'repository',v_policy.repository,
    'source_path',v_policy.source_path,
    'exact_head_sha',p_exact_head_sha,
    'source_sha256',v_source_sha,
    'source_bytes',v_bytes,
    'required_control_count',cardinality(v_policy.required_literals),
    'forbidden_pattern_count',cardinality(v_policy.forbidden_literals),
    'missing_required_controls',to_jsonb(v_missing),
    'forbidden_patterns_present',to_jsonb(v_forbidden_present),
    'raw_source_stored',false,
    'source_fetch_host','raw.githubusercontent.com',
    'disposition',coalesce(v_disposition,'HOLD_UNCLASSIFIED'),
  ...
  v_dail:=public.chlom_append_dail_event(
    p_event_type=>'penta.security.provider-source-review.completed.v1',
    p_entity_type=>'penta_security_provider_source_review',
    p_entity_id=>v_policy.resource_id||':'||p_exact_head_sha,
    ...
    p_correlation_id=>'penta-security-provider-review:'||v_policy.resource_id||':'||p_exact_head_sha,
    p_causation_id=>null,
  ...
  insert into penta_security.provider_source_review_receipts_v1(
    ...
    exact_head_sha,disposition,source_sha256,source_bytes,evidence,evidence_sha256,
    dail_event_id,dail_event_hash
  ) values (
    v_policy.policy_key,v_policy.policy_version,v_policy.provider_system,v_policy.resource_type,v_policy.resource_id,
    v_policy.repository,v_policy.source_path,p_exact_head_sha,coalesce(v_disposition,'HOLD_UNCLASSIFIED'),
    v_source_sha,v_bytes,v_payload,v_sha,v_event_id,v_event_hash
...
-- Exact policy for the existing institutional terminal-provider repair lane (#1967).
-- A new policy version must be appended to change these controls; this row is never mutated.
insert into penta_security.provider_source_policies_v1(
  ...
  array[
    ...
    'public.penta_pr_closeout_claim_v1',
    'public.penta_pr_closeout_result_v1'
  ]::text[],
  150000,
  'active',
  null
...
begin
  ...
  if v_policy_count<>1 then raise exception 'PENTASECURITY_PROVIDER_POLICY_REGISTRATION_FAILED'; end if;
