...
begin
  ...
  v_definition:=replace(v_definition,
    'set state=''active'',daily_cap=200,monthly_cap=5000,expires_at=''2099-12-31 23:59:59+00''::timestamp with time zone,nonrenewing=false',
  ...
  v_definition:=replace(v_definition,
    'or expires_at < ''2099-12-31 23:59:59+00''::timestamp with time zone or nonrenewing is distinct from false',
...
select 'ct.pentaself.control.locticians-campaign',3,'control','locticians_growth_campaign',
       ...
       'production:2026-08-29:pentaself-permanence-v3','ct.pentamarketer.locticians.dynamic-outreach.v3','PentaSELF/PentaMarketer',
       encode(extensions.digest(jsonb_build_object('contract_key','ct.pentaself.control.locticians-campaign','generation',3,'target_key','locticians_growth_campaign','renewal_mode','rolling_366_day_window','renew_when_days_remaining',60)::text,'sha256'),'hex');
