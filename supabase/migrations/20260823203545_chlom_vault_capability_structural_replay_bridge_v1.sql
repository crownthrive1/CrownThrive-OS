...
-- Purpose: restore the minimum non-secret structural prerequisites required by
-- 20260823203546_execution_builder_capability_contract_identity_v1.sql on a
-- fresh replay/Preview database when historical provider-applied source is absent.
--
-- This is deliberately structural only. It creates no credentials, Vault secrets,
-- trade-secret bodies, capability grants, provider activations, D3 authority,
-- money movement, rights grants, votes, certificates, or execution authority.
-- On production systems where the canonical objects already exist this migration
-- is idempotent and preserves existing rows/history.
...
-- Exact fail-closed structural readback required by the next historical migration.
do $verify$
declare
  v_view text;
  v_pk integer;
begin
  ...
  then
    raise exception 'HOLD_CAPABILITY_STRUCTURAL_REPLAY_BRIDGE_INCOMPLETE';
  end if;

  select pg_get_viewdef('chlom_runtime.capability_contracts'::regclass,true)
    into v_view;
  ...
  if v_pk<>1 then
    raise exception 'HOLD_CAPABILITY_BASE_PRIMARY_KEY_MISSING';
  end if;
end
$verify$;
