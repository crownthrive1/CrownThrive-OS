-- Provider-applied migration: penta_marketer_event_derived_projection_v2
-- Event-derived projection replaces mutable summary counters as the authoritative
-- PentaMarketer send projection and permanently resolves stale summary drift.
...
drop policy if exists penta_marketer_campaign_projection_service_role_v2
...
begin
  ...
  insert into crm.penta_marketer_campaign_projection_v2(
    ...
    p_campaign_ref, v_total, v_accepted,
    'append_only_event_ledger', 'crm.penta_marketer_campaign_events_v1',
    v_digest, now(), jsonb_build_object('mutable_summary_authority', false, 'event_ledger_authoritative', true)
  )
...
revoke all on function crm.penta_marketer_refresh_event_projection_v2(text)
  from public, anon, authenticated;
grant execute on function crm.penta_marketer_refresh_event_projection_v2(text)
  to service_role;
...
from cron.job
where jobname = 'ct-pentamarketer-event-projection-v2';
