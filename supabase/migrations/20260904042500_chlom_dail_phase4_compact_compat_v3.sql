-- CHLOM DAIL Phase-4 assurance compact-verifier compatibility v3.
-- Repairs a false HOT FAIL in read_dail_phase4_assurance_status_v2 after
-- verify_dail_chain_checkpoint_v3 was correctly upgraded to the compact v4
-- compatibility contract and stopped emitting the legacy tail_failure_count key.
--
-- No immutable DAIL event mutation, authority creation, external anchor broadcast,
-- credential operation, or certification is performed by this migration.
...
declare
  ...
  v_tail_failure_count bigint;
  v_tail_failure_count_source text;
  v_compact_compatible boolean:=false;
begin
  ...
  if v_verification ? 'tail_failure_count' then
    v_tail_failure_count:=coalesce((v_verification->>'tail_failure_count')::bigint,-1);
    v_tail_failure_count_source:='native_v3_tail_failure_count';
  else
    v_compact_compatible:=
      coalesce((v_verification->>'verified_prefix_ok')::boolean,false)
      ...
      and coalesce(v_verification->>'implementation_contract','')='ct.dail.verify-chain.v4';
    v_tail_failure_count:=
      coalesce((v_verification->>'v4_unresolved_hold_segment_count')::bigint,0)
      +coalesce((v_verification->>'v4_order_overlap_failures')::bigint,0)
      +coalesce((v_verification->>'v4_root_link_failures')::bigint,0);
    v_tail_failure_count_source:='derived_compact_v4_failure_counters';
  end if;

  v_hot_state:=case
    when coalesce((v_verification->>'ok')::boolean,false)
      and v_tail_failure_count=0
      and ((v_verification ? 'tail_failure_count') or v_compact_compatible)
    then 'PASS'
    else 'FAIL'
  end;
  ...
  if not found then
    return jsonb_build_object(
      'hot_route',jsonb_build_object(
        ...
        'tail_failure_count',v_tail_failure_count,
        'tail_failure_count_source',v_tail_failure_count_source,
        'compact_compatibility_ok',v_compact_compatible,
        'head_hash',coalesce(v_verification->>'current_head_hash',v_verification->>'head_hash'),
...
comment on function chlom_runtime.read_dail_phase4_assurance_status_v2(bigint) is
'Phase-4 assurance reader compatible with both legacy v3 tail_failure_count and compact v4 compatibility counters. It never infers PASS from missing evidence; compact PASS requires verified prefix, caught-up head and zero unresolved hold/order/root-link failures.';
