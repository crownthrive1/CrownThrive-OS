...
create or replace function integration_control.penta_pr_restack_hold_v1(
  p_assignment_id uuid,p_hold_code text,p_http_status integer default null,p_evidence jsonb default '{}'::jsonb
) returns jsonb
language plpgsql
security definer
...
begin
  ...
  set state='HOLD',hold_code=p_hold_code,provider_http_status=p_http_status,
      evidence=evidence||coalesce(p_evidence,'{}'::jsonb)||jsonb_build_object(
        'hold_code',p_hold_code,'provider_http_status',p_http_status,'authority_created',false,'observed_at',clock_timestamp()),
      updated_at=clock_timestamp()
  where assignment_id=p_assignment_id;
  return jsonb_build_object('state','HOLD','assignment_id',p_assignment_id,'hold_code',p_hold_code,
    'provider_http_status',p_http_status,'authority_created',false);
end
$function$;
revoke all on function integration_control.penta_pr_restack_hold_v1(uuid,text,integer,jsonb) from public,anon,authenticated;
grant execute on function integration_control.penta_pr_restack_hold_v1(uuid,text,integer,jsonb) to service_role;
...
declare
  ...
  v_main_sha text; v_base_tree text; v_branch text; v_branch_q text;
  v_tree_entries jsonb; v_tree_sha text; v_successor_head text; v_pr_number bigint;
  v_event jsonb; v_changed integer;
begin
  ...
  if a.source_repo is distinct from v_repo or a.source_pr_number is null or a.exact_head_sha !~ '^[0-9a-f]{40}$' then raise exception 'exact_source_required'; end if;
  if not a.provider_write_allowed then raise exception 'provider_write_forbidden'; end if;
  if a.risk_class not in ('D0','D1','D2') or a.authority_ceiling not in ('D0','D1','D2') then raise exception 'authority_ceiling_forbidden'; end if;
  if a.d3_human_reserved is distinct from true or a.money_movement_allowed or a.credential_change_allowed or a.authority_expansion then raise exception 'assignment_guardrail_violation'; end if;
  ...
  values(a.assignment_id,v_branch,'RUNNING',jsonb_build_object(
    'contract','ct.penta.pr-terminalization-policy.v2','predecessor_preserved',true,'draft_successor_only',true,
    'merge_performed',false,'predecessor_close_performed',false,'certification_claimed',false,
  ...
  if v_token is null then
    return integration_control.penta_pr_restack_hold_v1(a.assignment_id,'HOLD_GITHUB_PROVIDER_CREDENTIAL_UNAVAILABLE',null,'{}'::jsonb);
  end if;
  ...
  -- Idempotent recovery if a prior provider attempt created the draft PR before receipt completion.
  ...
  if v_existing.status=200 and jsonb_typeof(coalesce(v_existing.content,'[]')::jsonb)='array' and jsonb_array_length(coalesce(v_existing.content,'[]')::jsonb)>0 then
    ...
    if v_pr_number is not null and v_successor_head ~ '^[0-9a-f]{40}$' and coalesce((v_body->>'draft')::boolean,false)
       and v_body#>>'{base,ref}'='main' then
      update integration_control.penta_pr_restack_execution_receipts_v1
      set state='SUCCEEDED',hold_code=null,successor_pr_number=v_pr_number,successor_head_sha=v_successor_head,provider_http_status=200,
          evidence=evidence||jsonb_build_object('idempotent_existing_successor',true,'provider_readback',true,'authority_created',false),
          completed_at=clock_timestamp(),updated_at=clock_timestamp()
      where assignment_id=a.assignment_id;
      perform extensions.http_reset_curlopt();
      return jsonb_build_object('state','SUCCEEDED','assignment_id',a.assignment_id,'successor_pr_number',v_pr_number,
        'successor_head_sha',v_successor_head,'branch_name',v_branch,'idempotent',true,'authority_created',false);
    end if;
  end if;
  ...
  if v_main.status<>200 then perform extensions.http_reset_curlopt(); return integration_control.penta_pr_restack_hold_v1(a.assignment_id,'HOLD_GITHUB_MAIN_READBACK_FAILED',v_main.status,'{}'::jsonb); end if;
  v_main_body:=coalesce(v_main.content,'{}')::jsonb;
  if v_main_body#>>'{commit,sha}' is distinct from v_main_sha then perform extensions.http_reset_curlopt(); return integration_control.penta_pr_restack_hold_v1(a.assignment_id,'HOLD_CURRENT_MAIN_DRIFT',200,jsonb_build_object('expected_main_sha',v_main_sha,'observed_main_sha',v_main_body#>>'{commit,sha}')); end if;
  v_base_tree:=v_main_body#>>'{commit,commit,tree,sha}';
  if v_base_tree !~ '^[0-9a-f]{40}$' then perform extensions.http_reset_curlopt(); return integration_control.penta_pr_restack_hold_v1(a.assignment_id,'HOLD_MAIN_TREE_READBACK_FAILED',200,'{}'::jsonb); end if;

  select * into v_pred from chlom_runtime.dail_http_v1((
    'GET','https://api.github.com/repos/'||v_repo||'/pulls/'||a.source_pr_number::text,
    array[extensions.http_header('Authorization','Bearer '||v_token),extensions.http_header('Accept','application/vnd.github+json'),extensions.http_header('X-GitHub-Api-Version','2022-11-28'),extensions.http_header('User-Agent','CrownThrive-PentaPR-Restack/1.0')],null,null
  )::extensions.http_request);
  if v_pred.status<>200 then perform extensions.http_reset_curlopt(); return integration_control.penta_pr_restack_hold_v1(a.assignment_id,'HOLD_PREDECESSOR_READBACK_FAILED',v_pred.status,'{}'::jsonb); end if;
  v_pred_body:=coalesce(v_pred.content,'{}')::jsonb;
  if v_pred_body->>'state'<>'open' or coalesce((v_pred_body->>'merged')::boolean,false) then perform extensions.http_reset_curlopt(); return integration_control.penta_pr_restack_hold_v1(a.assignment_id,'HOLD_PREDECESSOR_NOT_OPEN',200,'{}'::jsonb); end if;
  if v_pred_body#>>'{head,sha}' is distinct from a.exact_head_sha then perform extensions.http_reset_curlopt(); return integration_control.penta_pr_restack_hold_v1(a.assignment_id,'HOLD_PREDECESSOR_HEAD_DRIFT',200,jsonb_build_object('expected_head',a.exact_head_sha,'observed_head',v_pred_body#>>'{head,sha}')); end if;
  ...
  if v_files.status<>200 then perform extensions.http_reset_curlopt(); return integration_control.penta_pr_restack_hold_v1(a.assignment_id,'HOLD_PREDECESSOR_DIFF_READBACK_FAILED',v_files.status,'{}'::jsonb); end if;
  v_files_body:=coalesce(v_files.content,'[]')::jsonb;
  if jsonb_typeof(v_files_body)<>'array' or jsonb_array_length(v_files_body)=0 then perform extensions.http_reset_curlopt(); return integration_control.penta_pr_restack_hold_v1(a.assignment_id,'HOLD_PREDECESSOR_ZERO_DELTA_RECLASSIFY',200,'{}'::jsonb); end if;
  if jsonb_array_length(v_files_body)>=100 then perform extensions.http_reset_curlopt(); return integration_control.penta_pr_restack_hold_v1(a.assignment_id,'HOLD_RESTACK_DIFF_PAGINATION_REQUIRED',200,jsonb_build_object('first_page_count',jsonb_array_length(v_files_body))); end if;
  v_changed:=jsonb_array_length(v_files_body);
  select jsonb_agg(jsonb_build_object('path',f->>'filename','mode','100644','type','blob','sha',case when f->>'status'='removed' then null else f->>'sha' end))
    into v_tree_entries from jsonb_array_elements(v_files_body) f;
  ...
  if v_tree.status<>201 then perform extensions.http_reset_curlopt(); return integration_control.penta_pr_restack_hold_v1(a.assignment_id,'HOLD_RESTACK_TREE_CREATE_FAILED',v_tree.status,'{}'::jsonb); end if;
  ...
  if v_commit.status<>201 then perform extensions.http_reset_curlopt(); return integration_control.penta_pr_restack_hold_v1(a.assignment_id,'HOLD_RESTACK_COMMIT_CREATE_FAILED',v_commit.status,'{}'::jsonb); end if;
  v_successor_head:=(coalesce(v_commit.content,'{}')::jsonb)->>'sha';
  if v_successor_head !~ '^[0-9a-f]{40}$' then perform extensions.http_reset_curlopt(); return integration_control.penta_pr_restack_hold_v1(a.assignment_id,'HOLD_RESTACK_COMMIT_READBACK_FAILED',201,'{}'::jsonb); end if;
  ...
  if v_ref.status<>201 then perform extensions.http_reset_curlopt(); return integration_control.penta_pr_restack_hold_v1(a.assignment_id,'HOLD_RESTACK_BRANCH_CREATE_FAILED',v_ref.status,jsonb_build_object('successor_head_sha',v_successor_head)); end if;
  ...
  if v_pr.status<>201 then perform extensions.http_reset_curlopt(); return integration_control.penta_pr_restack_hold_v1(a.assignment_id,'HOLD_RESTACK_PR_CREATE_FAILED',v_pr.status,jsonb_build_object('branch_name',v_branch,'successor_head_sha',v_successor_head)); end if;
  v_pr_number:=nullif((coalesce(v_pr.content,'{}')::jsonb)->>'number','')::bigint;
  if v_pr_number is null then perform extensions.http_reset_curlopt(); return integration_control.penta_pr_restack_hold_v1(a.assignment_id,'HOLD_RESTACK_PR_CREATE_READBACK_FAILED',201,'{}'::jsonb); end if;

  select * into v_read from chlom_runtime.dail_http_v1((
    'GET','https://api.github.com/repos/'||v_repo||'/pulls/'||v_pr_number::text,
    array[extensions.http_header('Authorization','Bearer '||v_token),extensions.http_header('Accept','application/vnd.github+json'),extensions.http_header('X-GitHub-Api-Version','2022-11-28'),extensions.http_header('User-Agent','CrownThrive-PentaPR-Restack/1.0')],null,null
  )::extensions.http_request);
  if v_read.status<>200 then perform extensions.http_reset_curlopt(); return integration_control.penta_pr_restack_hold_v1(a.assignment_id,'HOLD_RESTACK_PR_READBACK_FAILED',v_read.status,'{}'::jsonb); end if;
  ...
  if v_read_body->>'state'<>'open' or not coalesce((v_read_body->>'draft')::boolean,false)
    ...
    perform extensions.http_reset_curlopt();
    return integration_control.penta_pr_restack_hold_v1(a.assignment_id,'HOLD_RESTACK_PR_READBACK_FAILED',200,
      jsonb_build_object('successor_pr_number',v_pr_number,'successor_head_sha',v_successor_head));
  end if;
  ...
  set state='SUCCEEDED',hold_code=null,provider_http_status=200,successor_pr_number=v_pr_number,successor_head_sha=v_successor_head,
      evidence=evidence||jsonb_build_object('changed_files',v_changed,'base_main_sha',v_main_sha,'predecessor_head_sha',a.exact_head_sha,
        'provider_readback',true,'predecessor_preserved',true,'merge_performed',false,'predecessor_close_performed',false,
        'certification_claimed',false,'authority_created',false),completed_at=clock_timestamp(),updated_at=clock_timestamp()
  where assignment_id=a.assignment_id;
  v_event:=chlom_runtime.append_dail_event('penta.pr.restack.owner_execution.succeeded','penta_assignment',a.assignment_id::text,
    jsonb_build_object('source_repo',v_repo,'source_pr_number',a.source_pr_number,'predecessor_head_sha',a.exact_head_sha,
      'base_main_sha',v_main_sha,'successor_pr_number',v_pr_number,'successor_head_sha',v_successor_head,'successor_branch',v_branch,
      'changed_files',v_changed,'predecessor_preserved',true,'merge_performed',false,'predecessor_close_performed',false,
      'certification_claimed',false,'D3_human_reserved',true,'authority_created',false),
    'PentaPR/PentaMerge/PentaPM',null,'PentaPR','1.0.0','ctcorr:penta-pr-restack:'||a.assignment_id::text,null,
    'ct.penta.pr-terminalization-policy.v2',null,'internal');
  perform extensions.http_reset_curlopt();
  return jsonb_build_object('state','SUCCEEDED','assignment_id',a.assignment_id,'successor_pr_number',v_pr_number,
    'successor_head_sha',v_successor_head,'branch_name',v_branch,'changed_files',v_changed,'dail',v_event,'authority_created',false);
exception when others then
  perform extensions.http_reset_curlopt();
  raise;
end
$function$;
revoke all on function integration_control.penta_pr_restack_execute_assignment_v1(uuid) from public,anon,authenticated;
grant execute on function integration_control.penta_pr_restack_execute_assignment_v1(uuid) to service_role;
...
begin
  ...
  for h in
    select x.handoff_key,x.discovery_key,x.risk_class,x.payload,nullif(x.payload->>'assignment_id','')::uuid as assignment_id
  ...
  loop
    ...
    if v_result->>'state' in ('SUCCEEDED','SUCCEEDED_IDEMPOTENT') then
      update integration_control.penta_census_handoffs_v1 set state='acknowledged',
        payload=payload||jsonb_build_object('restack_execution',v_result,'authority_created',false),updated_at=now()
      where discovery_key=h.discovery_key and tag='assignment:PR_RESTACK_CURRENT_MAIN' and risk_class in ('D0','D1','D2');
      v_success:=v_success+1;
    else
      update integration_control.penta_census_handoffs_v1 set state='failed',
        payload=payload||jsonb_build_object('restack_execution',v_result,'authority_created',false),updated_at=now()
      where discovery_key=h.discovery_key and tag='assignment:PR_RESTACK_CURRENT_MAIN' and risk_class in ('D0','D1','D2');
      v_holds:=v_holds+1;
    end if;
  end loop;
  return jsonb_build_object('state',case when v_holds>0 then 'SUCCESS_HOLD' else 'SUCCESS_PROGRESS' end,
    'assignments_seen',coalesce(array_length(v_seen,1),0),'success',v_success,'holds',v_holds,
    'D3_executed',false,'authority_created',false,'observed_at',clock_timestamp());
end
$function$;
revoke all on function integration_control.penta_pr_restack_mobilize_v1(integer) from public,anon,authenticated;
grant execute on function integration_control.penta_pr_restack_mobilize_v1(integer) to service_role;
