-- Enforce both active and intentionally inactive scheduler desired states.
...
declare
  ...
  d record; c record; x record; v_action text; v_digest text;
  v_checked int:=0; v_restored int:=0; v_deactivated int:=0; v_healthy int:=0; v_failed int:=0; v_row_deactivated int;
begin
  ...
  for d in
    select * from integration_control.scheduler_desired_jobs_v2
    where (active=true and allow_auto_restore=true) or active=false
    order by jobname
  loop
    ...
    begin
      if d.active=false then
        v_row_deactivated:=0;
        for x in select jobid from cron.job where jobname=d.jobname and active order by jobid loop
          perform cron.alter_job(x.jobid,active=>false);
          v_row_deactivated:=v_row_deactivated+1;
        end loop;
        if v_row_deactivated>0 then
          v_action:='deactivated_desired_inactive';
          v_deactivated:=v_deactivated+v_row_deactivated;
          v_restored:=v_restored+1;
        else
          v_action:='healthy_inactive';
          v_healthy:=v_healthy+1;
        end if;
      elsif c.jobid is null then
      ...
      v_digest:=encode(extensions.digest(convert_to(jsonb_build_object(
        ...
        'observed_schedule',c.schedule,'observed_command',c.command,'observed_active',c.active,
        'desired_schedule',d.schedule,'desired_command',d.command,'desired_active',d.active
      )::text,'UTF8'),'sha256'),'hex');
      if v_action not in ('healthy','healthy_inactive') then
        insert into integration_control.scheduler_reconcile_events_v2(
          ...
          jsonb_build_object('schedule',d.schedule,'command',d.command,'active',d.active,'source_ref',d.source_ref),
    ...
    exception when others then
      v_failed:=v_failed+1;
      v_digest:=encode(extensions.digest(convert_to(jsonb_build_object(
        'jobname',d.jobname,'generation',d.generation,'error_state',sqlstate,'desired_active',d.active
      )::text,'UTF8'),'sha256'),'hex');
      insert into integration_control.scheduler_reconcile_events_v2(
        jobname,generation,observed_state,desired_state,action,result_state,evidence_sha256
      ) values (
        d.jobname,d.generation,jsonb_build_object('jobid',c.jobid,'schedule',c.schedule,'active',c.active),
        jsonb_build_object('schedule',d.schedule,'active',d.active,'source_ref',d.source_ref),
  ...
  return jsonb_build_object(
    ...
    'checked',v_checked,'healthy',v_healthy,'restored',v_restored,'deactivated',v_deactivated,'failed',v_failed,
    'mutation_api','cron.alter_job','job_identity_preserved',true,
    'database_and_username_preserved',true,'inactive_desired_state_enforced',true,
...
begin
  v_payload:=jsonb_build_object('contract','ct.scheduler.permanence.v2.inactive-enforcement',
    'active_desired_state_enforced',true,'inactive_desired_state_enforced',true,
    'missing_inactive_job_is_healthy',true,'active_retired_job_is_deactivated_in_place',true,
    'job_identity_preserved',true,'D3_human_reserved',true,'provider_write',false,'credential_material',false,
  ...
  insert into penta_self.action_receipts_v1(cycle_id,capability_key,action_key,target_ref,result_state,reversible,authority_class,after_sha256,evidence)
    values(v_cycle,'self.schedule','install_scheduler_inactive_enforcement_v2','scheduler-fabric:PentaTime',
      'applied',true,'D1',v_sha,v_payload);
  perform chlom_runtime.append_dail_event('scheduler.permanence.inactive-enforcement-installed','scheduler_security',
    'ct.scheduler.permanence.v2',v_payload,'PentaSELF/PentaTime/PentaAssure',null,'PentaTime','2.2.0',v_sha,null,
