-- CrownThrive OS full captured-intervention readback.
-- Provider-applied in ThriveBase as migration 20260904022111.
...
begin
  ...
  from (
    ...
    select updated_at as occurred_at,
           ...
           case
             when state in ('VERIFIED','SUCCEEDED','COMPLETED','CLOSED') then 'READBACK'
             when state in ('RUNNING','EXECUTING','APPLYING') then 'APPLY'
             when state in ('QUEUED','READY','CLAIMED') then 'PLAN'
             else 'OBSERVE'
           end::text as stage,
