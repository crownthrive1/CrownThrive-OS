create or replace function integration_control.penta_ads_production_stable_103_certifier_invoke_v1(p_head_sha text)
returns jsonb
language plpgsql
security definer
...
declare
  ...
  v_body jsonb;
begin
  if p_head_sha !~ '^[a-f0-9]{40}$' then raise exception 'penta_ads_103_head_sha_invalid'; end if;
  ...
  if nullif(v_jwt,'') is null or nullif(v_control,'') is null then raise exception 'penta_ads_103_certifier_control_missing'; end if;
  perform extensions.http_set_curlopt('CURLOPT_TIMEOUT_MS','120000');
  perform extensions.http_set_curlopt('CURLOPT_CONNECTTIMEOUT_MS','5000');
  v_resp:=chlom_runtime.dail_http_v1((
    'POST',
    'https://tzajnzshmtzjenqulehq.supabase.co/functions/v1/pentaads-production-stable-103-certifier',
    array[
    ...
    'application/json',
    jsonb_build_object('head_sha',p_head_sha)::text
...
$function$;
revoke all on function integration_control.penta_ads_production_stable_103_certifier_invoke_v1(text) from public, anon, authenticated;
grant execute on function integration_control.penta_ads_production_stable_103_certifier_invoke_v1(text) to service_role;
