-- Rollback for 20260901215319_penta_wire_intentional_gate_metadata_normalization_v1
-- Restores only the exact eight pre-migration notes. It does not alter enabled,
-- schema, risk, approval, credentials, or provider state.
...
begin
  ...
  if v_changed <> 8 then
    raise exception 'penta_wire_intentional_gate_metadata_rollback_expected_8_changed_got_%', v_changed;
  end if;
end
$$;
