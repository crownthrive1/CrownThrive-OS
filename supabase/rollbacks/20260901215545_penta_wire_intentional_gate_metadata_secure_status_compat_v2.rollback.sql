-- Rollback for 20260901215545_penta_wire_intentional_gate_metadata_secure_status_compat_v2
-- Restores the exact v1-prefixed intermediate metadata so v1 can then be
-- rolled back independently if required.
...
begin
  with expected(tool_name, old_notes) as (
    ...
    update integration_control.mcp_tools t
       set notes = 'INTENTIONALLY GATED: ' || e.old_notes,
           updated_at = clock_timestamp()
      from expected e
     where t.tool_name = e.tool_name
       and t.notes = e.old_notes || ' Lifecycle: INTENTIONALLY GATED.'
       and t.enabled = false
  ...
  if v_changed <> 8 then
    raise exception 'penta_wire_intentional_gate_secure_status_compat_rollback_expected_8_changed_got_%', v_changed;
  end if;
end
$$;
