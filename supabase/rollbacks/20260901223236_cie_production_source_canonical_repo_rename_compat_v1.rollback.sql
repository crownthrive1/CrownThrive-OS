-- Exact rollback for 20260901223236_cie_production_source_canonical_repo_rename_compat_v1.sql
-- This is not auto-executed. It restores the precise pre-migration function definition only from the expected post-state.
DO $rollback$
DECLARE
  v_oid oid;
  v_def text;
...
BEGIN
  ...
  IF v_pre_sha <> '5e41edbc4d5115e3fb05c18bd0e8a8f2643cd21ec9728fe388f39c0196627c47'
     OR v_legacy_count <> 0 OR v_new_count <> 2 THEN
    RAISE EXCEPTION 'unexpected_cie_reauthorize_rollback_prestate sha=% legacy=% new=%', v_pre_sha, v_legacy_count, v_new_count;
  END IF;

  EXECUTE replace(v_def,'crownthrive1/CrownThrive-CIE-OS','crownthrive1/CrownThrive-CIE');
  ...
  IF v_post_sha <> '0ba29d608fb27267d842e70d74ecd8d66391ffaadc595ee91dd9825e6bd10384'
     OR v_legacy_count <> 2 OR v_new_count <> 0 THEN
    RAISE EXCEPTION 'cie_reauthorize_rollback_poststate_failed sha=% legacy=% new=%', v_post_sha, v_legacy_count, v_new_count;
  END IF;
END
$rollback$;
