...
async function actor(req: Request) {
  ...
  return {
    ...
    oidc_role: "execution_identity_attestation_only",
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === "claim") {
      ...
      return j({ ok: true, server: SERVER, actor: a, job: data, authority_effect: "none" });
    }
    if (action === "complete") {
      ...
      return j({ ok: true, server: SERVER, actor: a, result: data, authority_effect: "none" });
    }
    if (action === "release") {
      const jobId = String(body.job_id ?? "");
      if (!jobId) throw new Error("job_id_required");
    ...
    return j({ ok: false, error: message, stage, server: SERVER, authority_effect: "none" }, status);
  }
});
