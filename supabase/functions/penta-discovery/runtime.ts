...
const RUNTIME_RELEASE = "ct.penta.discovery.runtime.2.0.1";
...
function legacyServiceRoleClaim(token: string): boolean {
  const parts = token.split(".");
  if (parts.length !== 3) return false;
  try {
    const payload = JSON.parse(decodeBase64Url(parts[1])) as JsonRecord;
...
function assertServiceRole(req: Request): void {
  ...
  if (!token) throw new SafeError("service_role_authorization_required", 403);

  // Modern Supabase service credentials are opaque. Exact comparison is the
  // primary path. A gateway-validated legacy service_role JWT remains accepted
  // for callers that have not yet rotated, preserving the stable v1 contract.
  if (constantTimeEqual(token, SERVICE_ROLE_KEY)) return;
  if (legacyServiceRoleClaim(token)) return;
  throw new SafeError("service_role_authorization_required", 403);
}
...
function stringValue(
  ...
  if (normalized.length > maxLength) throw new SafeError(`${field}_too_long`, 400);
  return normalized;
}
...
function unsafeV6(value: string): boolean {
  ...
  if (/^(fc|fd)/.test(host) || /^fe[89ab]/.test(host) || host.startsWith("ff")) return true;
...
async function boundedFetch(raw: string): Promise<JsonRecord> {
  ...
  for (let redirect = 0; redirect <= MAX_REDIRECTS; redirect++) {
    ...
    if ([301, 302, 303, 307, 308].includes(response.status)) {
      ...
      if (redirect >= MAX_REDIRECTS) throw new SafeError("redirect_limit_exceeded", 502);
    ...
    return {
      ...
      byte_length: encoder.encode(content).byteLength,
...
function normalizeParsedContent(content: string): unknown {
  try {
    ...
    return matched > 0 ? record : content.replace(/\s+/g, " ").trim().slice(0, 16_000);
  }
}

async function familyRows(): Promise<JsonRecord[]> {
  return await selectRows("penta_discovery", "family_registry_v1", new URLSearchParams({
    select: "member_key,canonical_name,family_role,execution_role,lifecycle_state,packet_contract,fabric_contract,authority_manufacture,metadata,updated_at",
...
async function systemRows(): Promise<JsonRecord[]> {
  return await selectRows("public", "penta_system_registry", new URLSearchParams({
    select: "system_key,canonical_name,category,purpose,authority_boundary,risk_ceiling,maturity,version,runtime_ref,metadata,last_verified_at,updated_at",
  ...
  switch (action) {
    ...
    case "status": {
      const [status, crawler] = await Promise.all([
        rpc("penta_discovery_status_v2"),
        rpc("penta_crawler_status_v3", {}, "public"),
      ]);
      return { contract: CONTRACT, status, crawler };
    }
    case "guard_status":
      return await rpc("penta_discovery_runtime_guard_status_v1");
    case "family":
    ...
    case "observe":
      return await rpc("intake_v1", {
        ...
        p_observed_subject: stringValue(body.observed_subject, "observed_subject", 1000),
        p_observation: objectValue(body.observation, "observation"),
        p_confidence: numberValue(body.confidence, "confidence", 0, 1, 0.75),
        p_receiver_penta_ref: stringValue(body.receiver_penta_ref ?? "penta.census", "receiver_penta_ref", 200),
        p_objective: stringValue(body.objective ?? "classify_register_route", "objective", 500),
        p_route_class: stringValue(body.route_class ?? "local_internal", "route_class", 100),
      }, "penta_discovery");
    case "search": {
      ...
      const results = systems.filter((row) => [
        row.system_key, row.canonical_name, row.category, row.purpose,
        row.runtime_ref, JSON.stringify(row.metadata ?? {}),
      ].join(" ").toLowerCase().includes(query)).slice(0, limit);
      return { query, count: results.length, results };
    }
    case "get": {
      const systemKey = stringValue(body.system_key, "system_key", 300);
      const rows = await selectRows("public", "penta_system_registry", new URLSearchParams({
        select: "*", system_key: `eq.${systemKey}`, limit: "1",
      }));
      if (rows.length === 0) throw new SafeError("system_not_found", 404);
      return rows[0];
    }
    case "resolve": {
      const identity = safeSearch(body.identity ?? body.query ?? body.system_key).toLowerCase();
      ...
      const candidates = [...systems, ...members]
        .filter((row) => JSON.stringify(row).toLowerCase().includes(identity))
        .slice(0, 10);
      return { identity, confidence: candidates.length === 1 ? 0.85 : 0.5, candidates };
    }
    case "parse": {
      const content = stringValue(body.content, "content", 64 * 1024);
      return { content_sha256: await sha256(content), parsed: normalizeParsedContent(content) };
    }
    case "fetch": {
      const fetched = await boundedFetch(stringValue(body.url, "url", 2000));
      ...
      if (preserve) {
        intake = await rpc("intake_v1", {
          ...
          p_route_class: stringValue(body.route_class ?? "provider_read", "route_class", 100),
        }, "penta_discovery");
      }
      return {
        url: fetched.url,
    ...
    case "economics_finalize":
      return await rpc("finalize_penta_packet_economics_v2", {
        p_packet_id: uuidValue(body.packet_id, "packet_id"),
        p_terminal_state: stringValue(body.terminal_state, "terminal_state", 100),
      });
    ...
    case "signal":
      return await rpc("penta_route_ingest_signal_v2", {
        p_source_penta_ref: stringValue(body.source_penta_ref, "source_penta_ref", 200),
        p_signal_kind: stringValue(body.signal_kind, "signal_kind", 100),
        p_numeric_value: numberValue(body.numeric_value, "numeric_value", -1_000_000_000, 1_000_000_000),
        ...
        p_subject_penta_ref: stringValue(body.subject_penta_ref, "subject_penta_ref", 200),
        p_receiver_penta_ref: stringValue(body.receiver_penta_ref, "receiver_penta_ref", 200),
        p_currency: stringValue(body.currency ?? "USD", "currency", 3),
        p_ttl_seconds: intValue(body.ttl_seconds, "ttl_seconds", 60, 604800, 3600),
        p_signal_key: stringValue(body.signal_key ?? crypto.randomUUID(), "signal_key", 300),
    ...
    case "governance_reconcile":
      return await rpc("reconcile_route_pay_signal_v1", {
        ...
        p_source_penta_ref: stringValue(body.source_penta_ref, "source_penta_ref", 200),
        p_requested_change_pct: numberValue(body.requested_change_pct, "requested_change_pct", -0.9, 10),
        p_confidence: numberValue(body.confidence, "confidence", 0, 1),
        p_evidence_refs: objectValue(body.evidence_refs, "evidence_refs"),
        p_executive_ref: body.executive_ref ? stringValue(body.executive_ref, "executive_ref", 500) : null,
        p_legislative_ref: body.legislative_ref ? stringValue(body.legislative_ref, "legislative_ref", 500) : null,
        p_judicial_ref: body.judicial_ref ? stringValue(body.judicial_ref, "judicial_ref", 500) : null,
      });
    // Stable v1 aliases. Automatic packet triggers continue to use v2 economics.
    case "reserve":
      return await rpc("reserve_penta_route_v1", {
        p_quote_id: uuidValue(body.quote_id, "quote_id"),
        p_authority_evidence: objectValue(body.authority_evidence, "authority_evidence"),
      });
    case "usage":
      return await rpc("record_penta_route_usage_v1", {
        ...
        p_route_edge_ref: stringValue(body.route_edge_ref, "route_edge_ref", 500),
        p_disposition: stringValue(body.disposition, "disposition", 100),
        p_actual_internal_units: intValue(body.actual_internal_units, "actual_internal_units", 0, 1_000_000_000),
        p_actual_provider_cost_minor: intValue(body.actual_provider_cost_minor, "actual_provider_cost_minor", 0, 1_000_000_000),
        p_currency: stringValue(body.currency ?? "USD", "currency", 3),
        p_metadata: objectValue(body.metadata, "metadata"),
      });
    case "reconcile":
      return await rpc("reconcile_penta_route_v1", {
        p_reservation_id: uuidValue(body.reservation_id, "reservation_id"),
        p_beneficiary_ref: stringValue(body.beneficiary_ref, "beneficiary_ref", 300),
        p_pay_rate_minor_per_1000_units: intValue(body.pay_rate_minor_per_1000_units, "pay_rate_minor_per_1000_units", 0, 1_000_000_000),
        p_chlom_authority_ref: stringValue(body.chlom_authority_ref, "chlom_authority_ref", 1000),
        p_dail_evidence_ref: stringValue(body.dail_evidence_ref, "dail_evidence_ref", 1000),
      });
    default:
...
Deno.serve(async (req: Request) => {
  ...
  try {
    if (req.method === "OPTIONS") return responseJson({ ok: true }, 200, requestId);
    if (req.method !== "POST" && req.method !== "GET") {
      throw new SafeError("method_not_allowed", 405);
    }

    // Supabase verify_jwt remains enabled. Exact service-role equivalence is
    // required inside the runtime; legacy service_role JWTs remain compatible.
