...
<Note>
  ...
  **This page:** Website Patch Workflow — The controlled GitHub and Codex workflow for investigating, implementing, testing, approving, deploying, and documenting a website change.
...
- documentation-impact state: affected articles/registries/changelog, reviewed no-impact declaration, or governed docs delta
...
## Step 11 — Reconcile documentation and institutional state

The documentation impact should normally be prepared in the same branch/change set rather than deferred until after deployment. After the production smoke test, reconcile the planned docs against actual behavior and update or confirm:

- issue and pull request;
- release notes/changelog;
- architecture decision when applicable;
- affected platform/capability/API/vendor/route registry;
- affected customer/operator/developer/support articles;
- article/page version, effective date and source evidence;
- known issues and incident state;
- phase-gate implications;
- rollback/last-known-good reference.

A material release may close only with `docs_updated`, an approved `docs_no_change`, or a governed `docs_delta_opened` whose risk class permits post-release completion. D3 changes do not use an open docs delta to bypass pre-release disclosure or acceptance requirements.
