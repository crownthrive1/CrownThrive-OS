...
function headers(method, name, overrides = {}) {
  const out = {
...
async function rawRequest({ body, requestHeaders, method = 'POST' }) {
  const response = await fetch(serverUrl, { method, headers: requestHeaders, body });
  const text = await response.text();
  let json = null;
  try { json = text ? JSON.parse(text) : null; } catch {}
  return { status: response.status, headers: Object.fromEntries(response.headers.entries()), text, json };
}
async function rpcRequest(id, method, params = {}, name = null, headerOverrides = {}) {
  return rawRequest({
    requestHeaders: headers(method, name, headerOverrides),
    body: JSON.stringify({ jsonrpc: '2.0', id, method, params: { ...params, _meta: requiredMeta(params?._meta || {}) } })
  });
}
function isGenericObjectSchema(schema) {
  if (!schema || typeof schema !== 'object' || Array.isArray(schema)) return true;
  const keys = Object.keys(schema).filter((key) => key !== '$schema' && key !== 'description' && key !== 'title');
  return keys.length === 1 && schema.type === 'object';
}
function boundedInputSchema(schema) {
  return Boolean(schema && typeof schema === 'object' && !Array.isArray(schema) && schema.type === 'object' && schema.additionalProperties === false);
}
function boundedOutputSchema(schema) {
  if (!schema || typeof schema !== 'object' || Array.isArray(schema) || isGenericObjectSchema(schema)) return false;
  return Boolean(
    schema.properties || schema.required || schema.oneOf || schema.anyOf || schema.allOf ||
    schema.$ref || schema.const !== undefined || schema.enum || schema.additionalProperties === false ||
    schema.maxProperties !== undefined || schema.maxItems !== undefined
  );
}
...
function runStaticPreflight() {
  ...
  assert(manifest.post_signature_execution.provider_read_budget.crownthrive_io === 1, 'PRE-010', 'CrownThrive IO live certification budget is bounded to one read');
  assert(manifest.post_signature_execution.provider_read_budget.thrivetools_seo === 1, 'PRE-011', 'ThriveTools SEO live certification budget is bounded to one read');
...
async function runLive() {
  ...
  const list1 = await rpcRequest('tools-1', 'tools/list');
  const list2 = await rpcRequest('tools-2', 'tools/list');
  assert(list1.status === 200 && list2.status === 200, 'EXT-002A', 'tools/list succeeds twice');
  const tools1 = list1.json?.result?.tools;
  const tools2 = list2.json?.result?.tools;
  ...
  const unsupported = await rawRequest({
    ...
    body: JSON.stringify({ jsonrpc: '2.0', id: 'version-1', method: 'tools/list', params: { _meta: { ...requiredMeta(), 'io.modelcontextprotocol/protocolVersion': '1900-01-01' } } })
  });
  ...
  if (!allowProviderReads) {
    results.push({ id: 'EXT-013', pass: false, skipped: true, detail: 'provider-read proof held because MCP_CERTIFICATION_ALLOW_PROVIDER_READS is not true' });
  } else {
    const io = await rpcRequest('io-live-1', 'tools/call', { name: 'crownthrive_io_get_user', arguments: {} }, 'crownthrive_io_get_user');
    ...
    const seo = await rpcRequest('seo-live-1', 'tools/call', { name: 'seo.user.read', arguments: {} }, 'seo.user.read');
...
try {
  const report = live ? await runLive() : runStaticPreflight();
