...
- exact live runtime credential verification for both declared Stripe account roles before provider writes;
- `integration_control.stripe_os_provider_request_v1` as the concrete CrownThrive runtime adapter;
- `integration_control.pentagreen_stripe_commerce_binder_tick_v1` as the existing PentaGreen Commerce Binder executor;
- exactly one canonical PentaGreen commerce clock; the legacy ThriveEvergreen alias is superseded and non-restorable.
