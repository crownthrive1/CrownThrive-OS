...
<Info>
  The current machine-read production architecture for Virality Music, Crown Credits, Stripe/Connect, stablecoins, CHLOM Wallet, Trust Wallet Agent Kit, the managed Base/USDC Agent Wallet, Changelly, and PentaGreen™ (legacy alias: ThriveEvergreen™) is documented in [Production Hybrid Commerce Convergence — 2026-08-23](/commerce/production-hybrid-commerce-convergence-2026-08-23). The current managed-wallet custody/signing boundary is documented in [Managed Agent Wallet](/commerce/chlom-managed-agent-wallet-base-usdc), with Trust Wallet Agent Kit retained as the multichain intelligence/routing plane. The embedded-wallet, funding and WalletConnect contract is documented in [MetaMask Embedded Wallets Funding & Dual Wallet Execution](/commerce/metamask-embedded-wallets-funding-and-dual-wallet).
</Info>
