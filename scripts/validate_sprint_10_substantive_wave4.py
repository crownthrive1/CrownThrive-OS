...
if not (0 < result["selected_count"] < result["p0_candidate_count"]):
    errors.append("Wave 4 must select a bounded nonzero P0 subset")
if result["selected_count"] + result["held_count"] != 795:
    errors.append("selected + held must equal 795")
if result["cumulative_machine_qualified_p0_count"] != 52 + result["selected_count"]:
    errors.append("cumulative qualified count mismatch")
if result["p0_outside_waves_1_2_3_4_count"] != 449 - result["cumulative_machine_qualified_p0_count"]:
...
for row in result["selected_records"]:
    ...
    if row["canonical_anchor_route"] != "/chlom/component-framework-reconciliation-contract":
