#!/usr/bin/env python3
"""Inspect the remaining P0 CHLOM AI/agent/algorithm family for a safe Sprint 11 pivot.

Diagnostic only: no qualification, activation, or terminal disposition is created.
"""
...
candidates = [
    ...
    and row.get("current_state_candidate") == "ai_agent_algorithm_reconciliation"
    and row.get("disposition_candidate") == "merged_successor"
    and not row.get("missing_target_routes")
]

print("PASS_SPRINT_11_AI_PIVOT_INSPECTION")
...
for row in candidates:
    ...
    print("AI_CANDIDATE=" + json.dumps(payload, ensure_ascii=False, sort_keys=True))
