...
A release is eligible only when exact-version evidence proves the ordered topology:
Build -> security scan -> threat model -> tests -> PentaSecurity -> CHLOM rights/authority
-> applicable CIE -> independent PentaCertifier -> release execution.
...
def verify(
    ...
    """Return RELEASE_ELIGIBLE only when the full mandatory topology is proven."""
    checks: list[dict[str, str]] = []
    ...
    if release.get("repository") != "crownthrive1/CrownThrive-OS" or not _HEX40.fullmatch(release_sha):
        checks.append(_fail("release.exact_head", "canonical repository and lowercase 40-character commit SHA are required"))
    else:
    ...
    if security_actor and security_actor in producer_ids:
        checks.append(_fail("penta_security.independence", "PentaSecurity decision actor must be independent of originator/build producers"))
    elif security_actor:
    ...
    if isinstance(chlom, dict):
        if chlom.get("rights_check") != "PASS" or chlom.get("authority_check") != "PASS":
            checks.append(_fail("chlom_authority_rights.scope", "both rights_check and authority_check must PASS"))
        else:
            checks.append(_pass("chlom_authority_rights.scope"))
        if chlom.get("authority_expansion") is not False:
            checks.append(_fail("chlom_authority_rights.authority_expansion", "release evidence may not manufacture authority expansion"))
        else:
            checks.append(_pass("chlom_authority_rights.authority_expansion"))
        if chlom.get("final_legal_or_rights_commitment") is not False:
            checks.append(_fail("chlom_authority_rights.final_commitment", "final legal/rights commitments remain separately gated"))
        else:
            checks.append(_pass("chlom_authority_rights.final_commitment"))
    if chlom_actor and chlom_actor in producer_ids:
        checks.append(_fail("chlom_authority_rights.independence", "CHLOM authority/rights decision actor must be independent of the originator/build producers"))
    elif chlom_actor:
    ...
    if cie_required:
        ...
        if cie_actor and cie_actor in producer_ids:
            checks.append(_fail("cie.independence", "applicable CIE decision must be independent of originator/build producers"))
        elif cie_actor:
    ...
    if certifier_actor and certifier_actor in prohibited_certifier_ids:
        checks.append(_fail("penta_certifier.independence", "PentaCertifier cannot be the originator, builder, PentaSecurity, CHLOM, or applicable CIE decision actor"))
    ...
    if rollback.get("tested") is not True or rollback.get("readback_verified") is not True:
        checks.append(_fail("rollback.readback", "rollback test and exact readback evidence are required"))
    else:
    ...
    if risk_class == "D3":
        ...
        if not isinstance(d3_bundle, dict):
            checks.append(_fail("d3.approval", "D3 is human-reserved; exact current approval evidence is required"))
        else:
            ...
            candidate = d3_bundle.get("candidate") if isinstance(d3_bundle.get("candidate"), dict) else {}
            if candidate.get("exact_version_ref") != release_sha:
                checks.append(_fail("d3.exact_release", "D3 candidate must bind the exact release commit"))
            else:
    ...
    if execution_actor and certifier_actor and execution_actor == certifier_actor:
        checks.append(_fail("release_execution.independence", "PentaCertifier cannot also execute the release"))
    ...
    if execution_actor and originator_id and execution_actor == originator_id:
        checks.append(_fail("release_execution.originator", "originator cannot execute its own final release in this topology"))
    elif execution_actor:
        checks.append(_pass("release_execution.originator"))

    token_model = bundle.get("token_model")
    if token_model not in (None, {}, "UNRESOLVED"):
        checks.append(_fail("token_model", "this verifier does not authorize or infer CHLOM token classes from DAIL semantic stages"))
    else:
        checks.append(_pass("token_model"))

    failed = [item for item in checks if item["status"] == "FAIL"]
    return ("RELEASE_ELIGIBLE" if not failed else "RELEASE_HOLD", checks)
...
def main() -> int:
    ...
    return 0 if decision == "RELEASE_ELIGIBLE" else 1
