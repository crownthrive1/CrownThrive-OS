#!/usr/bin/env python3
"""Build Sprint 9 substantive-rebuild Wave 3.

Wave 3 preserves the completed 795-title candidate map and the exact Wave 1/2
selections. It closes a target-side documentation gap by admitting only P0
identity/trust and evidence/audit merged-successor rows against new substantive
current reconciliation contracts. Missing historical bodies remain missing and
terminal dispositions remain pending parent certification.
"""
...
def classify(
    ...
    if state not in set(policy["allowed_state_families"]):
        reasons.append("outside_wave_3_state_family")
...
def build() -> dict[str, Any]:
    ...
    if wave1_ids & wave2_ids:
        raise ValueError("Wave 1 and Wave 2 overlap")
    prior_ids = wave1_ids | wave2_ids
    ...
    if overlap:
        raise ValueError(f"Wave 3 overlaps prior waves: {sorted(overlap)}")
    ...
    payload = {
        ...
        "p0_outside_waves_1_2_3_count": p0_total - cumulative_selected,
...
def main() -> None:
    ...
    result = build()
    print("PASS_SUBSTANTIVE_REBUILD_WAVE3_BUILD")
