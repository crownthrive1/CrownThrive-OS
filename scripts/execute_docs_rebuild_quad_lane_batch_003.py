#!/usr/bin/env python3
"""Execute Wave 7 documentation rebuild quad-lane batch 003.

Batch 003 advances the governed work queue from the 69-qualified / 380-pending
baseline established by AdLuxe substantive gates 001 and 002. It excludes every
identity touched in batches 001 and 002, excludes all already-qualified records,
and executes four bounded lanes across 16 new exact P0 identities.
"""
...
def build() -> dict[str, Any]:
    ...
    prior = q1.prior_ids()
    gate1 = load_json(GATE1)
    gate2 = load_json(GATE2)
    qualified1 = {str(x) for x in gate1["qualified_inventory_ids"]}
    qualified2 = {str(x) for x in gate2["qualified_inventory_ids"]}
    if len(prior) != 61:
        raise ValueError("expected 61 pre-Wave-7 qualified records")
    if len(qualified1) != 4 or len(qualified2) != 4:
        raise ValueError("each Wave 7 substantive gate must contribute exactly four records")
    if prior & qualified1 or prior & qualified2 or qualified1 & qualified2:
        raise ValueError("Wave 7 qualified cohorts overlap")
    qualified = prior | qualified1 | qualified2
    if len(qualified) != 69:
        raise ValueError("expected 69 machine-qualified P0 records")
    ...
    if len(pending_p0) != 380:
        raise ValueError(f"expected 380 pending P0 records, found {len(pending_p0)}")

    batch1 = load_json(BATCH1)
    batch2 = load_json(BATCH2)
    previous_touched = touched_ids(batch1, batch2)
    if len(previous_touched) != 32:
        raise ValueError("batches 001 and 002 must contain 32 unique touched identities")
    ...
    if len(current_ids) != 16:
        raise ValueError("batch 003 must touch exactly 16 unique identities")
    if current_ids & previous_touched:
        raise ValueError("batch 003 must not repeat batches 001 or 002 identities")
    if current_ids & qualified:
        raise ValueError("batch 003 must not touch already-qualified identities")
    ...
    payload: dict[str, Any] = {
        ...
        "execution_id": "ct.docs.rebuild.wave7.quad-lane.batch-003",
        "source_universe_count": 795,
        "p0_candidate_estate": 449,
        "machine_qualified_before_batch": 69,
        "remaining_p0_before_batch": 380,
        "previous_batches_touched_count": 32,
        ...
        "counting": {
            ...
            "official_machine_qualified_after_batch": 69,
            "official_remaining_p0_after_batch": 380,
            "reason": "batch 003 produces governed work receipts only; qualification requires a dedicated acceptance gate",
        },
        "guardrails": {
            "batch_001_or_002_identity_reuse": False,
...
def main() -> None:
    ...
    result = build()
    print("PASS_DOCS_REBUILD_QUAD_LANE_BATCH_003")
    print(f"execution_id={result['execution_id']}")
    for lane in ("A", "B", "C", "D"):
        print(f"lane_{lane}_inventory_ids=" + ",".join(r["inventory_id"] for r in result["lanes"][lane]))
    print("official_machine_qualified_after_batch=69")
    print("official_remaining_p0_after_batch=380")
