#!/usr/bin/env python3
"""Phase 2.99 hard-exit ledger v1.3.1 validator. PASS != hard-exit PASS."""
...
def validate(d,check_files=True):
    eq(d.get("manifest_version"),"1.3.1","manifest version"); ts(d["observed_at"],"observed_at"); sh(d["observed_main_sha"],"main")
    a=d["authority"]
    ...
    r=d["reconciliation"]; eq(r["retroactive_phase_2_0_through_2_9_lane"],"active_until_hard_exit","retroactive lane"); eq(r["restricted_source_final_audit"],"pending","restricted audit"); eq(r["continuity_recovery_final_reproducibility_audit"],"pending","recovery audit"); ni(r["approved_deferral_count_snapshot"],"deferrals")
    ...
    eq(t["pass"]+t["open"]+t["blocked"]+t["closed"]+t["deferred"],t["total"],"tag arithmetic"); eq(t["authoritative"],t["total"],"tag authoritative"); eq(t["scan_required"],t["total"],"tag scan"); eq(t["reconcile_required"],t["total"],"tag reconcile"); eq(t["pass_remains_drift_watched"],True,"PASS watch"); eq(t["deferral_is_not_pass"],True,"deferral semantics"); eq(t["unknown_never_becomes_zero_or_pass"],True,"unknown semantics")
    ...
    eq(m["full_current_scope_scan_required"],True,"full source scan"); eq(m["state"],"partial_current_full_scan_required","M state")
    ...
    if s["formal_scan_coverage_gap"]>0: eq(g["CT-P299-GATE-003"]["state"],"not_met","GATE003 incomplete scan"); eq(g["CT-P299-GATE-003"]["blocking"],True,"GATE003 block")
...
def main():
    ...
    if a.self_test: self_test(d); print("Phase 2.99 ledger v1.3.1 self-test PASS: PR91 canonical, scan gaps explicit, roadmap-v2 pending, GATE008 fail-closed."); return 0
    validate(d); s=d["reconciliation"]["latest_formal_reconciliation_scan"]; print("Phase 2.99 ledger v1.3.1 consistency PASS"); print(f"Current tags={d['reconciliation']['reconciliation_tag_snapshot']['total']}; formal-scan gap={s['formal_scan_coverage_gap']}"); print("Hard exit NOT MET; GATE006 deferred/NOT-PASS; Phase3 blocked."); return 0
