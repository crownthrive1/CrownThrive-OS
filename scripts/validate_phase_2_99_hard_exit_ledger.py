#!/usr/bin/env python3
"""Phase 2.99 hard-exit ledger v1.3.5 validator. PASS != hard-exit PASS."""
...
EXPECTED_68_EXCEPTIONS={
    "S100-PORT-004","S100-PORT-016","S100-PORT-018",
    "S100-PORT-019","S100-PORT-023","S100-PORT-063",
}
EXPECTED_74_EXCEPTIONS={
...
def validate(d,check_files=True):
    eq(d.get("manifest_version"),"1.3.5","manifest version"); ts(d["observed_at"],"observed_at"); sh(d["observed_main_sha"],"main")
    a=d["authority"]
    ...
    for k,n in COUNTS.items():
        eq(u[k]["count"],n,f"{k}.count"); eq(u[k]["source_count_state"],"certified",f"{k}.source")
        eq(u[k]["terminal_disposition_coverage"],n,f"{k}.terminal coverage")
        eq(u[k]["technical_full_current_certification_complete"],False,f"{k}.technical full certification")
        eq(u[k]["hard_exit_certified"],True,f"{k}.GATE001 certification")
    eq(u["holdings_portfolio_rows"]["typed_resolved_or_classified"]+u["holdings_portfolio_rows"]["terminally_dispositioned_unresolved"],68,"68 terminal arithmetic")
    eq(u["holdings_portfolio_rows"]["pending_identity_resolution"],6,"68 unresolved identity preservation")
    eq(u["phase_2_7_platform_framework_rows"]["typed_resolved_or_classified"]+u["phase_2_7_platform_framework_rows"]["terminally_dispositioned_unresolved"],74,"74 terminal arithmetic")
    eq(u["phase_2_7_platform_framework_rows"]["unresolved_current_identity"],20,"74 unresolved identity preservation")
    eq(u["holdings_domain_rows"]["prospective_phase20_gate_issue"],128,"82 phase20 authority")

    g1=d["gate_001_terminal_disposition"]
    eq(g1["authority_issue"],114,"GATE001 authority issue"); eq(g1["domain_continuity_authority_issue"],128,"GATE001 domain issue"); ts(g1["reevaluated_at"],"GATE001 time")
    eq(g1["state"],"pass_terminal_macro_disposition_complete","GATE001 terminal state"); eq(g1["source_universes_preserved"],True,"GATE001 source preservation"); eq(g1["production_certification_claimed"],False,"GATE001 production claim")
    eq(g1["gate_scope"],"terminal_macro_disposition_not_all_309_rows_production_verified","GATE001 scope")
    h68=g1["holdings_68"]; eq(h68["source_rows"],68,"GATE001 68 rows"); eq(h68["resolved_or_classified"],62,"GATE001 68 resolved"); eq(h68["terminal_unresolved"],6,"GATE001 68 terminal unresolved"); eq(h68["terminal_disposition_coverage"],68,"GATE001 68 coverage"); eq(h68["terminal_disposition_complete"],True,"GATE001 68 complete")
    ex68={v.get("source_row") for v in h68["exceptions"]}; eq(ex68,EXPECTED_68_EXCEPTIONS,"GATE001 68 exception identities"); eq(len(h68["exceptions"]),6,"GATE001 68 exception count")
    for v in h68["exceptions"]:
        if not v.get("disposition") or not v.get("reopen_trigger"): bad("GATE001 68 exception missing disposition/reopen trigger")
    d82=g1["domains_82"]; eq(d82["source_rows"],82,"GATE001 82 rows"); eq(d82["terminal_disposition_coverage"],82,"GATE001 82 coverage"); eq(d82["terminal_disposition_complete"],True,"GATE001 82 complete"); eq(d82["disposition"],"governed_deferred_not_passed_to_phase20","GATE001 82 disposition"); eq(d82["technical_registrar_dns_tls_runtime_certification_complete"],False,"GATE001 82 technical state"); eq(d82["authority_issue"],128,"GATE001 82 authority")
    if not d82.get("reopen_trigger"): bad("GATE001 82 reopen trigger missing")
    e85=g1["engines_85"]; eq(e85["source_rows"],85,"GATE001 85 rows"); eq(e85["terminal_disposition_coverage"],85,"GATE001 85 coverage"); eq(e85["terminal_disposition_complete"],True,"GATE001 85 complete"); eq(e85["technical_provider_account_version_deployment_api_export_certification_complete"],False,"GATE001 85 technical state"); eq(e85["unverified_current_state_is_not_pass"],True,"GATE001 85 unverified semantics")
    if not e85.get("terminal_classes") or not e85.get("reopen_trigger"): bad("GATE001 85 terminal classes/reopen trigger missing")
    p74=g1["platforms_74"]; eq(p74["source_rows"],74,"GATE001 74 rows"); eq(p74["resolved_or_classified"],54,"GATE001 74 resolved"); eq(p74["terminal_unresolved"],20,"GATE001 74 terminal unresolved"); eq(p74["terminal_disposition_coverage"],74,"GATE001 74 coverage"); eq(p74["terminal_disposition_complete"],True,"GATE001 74 complete")
    ex74={v.get("source_row") for v in p74["exceptions"]}; eq(ex74,EXPECTED_74_EXCEPTIONS,"GATE001 74 exception identities"); eq(len(p74["exceptions"]),20,"GATE001 74 exception count")
    for v in p74["exceptions"]:
        if not v.get("disposition"): bad("GATE001 74 exception missing disposition")
    if not p74.get("reopen_trigger"): bad("GATE001 74 reopen trigger missing")
    eq(g1["gate_001_result"],"pass","GATE001 result"); eq(g1["gate_003_proof_debt_unchanged"],True,"GATE001/GATE003 separation")
    eq(set(g1["independent_gates_unaffected"]),{"CT-P299-GATE-002","CT-P299-GATE-003","CT-P299-GATE-007","CT-P299-GATE-008"},"GATE001 independent gates")
    ...
    scan=r["latest_formal_reconciliation_scan"]; eq(scan["scanner_id"],"ct.reconciliation.lmno.agent-e","scanner"); eq(scan["status"],"partial","scan status"); eq(scan["coverage_denominator"],"current_reconcile_required_scopes","scan denominator")
    for k in ("tagged_scopes","reconciled_scopes","drift_scopes","unresolved_scopes","formal_scan_coverage_gap"): ni(scan[k],f"scan.{k}")
    if scan["tagged_scopes"]>t["reconcile_required"] or scan["reconciled_scopes"]>t["reconcile_required"]: bad("formal scan exceeds reconciliation-debt universe")
    eq(scan["formal_scan_coverage_gap"],t["reconcile_required"]-scan["reconciled_scopes"],"scan gap"); eq(scan["formal_scan_coverage_complete"],scan["formal_scan_coverage_gap"]==0,"scan complete"); eq(scan["formal_scan_stale_against_current_tags"],scan["formal_scan_coverage_gap"]>0,"scan stale"); ts(scan["completed_at"],"scan completed")
    ...
    gates=gate_map(d)
    eq(gates["CT-P299-GATE-001"]["state"],"pass","GATE001"); eq(gates["CT-P299-GATE-001"]["terminal_disposition_complete"],True,"GATE001 terminal"); eq(gates["CT-P299-GATE-001"]["technical_full_current_certification_complete"],False,"GATE001 technical")
    eq(gates["CT-P299-GATE-004"]["state"],"pass","GATE004"); eq(gates["CT-P299-GATE-005"]["state"],"pass","GATE005"); eq(gates["CT-P299-GATE-006"]["state"],"deferred_accepted_not_passed","GATE006"); eq(gates["CT-P299-GATE-006"]["blocking"],False,"GATE006 block"); eq(gates["CT-P299-GATE-008"]["state"],"not_met","GATE008"); eq(gates["CT-P299-GATE-008"]["full_documentation_estate_gate"],"nondeferrable_not_met","GATE008 docs")
    if scan["formal_scan_coverage_gap"]>0:
        eq(gates["CT-P299-GATE-003"]["state"],"not_met","GATE003"); eq(gates["CT-P299-GATE-003"]["blocking"],True,"GATE003 block"); eq(gates["CT-P299-GATE-003"]["formal_scan_coverage_gap"],scan["formal_scan_coverage_gap"],"GATE003 gap"); eq(gates["CT-P299-GATE-003"]["coverage_denominator"],"reconcile_required_only","GATE003 denominator")
    eq(len([v for v in gates.values() if v.get("blocking") and v.get("state")!="pass"]),4,"blocking gate count")
    ...
    for trig in ("reconciliation tag total/distribution/reconcile_required drift","GATE-001 terminal-disposition authority or source-universe drift","IP disclosure classification or publication-state change"):
...
def self_test(d):
    ...
    expect_fail(d,lambda x:x["gate_001_terminal_disposition"].__setitem__("production_certification_claimed",True),"GATE001 inflated to production certification")
    expect_fail(d,lambda x:x["gate_001_terminal_disposition"]["holdings_68"].__setitem__("terminal_disposition_coverage",67),"GATE001 incomplete 68 terminal coverage")
    expect_fail(d,lambda x:x["gate_001_terminal_disposition"]["platforms_74"]["exceptions"].pop(),"GATE001 missing 74 exception")
    expect_fail(d,lambda x:x["macro_count_universes"]["holdings_domain_rows"].__setitem__("hard_exit_certified",False),"GATE001 domain accounting regression")
...
def main():
    ...
    if a.self_test:
        ...
        print(f"Phase 2.99 ledger v1.3.5 self-test PASS: GATE001 terminal macro disposition PASS; registry={t['total']}; reconcile-required={t['reconcile_required']}; formal gap={s['formal_scan_coverage_gap']}; GATE006 deferred/NOT-PASS; GATE008 fail-closed."); return 0
    validate(d); t=d["reconciliation"]["reconciliation_tag_snapshot"]; s=d["reconciliation"]["latest_formal_reconciliation_scan"]
    print("Phase 2.99 ledger v1.3.5 consistency PASS"); print(f"GATE001=PASS terminal disposition only; production verification claim=false; current registry={t['total']}; reconcile-required={t['reconcile_required']}; formal-LMNO gap={s['formal_scan_coverage_gap']}"); print(f"Governed deferrals={d['reconciliation']['approved_deferral_count_snapshot']}; deferred routing tags={d['reconciliation']['deferred_routing_tag_count_snapshot']}"); print("Hard exit NOT MET; 4 blocking gates remain; GATE006 deferred/NOT-PASS; Phase3 blocked."); return 0
