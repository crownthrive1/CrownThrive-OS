...
def validate_manifest(data:dict[str,Any])->None:
    ...
    if not FEDERATION.is_file() or not FRAMEWORK_REGISTRY.is_file(): fail("repository federation or framework registry missing")
...
def next_candidate(data:dict[str,Any])->dict[str,Any]:
    ...
    if first.get("physical_child_repository_state")!="linked_governed":
        return {"framework_id":first["framework_id"],"next_safe_packet":"complete_CIE_governed_framework_acceptance_then_provision_backlink_oidc_and_parent_certify_child","activation_allowed":False,"parallel_research_allowed_for_next":True,"parallel_research_framework_id":sequence[1]["framework_id"],"implementation_of_next_allowed":False,"blocking_reason":"CIE child repository is not linked_governed"}
    return {"framework_id":sequence[1]["framework_id"],"next_safe_packet":"build_convergent_ecosystem_source_and_doctrine_reconciliation_packet","activation_allowed":False,"implementation_of_next_allowed":True,"blocking_reason":"candidate must progress through factory lifecycle and governed acceptance"}
...
def plan_for(data:dict[str,Any],framework_id:str)->dict[str,Any]:
    ...
    if item is None: fail("framework is not in authorized sequence")
...
def main()->int:
    ...
    if args.self_test:
        self_test(data); print("Framework Factory self-test PASS: eight sequential framework candidates, current five-voter constitution preserved, framework/subagent transport non-voting, parent-certified child repos and D3 boundary verified."); return 0
    ...
    print("Framework Factory manifest validation PASS"); return 0
