...
def main() -> int:
    ...
    for c in components:
        ...
        assert isinstance(c.get("aliases", []), list), c
    ...
    assert not (PENTAROUTE_PRIMITIVES & set(route.get("aliases", []))), "PentaRoute primitives are distinct identities"
    ...
    assert_migration(provider, "penta_os_vergence_v1", "20260826060819")
    assert_migration(provider, "penta_vergence_factory_repair_bridge_v1", "20260826062029")
    assert_function(provider, "penta_runtime.penta_vergence_claim_v1")
    assert_function(provider, "penta_runtime.penta_vergence_complete_v1")
    assert_cron(provider, "penta-vergence-continuity-4h-v1", "23 */4 * * *")
    assert_cron(provider, "penta-vergence-deep-local-gate-v1", "0 * * * *")
    ...
    print(f"PASS PentaOS registry={len(components)} components axes=5 required={len(REQUIRED)} route_primitives={len(PENTAROUTE_PRIMITIVES)} provider_runtime_custody=PASS applied_vergence=20260826060819")
    return 0
