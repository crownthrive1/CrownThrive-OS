...
for x in ['direct_main_write','force_push','self_merge','self_certification','sovereign_vote','d3_action','trade_secret_publication']:
...
if rec['properties']['certification_effect']['const'] is not False or rec['properties']['sovereign_vote_created']['const'] is not False: errors.append('receipt_authority')
sql=(ROOT/'supabase/migrations/20260823203000_execution_builder_agent_v1.sql').read_text()
low=sql.lower()
for token in ['ct.agent.execution-builder','agent_build_requests','agent_build_receipts','route_construction_work_to_execution_builder','route_capability_execution_to_execution_builder','built_pending_independent_verification']:
...
if 'vote_eligible,false' not in low.replace(' ','') and 'false,false,true' not in low.replace(' ',''): pass
...
if errors:
    ...
    raise SystemExit(1)
print(json.dumps({'status':'PASS','agent':'ct.agent.execution-builder','parent':'ct.relay.agent-c','authority':'D2','non_voting':True,'receipt_state':'BUILT_PENDING_INDEPENDENT_VERIFICATION'},sort_keys=True))
