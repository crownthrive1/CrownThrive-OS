...
def main() -> int:
    ...
    if auth.get("physical_child_repository_required") is not False:
        fail("physical child repository must not be a framework-package requirement")
    ...
    projection = repos.get("ct.repo.cie")
    if not parent or parent.get("role") != "canonical_parent" or not projection:
        fail("transport repository inventory drift")
    if projection.get("role") != "optional_framework_distribution_projection" or projection.get("physical_repository_required") is not False or projection.get("operationally_enabled") is not False or projection.get("can_vote") is not False:
        fail("optional CIE repository projection semantics drift")
    ...
    if policy.get("child_definition") != "independently_executable_framework_package" or policy.get("physical_repository_required") is not False or policy.get("canonical_monorepo_host_allowed") is not True:
        fail("framework package policy drift")
    for key in (
    ...
    if row.get("physical_repository_required") is not False:
        fail("CIE algorithm cannot require standalone repository")
    ...
    if fleet.get("child_definition") != "independently_executable_framework_package_not_physical_repository":
        fail("fleet/federation package semantics mismatch")

    print("Repository federation validation PASS: A/B/C/D/S preserved; Agent D mandatory; framework child is executable package; standalone repo optional; PR OIDC isolated; sync_agents non-voting; CIE package controlled-test/non-operational/non-voting; protected calibration private.")
    return 0
