...
def main() -> int:
    ...
    for gate in gates:
        ...
        filename = path.name
        if re.search(r"(?:^|[-_])v2(?:[-_.]|$)", filename) or "-002" in filename:
            legacy_v2 += 1
            if source_generation != "legacy_v2_name":
                fail(f"V2-named compatibility workflow is not marked legacy_v2_name: {workflow}")
        if "-003" in filename:
            legacy_v3 += 1
