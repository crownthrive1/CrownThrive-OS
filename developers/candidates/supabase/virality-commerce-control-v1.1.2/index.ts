...
async function readDispatchRegistry(
  db: ReturnType<typeof postgres>,
): Promise<DispatchRegistryEvidence> {
  try {
    const rows = await db`
...
async function manifest() {
  ...
  const [
    ...
    `,
    db`
      select project_key is not null as project_key_configured,
             environment, primary_origin, allowlist_state,
  ...
  const dispatchControl = classifyDispatchRegistry(dispatchEvidence);
  ...
  return {
    ...
    version: "1.1.2-staged-candidate",
    environment: "production_candidate",
    deployment_state: "STAGED_REPOSITORY_ONLY_NOT_DEPLOYED",
    live_edge_function_version: 4,
    live_api_version: "1.1.1",
    live_state: "HOLD_UNCHANGED",
    ...
    authority: {
      ...
      generalized_dispatch: {
        ...dispatchControl,
        exact_mutation_contract_state: "HOLD",
        registry: dispatchEvidence.row,
      },
      d3: "HUMAN_RESERVED",
    ...
    raw_secret_export: MUTATION_AUTHORITY.raw_secret_export,
  };
}

Deno.serve(createHandler(manifest));
