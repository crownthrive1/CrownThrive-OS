...
stable identity + version + evidence
            ↓
CIE cultural/context constraints where applicable
            ↓
CHLOM rights / rules / roles / authority gates
            ↓
ThriveBase durable operational state
            ↓
factory / workflow / agent execution
            ↓
provider adapter certification
            ↓
write / deploy / publish only within authority
            ↓
...
Phase 3 does **not** mean:

- every platform is production-certified;
- every historical Help Center article has a current substantive successor;
...
| State | Meaning |
| --- | --- |
| `PRODUCTION` / `ACTIVE` | Currently operational within the described scope and supported by applicable evidence. |
| `WRITE_VERIFIED` | A bounded provider mutation path has been tested, read back, and certified for the stated scope. |
| `CONTROLLED_TEST` | Executable or materially implemented, but intentionally limited and not represented as general production authority. |
| `HOLD` | A required gate or evidence condition is unresolved; fail closed. |
| `BUILT_UNDEPLOYED` | Artifacts/software exist, but deployment or readback has not been established. |
| `CANDIDATE` | Proposed/generated material awaiting required validation/authority. |
| `HISTORICAL` | Preserved evidence of a prior state; not current operating instruction. |
| `SUPERSEDED` | Replaced prospectively by a newer canonical record while retained for lineage. |
| `RESERVE` / `DORMANT` | Preserved asset or capability not currently activated. |
...
- **CrownThrive IO / API / MCP surfaces** — typed integration interfaces whose runtime/write authority is independently certified.
