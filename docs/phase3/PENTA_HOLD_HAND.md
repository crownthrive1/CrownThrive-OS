...
`HOLD → PentaHoldHand → PentaCrawler → PentaHelp → named Penta remediation routes → independent recheck → additive resolution receipt`
...
Disable the `penta-hold-hand-crawler-v1` schedule and revoke execution on the new functions. Preserve all hand, observation, remediation, and resolution rows as evidence. The original campaign HOLD remains unchanged throughout rollback.
