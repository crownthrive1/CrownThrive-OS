...
function send(response, status, payload) {
  ...
  if (payload?.evidence?.digest) {
    response.setHeader('X-PentaFabric-Receipt', payload.evidence.digest);
  }
  response.setHeader('X-PentaFabric-State', payload?.status || 'UNKNOWN');
...
export default async function handler(request, response) {
  ...
  try {
    ...
    return send(response, fabric.status === 'OPERATIONAL' ? 200 : 503, fabric);
  } catch (error) {
