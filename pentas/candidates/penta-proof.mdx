---
title: "PentaProof"
sidebarTitle: "PentaProof"
description: "Penta namespace reference and canonicalization guide for PentaProof."
...
<Info>
  ...
  **This page:** PentaProof — Penta namespace reference and canonicalization guide for PentaProof.
...
> **Namespace reference / canonicalization pending — not a production claim.** Creating this page does not create a new canonical Penta, maturity, authority, credentials, provider permission, or execution eligibility. A governed extension record currently reports maturity `implemented`, but it is outside the frozen Penta OS V1.5 canonical registry.
...
| Candidate class | `governed_extension` |
| Source state | `governed_system_extension` |
| Canonical target | No canonical target is established. |
| Extension maturity | `implemented` |
| Provisional family | [Penta Build, Certification & Release Family](/pentas/families/build-release) |
...
| --- | --- |
| Architectural layers | [Control & Governance](/pentas/layers/control-governance), [Data, Knowledge & Semantics](/pentas/layers/data-knowledge), [Build, Test & Release](/pentas/layers/build-release), [Routing, Interop & Transport](/pentas/layers/routing-transport) |
| Jobs / functions | [Govern](/pentas/jobs/govern), [Document](/pentas/jobs/document), [Build](/pentas/jobs/build), [Test & Certify](/pentas/jobs/test-certify), [Publish](/pentas/jobs/publish), [Route](/pentas/jobs/route) |
...
| Classification provenance | `candidate_canonicalization_plus_registry_family_plus_keyword` / `candidate_canonicalization_plus_registry_family_plus_keyword` |
...
Route work to PentaProof when the request matches its recorded role (Automatically collect, validate, hash, bind and project CI/runtime certification evidence into a fail-closed proof envelope for PentaCertify, PentaAssure, DAIL and PentaRelease.) and one of its job classifications: govern, document, build, test-certify, publish, route.
...
2. Match the request to the documented jobs: `govern`, `document`, `build`, `test-certify`, `publish`, `route`.
...
- `data/penta/systems.pentaproof.v1.json`

## Agent ingestion contract

Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `PentaProof` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `4b87f610652a3b13733b97a541dd659a2f208b05e4b46e711d8ad11101e46733`
