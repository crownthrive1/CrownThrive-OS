...
<Info>
  ...
  **This page:** PentaCompliance — Registry-driven PentaDocs portal guide for PentaCompliance.
...
| --- | --- |
| Architectural layers | [Security & Trust](/pentas/layers/security-trust), [Control & Governance](/pentas/layers/control-governance), [Build, Test & Release](/pentas/layers/build-release), [Media, Creative & Publishing](/pentas/layers/media-creative) |
...
Route work to PentaCompliance when the request matches its recorded role (Maintain obligation inventories, applicability matrices, control mappings, evidence requirements, compliance calendars, exceptions, attestations and remediation routing across CrownThrive programs and providers.) and one of its job classifications: secure, govern, observe, build, test-certify, publish.
...
5. Verify declared dependencies before execution: penta.assure, penta.audit, penta.legal, penta.ofac, penta.policy, penta.risk.
...
1. Start from this exact identity and machine key: `penta.compliance`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.compliance` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.compliance` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `264f6e69dd34afaa58541f075a02d1e24e410390903d76324c45b59aedc76ff8`
