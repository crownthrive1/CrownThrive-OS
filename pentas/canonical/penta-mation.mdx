---
title: "PentaMation"
sidebarTitle: "PentaMation"
description: "Registry-driven PentaDocs portal guide for PentaMation."
...
<Info>
  ...
  **This page:** PentaMation — Registry-driven PentaDocs portal guide for PentaMation.
...
## Overview
**Mission / role:** Governed event-driven and scheduled workflow orchestration across jobs, queues, dependencies, retries, compensation and convergence.
...
| Operator route | `/io/pentas/mation` |
| Public status route | `/penta/mation` |
...
## Dependencies
`penta.assure`, `penta.docs`, `penta.hybrid`, `penta.route`, `penta.time`
...
| --- | --- |
| Architectural layers | [Automation & Agentic](/pentas/layers/automation-agentic), [Routing, Interop & Transport](/pentas/layers/routing-transport), [Build, Test & Release](/pentas/layers/build-release), [Observability & Reliability](/pentas/layers/observability-reliability) |
| Jobs / functions | [Orchestrate](/pentas/jobs/orchestrate), [Build](/pentas/jobs/build), [Support & Operate](/pentas/jobs/support-operate), [Route](/pentas/jobs/route), [Retrieve](/pentas/jobs/retrieve), [Transform](/pentas/jobs/transform) |
...
Route work to PentaMation when the request matches its recorded role (Governed event-driven and scheduled workflow orchestration across jobs, queues, dependencies, retries, compensation and convergence.) and one of its job classifications: orchestrate, build, support-operate, route, retrieve, transform.
...
5. Verify declared dependencies before execution: penta.assure, penta.docs, penta.hybrid, penta.route, penta.time.
...
1. Start from this exact identity and machine key: `penta.mation`.
2. Match the request to the documented jobs: `orchestrate`, `build`, `support-operate`, `route`, `retrieve`, `transform`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.mation` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.mation` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `b16764b197ef5bd5af5d38e9f5570575922f320d1151764128382804cea574a8`
