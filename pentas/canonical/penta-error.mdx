---
title: "PentaError"
sidebarTitle: "PentaError"
description: "Registry-driven PentaDocs portal guide for PentaError."
...
<Info>
  ...
  **This page:** PentaError — Registry-driven PentaDocs portal guide for PentaError.
...
## Overview
**Mission / role:** Canonical typed error taxonomy, safe error envelopes, retryability classification, normalization, and dead-letter evidence across PENTA runtimes.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.error` |
| Kind | `system` |
| Maturity | `production` |
| Risk ceiling | `D2` |
...
| Operator route | `/io/pentas/error` |
| Public status route | `/penta/error` |
...
| --- | --- |
| Architectural layers | [Observability & Reliability](/pentas/layers/observability-reliability), [Routing, Interop & Transport](/pentas/layers/routing-transport) |
| Jobs / functions | [Observe](/pentas/jobs/observe), [Recover](/pentas/jobs/recover), [Support & Operate](/pentas/jobs/support-operate), [Route](/pentas/jobs/route), [Retrieve](/pentas/jobs/retrieve), [Transform](/pentas/jobs/transform) |
...
Route work to PentaError when the request matches its recorded role (Canonical typed error taxonomy, safe error envelopes, retryability classification, normalization, and dead-letter evidence across PENTA runtimes.) and one of its job classifications: observe, recover, support-operate, route, retrieve, transform.
...
5. Verify declared dependencies before execution: penta.docs, penta.trace.
...
1. Start from this exact identity and machine key: `penta.error`.
2. Match the request to the documented jobs: `observe`, `recover`, `support-operate`, `route`, `retrieve`, `transform`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.error` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.error` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `7292e1b0f1559734516401d1782b8b35a4d62c4664e91fd2da1cde527be7a848`
