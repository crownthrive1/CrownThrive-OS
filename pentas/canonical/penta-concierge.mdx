---
title: "PentaConcierge"
sidebarTitle: "PentaConcierge"
description: "Registry-driven PentaDocs portal guide for PentaConcierge."
...
<Info>
  ...
  **This page:** PentaConcierge — Registry-driven PentaDocs portal guide for PentaConcierge.
...
## Overview
**Mission / role:** Coordinate bounded concierge requests, research, bookings, calls, follow-ups, service fulfillment and customer/operator handoffs.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.concierge` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D2` |
| Execution eligible by registry | `false` |
| Strict readiness | `HOLD_MEMBER_MATURITY` |
| Family | [Penta Communications & Service Family](/pentas/families/communications-service) |
| Family assignment | `registry_backed` |
| Operator route | `/io/pentas/concierge` |
| Public status route | `/penta/concierge` |
...
Primary family context: [Penta Communications & Service Family](/pentas/families/communications-service). Sibling, upstream, downstream, and cross-family relationships resolve through the family topology and dependency graph rather than prose inference.

## Dependencies
`penta.credentials`, `penta.mail`, `penta.route`, `penta.status`
...
| --- | --- |
| Architectural layers | [Communications, Growth & Service](/pentas/layers/communications-growth), [Experience & Interface](/pentas/layers/experience-interface), [Routing, Interop & Transport](/pentas/layers/routing-transport), [Build, Test & Release](/pentas/layers/build-release), [Observability & Reliability](/pentas/layers/observability-reliability) |
| Jobs / functions | [Communicate & Grow](/pentas/jobs/communicate-grow), [Support & Operate](/pentas/jobs/support-operate), [Analyze & Research](/pentas/jobs/analyze-research), [Route](/pentas/jobs/route), [Retrieve](/pentas/jobs/retrieve), [Transform](/pentas/jobs/transform) |
...
Route work to PentaConcierge when the request matches its recorded role (Coordinate bounded concierge requests, research, bookings, calls, follow-ups, service fulfillment and customer/operator handoffs.) and one of its job classifications: communicate-grow, support-operate, analyze-research, route, retrieve, transform.
...
5. Verify declared dependencies before execution: penta.credentials, penta.mail, penta.route, penta.status.
...
1. Start from this exact identity and machine key: `penta.concierge`.
2. Match the request to the documented jobs: `communicate-grow`, `support-operate`, `analyze-research`, `route`, `retrieve`, `transform`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.concierge` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.concierge` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `41aa4b2967470c8290fa0f92bf6a5c81b46677d2babe58c282b83a3887f87441`
