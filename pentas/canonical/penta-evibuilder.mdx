---
title: "PentaEVIBuilder"
sidebarTitle: "PentaEVIBuilder"
description: "Registry-driven PentaDocs portal guide for PentaEVIBuilder."
...
<Info>
  ...
  **This page:** PentaEVIBuilder — Registry-driven PentaDocs portal guide for PentaEVIBuilder.
...
## Overview
**Mission / role:** Construct deterministic exact-head evidence bundles, provenance receipts, test bindings, rollback/fallback evidence and advisory repair memory for governed CrownThrive software operations.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.evi-builder` |
| Kind | `system` |
| Maturity | `production` |
| Risk ceiling | `D2` |
...
| Operator route | `/io/pentas/evi-builder` |
| Public status route | `/penta/evi-builder` |
...
## Dependencies
`penta.assure`, `penta.certify`, `penta.docs`, `penta.factory`, `penta.trace`
...
| --- | --- |
| Architectural layers | [Security & Trust](/pentas/layers/security-trust), [Control & Governance](/pentas/layers/control-governance), [Data, Knowledge & Semantics](/pentas/layers/data-knowledge), [Routing, Interop & Transport](/pentas/layers/routing-transport), [Build, Test & Release](/pentas/layers/build-release) |
| Jobs / functions | [Secure](/pentas/jobs/secure), [Govern](/pentas/jobs/govern), [Observe](/pentas/jobs/observe), [Document](/pentas/jobs/document), [Retrieve](/pentas/jobs/retrieve), [Route](/pentas/jobs/route) |
...
Route work to PentaEVIBuilder when the request matches its recorded role (Construct deterministic exact-head evidence bundles, provenance receipts, test bindings, rollback/fallback evidence and advisory repair memory for governed CrownThrive software operations.) and one of its job classifications: secure, govern, observe, document, retrieve, route.
...
5. Verify declared dependencies before execution: penta.assure, penta.certify, penta.docs, penta.factory, penta.trace.
...
1. Start from this exact identity and machine key: `penta.evi-builder`.
2. Match the request to the documented jobs: `secure`, `govern`, `observe`, `document`, `retrieve`, `route`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.evi-builder` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.evi-builder` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `9ff45605049629551c02c452c23f6d2064ddb6598ae5317bbf15264b5d425006`
