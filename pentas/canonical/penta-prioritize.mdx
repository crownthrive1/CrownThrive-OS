---
title: "PentaPrioritize"
sidebarTitle: "PentaPrioritize"
description: "Registry-driven PentaDocs portal guide for PentaPrioritize."
...
<Info>
  ...
  **This page:** PentaPrioritize — Registry-driven PentaDocs portal guide for PentaPrioritize.
...
| Operator route | `/io/pentas/prioritize` |
| Public status route | `/penta/prioritize` |
...
| Jobs / functions | [Route](/pentas/jobs/route), [Retrieve](/pentas/jobs/retrieve), [Transform](/pentas/jobs/transform) |
...
Route work to PentaPrioritize when the request matches its recorded role (Unresolved PentaSELF delegation reference pending owner mapping or a complete contract.) and one of its job classifications: route, retrieve, transform.
...
1. Start from this exact identity and machine key: `penta.prioritize`.
2. Match the request to the documented jobs: `route`, `retrieve`, `transform`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.prioritize` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.prioritize` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `fd4fd737770bd48a0f53c7326dbf75b98d734861ec15ff0fe3f4ed5865c712eb`
