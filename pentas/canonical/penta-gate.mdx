---
title: "PentaGate"
sidebarTitle: "PentaGate"
description: "Registry-driven PentaDocs portal guide for PentaGate."
...
<Info>
  ...
  **This page:** PentaGate — Registry-driven PentaDocs portal guide for PentaGate.
...
## Overview
**Mission / role:** Perform a read-only fail-closed preflight over repository-local Python syntax, test-discovery dependency availability and CI runner contract assumptions before expensive certification or terminal PR authority executes.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.gate` |
| Kind | `system` |
...
| Operator route | `/io/pentas/gate` |
| Public status route | `/penta/gate` |
...
## Dependencies
`penta.assure`, `penta.certify`, `penta.immune`, `penta.pr`
...
| --- | --- |
| Architectural layers | [Build, Test & Release](/pentas/layers/build-release), [Security & Trust](/pentas/layers/security-trust), [Routing, Interop & Transport](/pentas/layers/routing-transport), [Experience & Interface](/pentas/layers/experience-interface) |
| Jobs / functions | [Build](/pentas/jobs/build), [Test & Certify](/pentas/jobs/test-certify), [Publish](/pentas/jobs/publish), [Secure](/pentas/jobs/secure), [Govern](/pentas/jobs/govern), [Route](/pentas/jobs/route) |
...
Route work to PentaGate when the request matches its recorded role (Perform a read-only fail-closed preflight over repository-local Python syntax, test-discovery dependency availability and CI runner contract assumptions before expensive certification or terminal PR authority executes.) and one of its job classifications: build, test-certify, publish, secure, govern, route.
...
5. Verify declared dependencies before execution: penta.assure, penta.certify, penta.immune, penta.pr.
...
1. Start from this exact identity and machine key: `penta.gate`.
2. Match the request to the documented jobs: `build`, `test-certify`, `publish`, `secure`, `govern`, `route`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.gate` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.gate` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `6c54fb998d18bfb0163150e921b5d85a11342dc6757ab4073f91781543a4f168`
