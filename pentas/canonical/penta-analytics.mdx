---
title: "PentaAnalytics"
sidebarTitle: "PentaAnalytics"
description: "Registry-driven PentaDocs portal guide for PentaAnalytics."
...
<Info>
  ...
  **This page:** PentaAnalytics — Registry-driven PentaDocs portal guide for PentaAnalytics.
...
## Overview
**Mission / role:** Cross-ecosystem metric semantics, data lineage, evidence aggregation, comparative analysis, forecasting and decision-grade analytical products.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.analytics` |
| Kind | `system` |
...
| Operator route | `/io/pentas/analytics` |
| Public status route | `/penta/analytics` |
...
| --- | --- |
| Architectural layers | [Data, Knowledge & Semantics](/pentas/layers/data-knowledge), [Observability & Reliability](/pentas/layers/observability-reliability), [Build, Test & Release](/pentas/layers/build-release) |
| Jobs / functions | [Analyze & Research](/pentas/jobs/analyze-research), [Retrieve](/pentas/jobs/retrieve), [Observe](/pentas/jobs/observe), [Build](/pentas/jobs/build), [Test & Certify](/pentas/jobs/test-certify), [Publish](/pentas/jobs/publish) |
...
Route work to PentaAnalytics when the request matches its recorded role (Cross-ecosystem metric semantics, data lineage, evidence aggregation, comparative analysis, forecasting and decision-grade analytical products.) and one of its job classifications: analyze-research, retrieve, observe, build, test-certify, publish.
...
1. Start from this exact identity and machine key: `penta.analytics`.
2. Match the request to the documented jobs: `analyze-research`, `retrieve`, `observe`, `build`, `test-certify`, `publish`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.analytics` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.analytics` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `b478011d57162ec5e467ce59880317d9bbd29ac206c615fbe0079cb83eb65e73`
