---
title: "PentaData"
sidebarTitle: "PentaData"
description: "Registry-driven PentaDocs portal guide for PentaData."
...
<Info>
  ...
  **This page:** PentaData — Registry-driven PentaDocs portal guide for PentaData.
...
## Overview
**Mission / role:** Govern institutional data classification, ownership, stewardship, lineage, quality contracts, schemas, data products, sharing boundaries and lifecycle rules.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.data` |
| Kind | `system` |
...
| Operator route | `/io/pentas/data` |
| Public status route | `/penta/data` |
...
## Dependencies
`penta.analytics`, `penta.docs`, `penta.privacy`, `penta.records`, `penta.security`
...
| --- | --- |
| Architectural layers | [Data, Knowledge & Semantics](/pentas/layers/data-knowledge) |
| Jobs / functions | [Document](/pentas/jobs/document), [Retrieve](/pentas/jobs/retrieve), [Transform](/pentas/jobs/transform) |
...
Route work to PentaData when the request matches its recorded role (Govern institutional data classification, ownership, stewardship, lineage, quality contracts, schemas, data products, sharing boundaries and lifecycle rules.) and one of its job classifications: document, retrieve, transform.
...
5. Verify declared dependencies before execution: penta.analytics, penta.docs, penta.privacy, penta.records, penta.security.
...
1. Start from this exact identity and machine key: `penta.data`.
2. Match the request to the documented jobs: `document`, `retrieve`, `transform`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.data` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.data` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `ce8b85d176abcd161f043149e21650f79b31a5811fbbe7d4a2ec5fc8f9339bfd`
