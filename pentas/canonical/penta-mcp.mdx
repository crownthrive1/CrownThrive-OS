---
title: "PentaMCP"
sidebarTitle: "PentaMCP"
description: "Registry-driven PentaDocs portal guide for PentaMCP."
...
<Info>
  ...
  **This page:** PentaMCP — Registry-driven PentaDocs portal guide for PentaMCP.
...
## Overview
**Mission / role:** Machine/tool interoperability and capability invocation across registered PENTA systems.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.mcp` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D3` |
...
| Operator route | `/io/pentas/mcp` |
| Public status route | `/penta/mcp` |
...
| --- | --- |
| Architectural layers | [Routing, Interop & Transport](/pentas/layers/routing-transport), [Foundation & Kernel](/pentas/layers/foundation-kernel), [Observability & Reliability](/pentas/layers/observability-reliability) |
| Jobs / functions | [Route](/pentas/jobs/route), [Orchestrate](/pentas/jobs/orchestrate), [Support & Operate](/pentas/jobs/support-operate), [Observe](/pentas/jobs/observe), [Recover](/pentas/jobs/recover) |
...
Route work to PentaMCP when the request matches its recorded role (Machine/tool interoperability and capability invocation across registered PENTA systems.) and one of its job classifications: route, orchestrate, support-operate, observe, recover.
...
5. Verify declared dependencies before execution: penta.docs, penta.route.
...
1. Start from this exact identity and machine key: `penta.mcp`.
2. Match the request to the documented jobs: `route`, `orchestrate`, `support-operate`, `observe`, `recover`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.mcp` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.mcp` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `bd92551c9d8dbb0879932f019be8b9abb6b14aadfa458cae909a10c72dbb229d`
