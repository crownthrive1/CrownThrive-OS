---
title: "PentaStatus"
sidebarTitle: "PentaStatus"
description: "Registry-driven PentaDocs portal guide for PentaStatus."
...
<Info>
  ...
  **This page:** PentaStatus — Registry-driven PentaDocs portal guide for PentaStatus.
...
## Overview
**Mission / role:** Produce owner-facing self-status reports for PENTA systems, jobs, providers, incidents, health, maturity, holds and evidence gaps.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.status` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D1` |
...
| Operator route | `/io/pentas/status` |
| Public status route | `/penta/status` |
...
## Dependencies
`penta.audit`, `penta.docs`, `penta.signal`
...
Route work to PentaStatus when the request matches its recorded role (Produce owner-facing self-status reports for PENTA systems, jobs, providers, incidents, health, maturity, holds and evidence gaps.) and one of its job classifications: recover, observe, support-operate, build, test-certify, publish.
...
5. Verify declared dependencies before execution: penta.audit, penta.docs, penta.signal.
...
1. Start from this exact identity and machine key: `penta.status`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.status` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.status` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `8de3fe60b13501bdae46567b2a0d980b4efe9bee32bd943e2e2f33487bdc2857`
