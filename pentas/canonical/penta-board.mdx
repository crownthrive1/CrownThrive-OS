---
title: "PentaBoard"
sidebarTitle: "PentaBoard"
description: "Registry-driven PentaDocs portal guide for PentaBoard."
...
<Info>
  ...
  **This page:** PentaBoard — Registry-driven PentaDocs portal guide for PentaBoard.
...
## Overview
**Mission / role:** Represent governed board/directive intake, resolutions, delegated mandates, accountability and routing to subordinate PENTA governance lanes.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.board` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D3` |
...
| Operator route | `/io/pentas/board` |
| Public status route | `/penta/board` |
...
## Dependencies
`penta.audit`, `penta.directors`, `penta.legal`, `penta.policy`
...
| --- | --- |
| Architectural layers | [Control & Governance](/pentas/layers/control-governance), [Security & Trust](/pentas/layers/security-trust) |
| Jobs / functions | [Govern](/pentas/jobs/govern), [Secure](/pentas/jobs/secure), [Document](/pentas/jobs/document) |
...
Route work to PentaBoard when the request matches its recorded role (Represent governed board/directive intake, resolutions, delegated mandates, accountability and routing to subordinate PENTA governance lanes.) and one of its job classifications: govern, secure, document.
...
5. Verify declared dependencies before execution: penta.audit, penta.directors, penta.legal, penta.policy.
...
1. Start from this exact identity and machine key: `penta.board`.
2. Match the request to the documented jobs: `govern`, `secure`, `document`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.board` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.board` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `06413bfe539f50bf3c2dff4df120937dccba7f216dfdf3dd77af4dd2abeec734`
