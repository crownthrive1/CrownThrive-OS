---
title: "PentaHeal"
sidebarTitle: "PentaHeal"
description: "Registry-driven PentaDocs portal guide for PentaHeal."
...
<Info>
  ...
  **This page:** PentaHeal — Registry-driven PentaDocs portal guide for PentaHeal.
...
## Overview
**Mission / role:** Convert deterministic PentaGate findings into bounded PentaImmune repair candidates with rollback, fallback, exact-head testing and independent certification requirements, then route execution to PentaBuild/PentaFactory and delivery to PentaPR.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.heal` |
| Kind | `system` |
...
| Operator route | `/io/pentas/heal` |
| Public status route | `/penta/heal` |
...
## Dependencies
`penta.assure`, `penta.certify`, `penta.factory`, `penta.immune`, `penta.pr`
...
| --- | --- |
| Architectural layers | [Observability & Reliability](/pentas/layers/observability-reliability), [Build, Test & Release](/pentas/layers/build-release), [Routing, Interop & Transport](/pentas/layers/routing-transport) |
| Jobs / functions | [Recover](/pentas/jobs/recover), [Observe](/pentas/jobs/observe), [Support & Operate](/pentas/jobs/support-operate), [Route](/pentas/jobs/route), [Retrieve](/pentas/jobs/retrieve), [Transform](/pentas/jobs/transform) |
...
Route work to PentaHeal when the request matches its recorded role (Convert deterministic PentaGate findings into bounded PentaImmune repair candidates with rollback, fallback, exact-head testing and independent certification requirements, then route execution to PentaBuild/PentaFactory and delivery to PentaPR.) and one of its job classifications: recover, observe, support-operate, route, retrieve, transform.
...
5. Verify declared dependencies before execution: penta.assure, penta.certify, penta.factory, penta.immune, penta.pr.
...
1. Start from this exact identity and machine key: `penta.heal`.
2. Match the request to the documented jobs: `recover`, `observe`, `support-operate`, `route`, `retrieve`, `transform`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.heal` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.heal` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `a5b73abb7d56e9db0a093fabf24bdc247c4e4b7ee7c54acb116f0e723588fee1`
