---
title: "PentaList"
sidebarTitle: "PentaList"
description: "Registry-driven PentaDocs portal guide for PentaList."
...
<Info>
  ...
  **This page:** PentaList — Registry-driven PentaDocs portal guide for PentaList.
...
| Operator route | `/io/pentas/primitives#list` |
| Public status route | `/penta/list` |
...
Route work to PentaList when the request matches its recorded role (Governed capability primitive under PentaRoute and the universal Penta OS contract.) and one of its job classifications: retrieve, transform, route.
...
1. Start from this exact identity and machine key: `penta.list`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.list` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.list` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `b4b8a14c29ee64f4ea965d08d9df15edc7ab9a94493d008a42ee0f9cb389ae80`
