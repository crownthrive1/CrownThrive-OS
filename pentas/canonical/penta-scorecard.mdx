...
Route work to PentaScorecard when the request matches its recorded role (Unresolved PentaSELF delegation reference pending owner mapping or a complete contract.) and one of its job classifications: analyze-research, retrieve, observe, route, transform.
...
1. Start from this exact identity and machine key: `penta.scorecard`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.scorecard` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.scorecard` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `852cf301dca4646d2f890396a01576041b945c3395e8eae782c94fd9ab1f102e`
