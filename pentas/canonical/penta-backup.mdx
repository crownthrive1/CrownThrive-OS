---
title: "PentaBackup"
sidebarTitle: "PentaBackup"
description: "Registry-driven PentaDocs portal guide for PentaBackup."
...
<Info>
  ...
  **This page:** PentaBackup — Registry-driven PentaDocs portal guide for PentaBackup.
...
## Overview
**Mission / role:** Control-plane snapshot capture and governed external backup intent.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.backup` |
| Kind | `system` |
...
| Operator route | `/io/pentas/backup` |
| Public status route | `/penta/backup` |
...
Route work to PentaBackup when the request matches its recorded role (Control-plane snapshot capture and governed external backup intent.) and one of its job classifications: recover, observe, support-operate.
...
1. Start from this exact identity and machine key: `penta.backup`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.backup` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.backup` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `c5fde0efb10c64acf2ba7eff381e43a54447aa503447c0d2b51942ba4d9f3df2`
