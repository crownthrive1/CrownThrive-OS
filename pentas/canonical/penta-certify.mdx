...
<Info>
  ...
  **This page:** PentaCertify — Registry-driven PentaDocs portal guide for PentaCertify.
...
| --- | --- |
| Architectural layers | [Build, Test & Release](/pentas/layers/build-release) |
| Jobs / functions | [Build](/pentas/jobs/build), [Test & Certify](/pentas/jobs/test-certify), [Publish](/pentas/jobs/publish) |
...
Route work to PentaCertify when the request matches its recorded role (Certify provider adapters, plugins, integrations and execution paths using governed tests and evidence before production eligibility.) and one of its job classifications: build, test-certify, publish.
...
5. Verify declared dependencies before execution: penta.assure, penta.credentials, penta.docs.
...
1. Start from this exact identity and machine key: `penta.certify`.
2. Match the request to the documented jobs: `build`, `test-certify`, `publish`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.certify` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.certify` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `61744a6f03c04f7fc686e80398e0ac6a023920b48f9ad7567a2530f5b7d04c98`
