---
title: "PentaLogger"
sidebarTitle: "PentaLogger"
description: "Registry-driven PentaDocs portal guide for PentaLogger."
...
<Info>
  ...
  **This page:** PentaLogger — Registry-driven PentaDocs portal guide for PentaLogger.
...
## Overview
**Mission / role:** Structured JSON-line logging with severity controls, trace correlation, contextual child loggers, mandatory secret redaction, and normalized exception evidence.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.logger` |
| Kind | `system` |
| Maturity | `production` |
| Risk ceiling | `D1` |
...
| Operator route | `/io/pentas/logger` |
| Public status route | `/penta/logger` |
...
## Dependencies
`penta.docs`, `penta.error`, `penta.trace`
...
| --- | --- |
| Architectural layers | [Observability & Reliability](/pentas/layers/observability-reliability), [Data, Knowledge & Semantics](/pentas/layers/data-knowledge) |
| Jobs / functions | [Observe](/pentas/jobs/observe), [Recover](/pentas/jobs/recover), [Support & Operate](/pentas/jobs/support-operate), [Document](/pentas/jobs/document), [Retrieve](/pentas/jobs/retrieve) |
...
Route work to PentaLogger when the request matches its recorded role (Structured JSON-line logging with severity controls, trace correlation, contextual child loggers, mandatory secret redaction, and normalized exception evidence.) and one of its job classifications: observe, recover, support-operate, document, retrieve.
...
5. Verify declared dependencies before execution: penta.docs, penta.error, penta.trace.
...
1. Start from this exact identity and machine key: `penta.logger`.
2. Match the request to the documented jobs: `observe`, `recover`, `support-operate`, `document`, `retrieve`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.logger` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.logger` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `131eb55e51168e6f925d447facc95bb920ff9297f2ea246c77c814e15e8403fa`
