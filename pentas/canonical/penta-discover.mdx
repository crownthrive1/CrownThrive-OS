---
title: "PentaDiscover"
sidebarTitle: "PentaDiscover"
description: "Registry-driven PentaDocs portal guide for PentaDiscover."
...
<Info>
  ...
  **This page:** PentaDiscover — Registry-driven PentaDocs portal guide for PentaDiscover.
...
<Warning>
  **Identity distinction:** `PentaDiscover` (`penta.discover`) is the governed PentaRoute capability primitive documented on this page. It is **not** the same record as the systemwide `PentaDiscovery` system (`penta.discovery`), which currently has a separate production runtime/certification lineage. See [PentaDiscovery System](/pentas/operational/penta-discovery-system). Do not alias, merge or transfer authority between the two identities without PentaScribe/PentaCensus evidence.
</Warning>
...
| Operator route | `/io/pentas/primitives#discover` |
| Public status route | `/penta/discover` |
...
| --- | --- |
| Architectural layers | [Routing, Interop & Transport](/pentas/layers/routing-transport) |
| Jobs / functions | [Retrieve](/pentas/jobs/retrieve), [Transform](/pentas/jobs/transform), [Route](/pentas/jobs/route) |
...
Route work to PentaDiscover when the request matches its recorded role (Governed PentaRoute capability primitive.) and one of its job classifications: retrieve, transform, route.
...
1. Start from this exact identity and machine key: `penta.discover`.
2. Match the request to the documented jobs: `retrieve`, `transform`, `route`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.discover` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.discover` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
2. Confirm namespace\_state=canonical before considering execution; candidates stay fail-closed.
...
- This record hash: `7594cdb5d727185bbd14fd32c8f19827c5d6eea7112dbeb4a0cb284ad0c242c2`
