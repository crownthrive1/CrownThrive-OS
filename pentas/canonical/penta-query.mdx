---
title: "PentaQuery"
sidebarTitle: "PentaQuery"
description: "Registry-driven PentaDocs portal guide for PentaQuery."
...
<Info>
  ...
  **This page:** PentaQuery — Registry-driven PentaDocs portal guide for PentaQuery.
...
| Operator route | `/io/pentas/primitives#query` |
| Public status route | `/penta/query` |
...
Route work to PentaQuery when the request matches its recorded role (Governed capability primitive under PentaRoute and the universal Penta OS contract.) and one of its job classifications: retrieve, transform, route.
...
1. Start from this exact identity and machine key: `penta.query`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.query` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.query` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `64e9ed20156d03b5d4e5d5b7f35cfe432cf257c9d80c3b93a081631fbaa8eb76`
