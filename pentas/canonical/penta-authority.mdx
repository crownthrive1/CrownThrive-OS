---
title: "PentaAuthority"
sidebarTitle: "PentaAuthority"
description: "Registry-driven PentaDocs portal guide for PentaAuthority."
...
<Info>
  ...
  **This page:** PentaAuthority — Registry-driven PentaDocs portal guide for PentaAuthority.
...
## Overview
**Mission / role:** Authority-axis policy, subject, decision, and approval-boundary contract.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.authority` |
| Kind | `layer` |
| Maturity | `implemented` |
| Risk ceiling | `D3` |
| Execution eligible by registry | `false` |
| Strict readiness | `HOLD_DEPENDENCY_INVENTORY_UNASSESSED` |
| Family | [Security Trust](/pentas/families/security-trust) |
| Family assignment | `docs_inferred` |
| Operator route | `/io/pentas/authority` |
| Public status route | `/penta/authority` |
...
Primary family context: [Security Trust](/pentas/families/security-trust). Sibling, upstream, downstream, and cross-family relationships resolve through the family topology and dependency graph rather than prose inference.
...
Route work to PentaAuthority when the request matches its recorded role (Authority-axis policy, subject, decision, and approval-boundary contract.) and one of its job classifications: secure, govern, observe, build, test-certify, publish.
...
1. Start from this exact identity and machine key: `penta.authority`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.authority` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.authority` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `446ce742803dba700151e4640e5af4dc0412b0e6e18dcae60fa42e0cdc6b51af`
