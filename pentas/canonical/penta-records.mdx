---
title: "PentaRecords"
sidebarTitle: "PentaRecords"
description: "Registry-driven PentaDocs portal guide for PentaRecords."
...
<Info>
  ...
  **This page:** PentaRecords — Registry-driven PentaDocs portal guide for PentaRecords.
...
## Overview
**Mission / role:** Manage institutional record classes, retention schedules, legal/administrative holds, disposition eligibility, evidentiary integrity and authoritative-record designation.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.records` |
| Kind | `system` |
...
| Operator route | `/io/pentas/records` |
| Public status route | `/penta/records` |
...
## Dependencies
`penta.audit`, `penta.docs`, `penta.generation`, `penta.legal`, `penta.privacy`
...
| --- | --- |
| Architectural layers | [Data, Knowledge & Semantics](/pentas/layers/data-knowledge), [Security & Trust](/pentas/layers/security-trust), [Observability & Reliability](/pentas/layers/observability-reliability) |
| Jobs / functions | [Document](/pentas/jobs/document), [Retrieve](/pentas/jobs/retrieve), [Transform](/pentas/jobs/transform), [Secure](/pentas/jobs/secure), [Govern](/pentas/jobs/govern), [Observe](/pentas/jobs/observe) |
...
Route work to PentaRecords when the request matches its recorded role (Manage institutional record classes, retention schedules, legal/administrative holds, disposition eligibility, evidentiary integrity and authoritative-record designation.) and one of its job classifications: document, retrieve, transform, secure, govern, observe.
...
5. Verify declared dependencies before execution: penta.audit, penta.docs, penta.generation, penta.legal, penta.privacy.
...
1. Start from this exact identity and machine key: `penta.records`.
2. Match the request to the documented jobs: `document`, `retrieve`, `transform`, `secure`, `govern`, `observe`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.records` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.records` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `86c60a1f9d553a47775ac129b8b7ee338f45664fe46c08404cae4c0f52a5e531`
