---
title: "PentaRFA"
sidebarTitle: "PentaRFA"
description: "Registry-driven PentaDocs portal guide for PentaRFA."
...
<Info>
  ...
  **This page:** PentaRFA — Registry-driven PentaDocs portal guide for PentaRFA.
...
## Overview
**Mission / role:** Govern requests for new agents/capabilities, including job need, scope, TTL, authority, tools, constraints, approval/conditional approval/denial and reapplication lockout state.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.rfa` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D3` |
...
| Operator route | `/io/pentas/rfa` |
| Public status route | `/penta/rfa` |
...
## Dependencies
`penta.alumni`, `penta.cost`, `penta.hybrid`, `penta.risk`
...
| --- | --- |
| Architectural layers | [Automation & Agentic](/pentas/layers/automation-agentic), [Security & Trust](/pentas/layers/security-trust) |
| Jobs / functions | [Orchestrate](/pentas/jobs/orchestrate), [Build](/pentas/jobs/build), [Support & Operate](/pentas/jobs/support-operate), [Secure](/pentas/jobs/secure), [Govern](/pentas/jobs/govern) |
...
Route work to PentaRFA when the request matches its recorded role (Govern requests for new agents/capabilities, including job need, scope, TTL, authority, tools, constraints, approval/conditional approval/denial and reapplication lockout state.) and one of its job classifications: orchestrate, build, support-operate, secure, govern.
...
5. Verify declared dependencies before execution: penta.alumni, penta.cost, penta.hybrid, penta.risk.
...
1. Start from this exact identity and machine key: `penta.rfa`.
2. Match the request to the documented jobs: `orchestrate`, `build`, `support-operate`, `secure`, `govern`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.rfa` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.rfa` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `ffcba9b344da2f382268e3be7c781b0d2540a76124beca9e958de6ba9474ec62`
