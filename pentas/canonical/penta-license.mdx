---
title: "PentaLicense"
sidebarTitle: "PentaLicense"
description: "Registry-driven PentaDocs portal guide for PentaLicense."
...
<Info>
  ...
  **This page:** PentaLicense — Registry-driven PentaDocs portal guide for PentaLicense.
...
## Overview
**Mission / role:** Evaluate exact asset rights profiles and license requests; prepare or create immutable internal grant records; route reviewed/provider issuance; and maintain append-only amendment, renewal, suspension, revocation and public-verification evidence.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.license` |
| Kind | `system` |
| Maturity | `production` |
| Risk ceiling | `D3` |
| Execution eligible by registry | `true` |
| Strict readiness | `HOLD_DEPENDENCY_MATURITY` |
| Family | [Penta Governance, Legal & Institutional Controls Family](/pentas/families/governance-legal) |
| Family assignment | `registry_backed` |
| Operator route | `/io/pentas/license` |
| Public status route | `/penta/license` |
...
Primary family context: [Penta Governance, Legal & Institutional Controls Family](/pentas/families/governance-legal). Sibling, upstream, downstream, and cross-family relationships resolve through the family topology and dependency graph rather than prose inference.

## Dependencies
`penta.assure`, `penta.audit`, `penta.compliance`, `penta.contracts`, `penta.hybrid`, `penta.identity`, `penta.legal`, `penta.records`
...
| --- | --- |
| Architectural layers | [Control & Governance](/pentas/layers/control-governance), [Security & Trust](/pentas/layers/security-trust), [Data, Knowledge & Semantics](/pentas/layers/data-knowledge), [Routing, Interop & Transport](/pentas/layers/routing-transport) |
| Jobs / functions | [Govern](/pentas/jobs/govern), [Secure](/pentas/jobs/secure), [Document](/pentas/jobs/document), [Retrieve](/pentas/jobs/retrieve), [Route](/pentas/jobs/route), [Transform](/pentas/jobs/transform) |
...
| Namespace | `canonical` |
| Registry execution eligible | `true` |
| Risk ceiling | `D3` |
| Strict readiness | `HOLD_DEPENDENCY_MATURITY` |
...
Route work to PentaLicense when the request matches its recorded role (Evaluate exact asset rights profiles and license requests; prepare or create immutable internal grant records; route reviewed/provider issuance; and maintain append-only amendment, renewal, suspension, revocation and public-verification evidence.) and one of its job classifications: govern, secure, document, retrieve, route, transform.
...
5. Verify declared dependencies before execution: penta.assure, penta.audit, penta.compliance, penta.contracts, penta.hybrid, penta.identity, penta.legal, penta.records.
6. For provider writes, require a certified provider binding, credential reference, idempotency boundary and provider readback.

## Quickstart

1. Start from this exact identity and machine key: `penta.license`.
2. Match the request to the documented jobs: `govern`, `secure`, `document`, `retrieve`, `route`, `transform`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.license` and the evidence paths below.
...
Execution eligibility is `true` from the namespace projection. That flag is necessary but never sufficient for a material provider action; the caller must also satisfy current authority, risk, provider-binding, readiness and evidence gates.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.license` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `728cb581d443bc784453099f76432be0c9e28c1d2b4f1c3a77489fcec22f43bb`
