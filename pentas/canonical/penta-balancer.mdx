...
<Info>
  ...
  **This page:** PentaBalancer — Registry-driven PentaDocs portal guide for PentaBalancer.
...
| --- | --- |
| Architectural layers | [Observability & Reliability](/pentas/layers/observability-reliability), [Build, Test & Release](/pentas/layers/build-release), [Communications, Growth & Service](/pentas/layers/communications-growth) |
...
Route work to PentaBalancer when the request matches its recorded role (Redundancy restoration, load shedding, and bounded capacity growth.) and one of its job classifications: observe, recover, support-operate, build, test-certify, publish.
...
1. Start from this exact identity and machine key: `penta.balancer`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.balancer` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.balancer` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `fe1fe4320506893874dc4a454b28fead7605d9d75a864f83f7447bf1739c6383`
