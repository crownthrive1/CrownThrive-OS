---
title: "PentaCost"
sidebarTitle: "PentaCost"
description: "Registry-driven PentaDocs portal guide for PentaCost."
...
<Info>
  ...
  **This page:** PentaCost — Registry-driven PentaDocs portal guide for PentaCost.
...
## Overview
**Mission / role:** Measure and govern PENTA operating cost, budgets, quotas, consumption, abuse prevention, unit economics and spend ceilings.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.cost` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D3` |
...
| Operator route | `/io/pentas/cost` |
| Public status route | `/penta/cost` |
...
## Dependencies
`penta.analytics`, `penta.audit`, `penta.board`
...
| --- | --- |
| Architectural layers | [Commerce & Economic](/pentas/layers/commerce-economic), [Control & Governance](/pentas/layers/control-governance), [Routing, Interop & Transport](/pentas/layers/routing-transport) |
| Jobs / functions | [Transact & Economic](/pentas/jobs/transact-economic), [Govern](/pentas/jobs/govern), [Observe](/pentas/jobs/observe), [Route](/pentas/jobs/route), [Retrieve](/pentas/jobs/retrieve), [Transform](/pentas/jobs/transform) |
...
Route work to PentaCost when the request matches its recorded role (Measure and govern PENTA operating cost, budgets, quotas, consumption, abuse prevention, unit economics and spend ceilings.) and one of its job classifications: transact-economic, govern, observe, route, retrieve, transform.
...
5. Verify declared dependencies before execution: penta.analytics, penta.audit, penta.board.
...
1. Start from this exact identity and machine key: `penta.cost`.
2. Match the request to the documented jobs: `transact-economic`, `govern`, `observe`, `route`, `retrieve`, `transform`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.cost` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.cost` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `cca5b4f54b7f94d4604b8c4e3c922aeab02f4001fcb4f35049428f03ccacfe8d`
