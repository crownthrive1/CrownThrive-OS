...
| Jobs / functions | [Document](/pentas/jobs/document), [Retrieve](/pentas/jobs/retrieve), [Build](/pentas/jobs/build), [Test & Certify](/pentas/jobs/test-certify), [Publish](/pentas/jobs/publish) |
...
Route work to PentaFlush when the request matches its recorded role (TTL-bounded cleanup of explicitly ephemeral state; canonical records are excluded.) and one of its job classifications: document, retrieve, build, test-certify, publish.
...
1. Start from this exact identity and machine key: `penta.flush`.
2. Match the request to the documented jobs: `document`, `retrieve`, `build`, `test-certify`, `publish`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.flush` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.flush` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `1429c7b2877a17cf76b81aad91bfb0fda3db6e68f949344248e6a518f97a6ce4`
