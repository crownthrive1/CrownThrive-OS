---
title: "PentaRelay"
sidebarTitle: "PentaRelay"
description: "Registry-driven PentaDocs portal guide for PentaRelay."
...
<Info>
  ...
  **This page:** PentaRelay — Registry-driven PentaDocs portal guide for PentaRelay.
...
## Overview
**Mission / role:** signed, allowlisted, durable, authority-aware event relay

| Field | Registry value |
| --- | --- |
| Machine key | `penta.relay` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D2` |
...
| Operator route | `/io/pentas/relay` |
| Public status route | `/penta/relay` |
...
| --- | --- |
| Architectural layers | [Security & Trust](/pentas/layers/security-trust), [Routing, Interop & Transport](/pentas/layers/routing-transport) |
| Jobs / functions | [Secure](/pentas/jobs/secure), [Govern](/pentas/jobs/govern), [Route](/pentas/jobs/route), [Retrieve](/pentas/jobs/retrieve), [Transform](/pentas/jobs/transform) |
...
Route work to PentaRelay when the request matches its recorded role (signed, allowlisted, durable, authority-aware event relay) and one of its job classifications: secure, govern, route, retrieve, transform.
...
1. Start from this exact identity and machine key: `penta.relay`.
2. Match the request to the documented jobs: `secure`, `govern`, `route`, `retrieve`, `transform`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.relay` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.relay` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `d56ddb588e0b3453e1cb98180e32971a8544b59dd5990483e5aa070956b3c6cd`
