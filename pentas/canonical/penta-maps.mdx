---
title: "PentaMaps"
sidebarTitle: "PentaMaps"
description: "Registry-driven PentaDocs portal guide for PentaMaps."
...
<Info>
  ...
  **This page:** PentaMaps — Registry-driven PentaDocs portal guide for PentaMaps.
...
## Overview
**Mission / role:** system/relationship/status visualizations and operational maps

| Field | Registry value |
| --- | --- |
| Machine key | `penta.maps` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D3` |
...
| Operator route | `/io/pentas/maps` |
| Public status route | `/penta/maps` |
...
| Jobs / functions | [Document](/pentas/jobs/document), [Retrieve](/pentas/jobs/retrieve), [Transform](/pentas/jobs/transform), [Observe](/pentas/jobs/observe), [Recover](/pentas/jobs/recover) |
...
Route work to PentaMaps when the request matches its recorded role (system/relationship/status visualizations and operational maps) and one of its job classifications: document, retrieve, transform, observe, recover.
...
1. Start from this exact identity and machine key: `penta.maps`.
2. Match the request to the documented jobs: `document`, `retrieve`, `transform`, `observe`, `recover`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.maps` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.maps` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `47fd13ee6e5be55630f1a50805975bd4b58c130d4087fa5233456b7f5dbebc60`
