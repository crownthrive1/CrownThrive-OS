---
title: "PentaParse"
sidebarTitle: "PentaParse"
description: "Registry-driven PentaDocs portal guide for PentaParse."
...
<Info>
  ...
  **This page:** PentaParse — Registry-driven PentaDocs portal guide for PentaParse.
...
| Operator route | `/io/pentas/primitives#parse` |
| Public status route | `/penta/parse` |
...
Route work to PentaParse when the request matches its recorded role (Governed capability primitive under PentaRoute and the universal Penta OS contract.) and one of its job classifications: retrieve, transform, route.
...
1. Start from this exact identity and machine key: `penta.parse`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.parse` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.parse` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `5fef3351fbc8fc0bb7c4e727cd76530dbc12800e5ed0c0f5c86f7d3f15eed9dd`
