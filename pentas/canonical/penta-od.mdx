...
<Info>
  ...
  **This page:** PentaOD — Registry-driven PentaDocs portal guide for PentaOD.
...
| --- | --- |
| Architectural layers | [Observability & Reliability](/pentas/layers/observability-reliability), [Build, Test & Release](/pentas/layers/build-release), [Security & Trust](/pentas/layers/security-trust), [Routing, Interop & Transport](/pentas/layers/routing-transport) |
| Jobs / functions | [Recover](/pentas/jobs/recover), [Observe](/pentas/jobs/observe), [Support & Operate](/pentas/jobs/support-operate), [Secure](/pentas/jobs/secure), [Govern](/pentas/jobs/govern), [Route](/pentas/jobs/route) |
...
Route work to PentaOD when the request matches its recorded role (On-demand readiness, wake, idle and dispatch coordination for CrownThrive capabilities that already possess the required authority, credentials and certification.) and one of its job classifications: recover, observe, support-operate, secure, govern, route.
...
5. Verify declared dependencies before execution: penta.beata, penta.certify, penta.fabric, penta.heartbeat, penta.mesh, penta.status.
...
1. Start from this exact identity and machine key: `penta.od`.
2. Match the request to the documented jobs: `recover`, `observe`, `support-operate`, `secure`, `govern`, `route`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.od` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.od` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `be6307e1dc129ffc5a0142907d5f59d3a4c711e128370dbc797da47eb12f41b3`
