---
title: "PentaDeploy"
sidebarTitle: "PentaDeploy"
description: "Registry-driven PentaDocs portal guide for PentaDeploy."
...
<Info>
  ...
  **This page:** PentaDeploy — Registry-driven PentaDocs portal guide for PentaDeploy.
...
| Operator route | `/io/pentas/primitives#deploy` |
| Public status route | `/penta/deploy` |
...
| --- | --- |
| Architectural layers | [Routing, Interop & Transport](/pentas/layers/routing-transport), [Build, Test & Release](/pentas/layers/build-release) |
| Jobs / functions | [Retrieve](/pentas/jobs/retrieve), [Transform](/pentas/jobs/transform), [Route](/pentas/jobs/route), [Build](/pentas/jobs/build), [Test & Certify](/pentas/jobs/test-certify), [Publish](/pentas/jobs/publish) |
...
Route work to PentaDeploy when the request matches its recorded role (Governed PentaRoute capability primitive.) and one of its job classifications: retrieve, transform, route, build, test-certify, publish.
...
1. Start from this exact identity and machine key: `penta.deploy`.
2. Match the request to the documented jobs: `retrieve`, `transform`, `route`, `build`, `test-certify`, `publish`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.deploy` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.deploy` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `8646c8f82a895ada67afbbaa40f7d6b78a30d56ade2bef1446aac8f7ada29652`
