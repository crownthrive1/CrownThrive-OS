---
title: "PentaSecurity"
sidebarTitle: "PentaSecurity"
description: "Registry-driven PentaDocs portal guide for PentaSecurity."
...
<Info>
  ...
  **This page:** PentaSecurity — Registry-driven PentaDocs portal guide for PentaSecurity.
...
**Mission / role:** Security posture, threat/control assessment, incident-governance routing, protective-action proposals, verification and security evidence coordination.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.security` |
| Kind | `system` |
| Current registry maturity | `production` |
| Registry risk ceiling | `D3` |
| Bounded runtime risk | `D1` |
| Current identity/runtime posture | `production_active_pm_executable` / `RUNTIME_PRESENT` |
| Current runtime contract | `ct.penta.security.runtime-review.v1` |
| Runtime reference | `function:penta_security.review_system_v1(text)` |
| Latest runtime security review | `PASS` — 2026-09-02T16:02:18Z |
| Latest runtime review evidence | `71bc64dea49be3e8c1ac44fb6dbd685038f09e496c1c17872752acc3a8f939fa` |
| Current assignment-transport repair | PR `#2574` @ `12c43a32b6a78234eae820ecfd76426cf0f6fa27` — exact-current prerelease lane, not yet production-applied |
| Pending Pentas v2 transport ceiling | `D2` for `ct.penta.security`; D3 remains human-reserved |
| Family | [Penta Security, Identity & Trust Family](/pentas/families/security-trust) |
| Family assignment | `registry_backed` |
| Operator route | `/io/pentas/security` |
| Public status route | `/penta/security` |

<Warning>
  Runtime truth supersedes the older migrated namespace snapshot on this page. The current PentaSecurity runtime is production-registered and its exact runtime ACL/security review is `PASS`. The separate PR `#2574` only repairs the missing Pentas v2 assignment-mobilization route; it has passed current exact-source transactional rollback/security QC but is not merged or production-applied yet. Documentation does not promote that PR to production and does not create authority.
</Warning>
...
| --- | --- |
| Architectural layers | [Security & Trust](/pentas/layers/security-trust), [Control & Governance](/pentas/layers/control-governance), [Build, Test & Release](/pentas/layers/build-release) |
| Jobs / functions | [Secure](/pentas/jobs/secure), [Govern](/pentas/jobs/govern), [Observe](/pentas/jobs/observe), [Build](/pentas/jobs/build), [Test & Certify](/pentas/jobs/test-certify), [Publish](/pentas/jobs/publish) |
...
| Current identity/runtime posture | `production_active_pm_executable` / `RUNTIME_PRESENT` |
| Registry risk ceiling | `D3` |
| Current bounded runtime security review | `PASS` (exact runtime/ACL review; material execution still requires exact current gates) |
...
Route work to PentaSecurity when the request matches its recorded role (Security posture, threat/control assessment, incident-governance routing, protective-action proposals, verification and security evidence coordination.) and one of its job classifications: secure, govern, observe, build, test-certify, publish.
...
5. The current identity projection is `production_active_pm_executable`, but execution remains bounded by the runtime contract, D3 human-reserved controls, provider bindings, current authority, and exact release/readback gates. Documentation never creates execution authority.

## Quickstart

1. Start from this exact identity and machine key: `penta.security`.
2. Match the request to the documented jobs: `secure`, `govern`, `observe`, `build`, `test-certify`, `publish`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.security` and the evidence paths below.
...
The current identity projection is `production_active_pm_executable`, while the public registry remains non-public and the material authority boundary is unchanged. That posture is necessary but never sufficient for a material provider action; the caller must also satisfy current authority, risk, provider-binding, D3 human-reserved, readiness, exact-head and evidence gates.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.security` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `6f0e16e127e0e6a87eaac4a9f320df17ab265a22ea73b8ba00a36795ac1837db`
