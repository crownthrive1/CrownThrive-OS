---
title: "PentaBeata"
sidebarTitle: "PentaBeata"
description: "Registry-driven PentaDocs portal guide for PentaBeata."
...
<Info>
  ...
  **This page:** PentaBeata — Registry-driven PentaDocs portal guide for PentaBeata.
...
## Overview
**Mission / role:** Public-safe heartbeat and liveness primitive that projects PentaRoute plus sanitized PentaOD/PentaHeartbeat readiness without executing provider actions.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.beata` |
| Kind | `system` |
| Maturity | `production` |
| Risk ceiling | `D0` |
...
| Operator route | `/io/pentas/beata` |
| Public status route | `/penta/beata` |
...
## Dependencies
`penta.heartbeat`, `penta.od`, `penta.status`
...
Route work to PentaBeata when the request matches its recorded role (Public-safe heartbeat and liveness primitive that projects PentaRoute plus sanitized PentaOD/PentaHeartbeat readiness without executing provider actions.) and one of its job classifications: recover, observe, support-operate, route, retrieve, transform.
...
5. Verify declared dependencies before execution: penta.heartbeat, penta.od, penta.status.
...
1. Start from this exact identity and machine key: `penta.beata`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.beata` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.beata` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `aa8d377f6696d4b455ecdde9fc1a089f34695d1443f36c30f940e335f672156e`
