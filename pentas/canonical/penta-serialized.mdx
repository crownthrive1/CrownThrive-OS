---
title: "PentaSerialized"
sidebarTitle: "PentaSerialized"
description: "Registry-driven PentaDocs portal guide for PentaSerialized."
...
<Info>
  ...
  **This page:** PentaSerialized — Registry-driven PentaDocs portal guide for PentaSerialized.
...
## Overview
**Mission / role:** append-only serialization, stable identity, anti-overwrite/delete gating, tombstones, lineage, integrity, snapshots, and continuity receipts

| Field | Registry value |
| --- | --- |
| Machine key | `penta.serialized` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D3` |
...
| Operator route | `/io/pentas/serialized` |
| Public status route | `/penta/serialized` |
...
| Jobs / functions | [Recover](/pentas/jobs/recover), [Observe](/pentas/jobs/observe), [Support & Operate](/pentas/jobs/support-operate) |
...
| Risk ceiling | `D3` |
| Strict readiness | `HOLD_DEPENDENCY_INVENTORY_UNASSESSED` |
| Classification provenance | `family_default` / `family_default` |
...
Route work to PentaSerialized when the request matches its recorded role (append-only serialization, stable identity, anti-overwrite/delete gating, tombstones, lineage, integrity, snapshots, and continuity receipts) and one of its job classifications: recover, observe, support-operate.
...
1. Start from this exact identity and machine key: `penta.serialized`.
2. Match the request to the documented jobs: `recover`, `observe`, `support-operate`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.serialized` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.serialized` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `5d64a850c016f9b1bf9ed6de73f82c3820b379dcb83a713b71214d36cbb2868b`
