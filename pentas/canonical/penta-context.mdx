---
title: "PentaContext"
sidebarTitle: "PentaContext"
description: "Registry-driven PentaDocs portal guide for PentaContext."
...
<Info>
  ...
  **This page:** PentaContext — Registry-driven PentaDocs portal guide for PentaContext.
...
## Overview
**Mission / role:** Scoped context ingestion, provenance, redaction, retrieval, retention, and operational-memory plane; context is evidence and never authority.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.context` |
| Kind | `system` |
...
| Operator route | `/io/pentas/context` |
| Public status route | `/penta/context` |
...
- `supabase/functions/penta-context/index.ts`
- `supabase/migrations/20260826230616_penta_context_production_v1.sql`
- `supabase/migrations/20260826231217_penta_context_production_promotion_v1.sql`
- `supabase/migrations/20260826232536_penta_context_automation_promotion_v1_1.sql`
...
| --- | --- |
| Architectural layers | [Data, Knowledge & Semantics](/pentas/layers/data-knowledge), [Security & Trust](/pentas/layers/security-trust) |
| Jobs / functions | [Document](/pentas/jobs/document), [Retrieve](/pentas/jobs/retrieve), [Transform](/pentas/jobs/transform), [Secure](/pentas/jobs/secure), [Govern](/pentas/jobs/govern) |
...
Route work to PentaContext when the request matches its recorded role (Scoped context ingestion, provenance, redaction, retrieval, retention, and operational-memory plane; context is evidence and never authority.) and one of its job classifications: document, retrieve, transform, secure, govern.
...
1. Start from this exact identity and machine key: `penta.context`.
2. Match the request to the documented jobs: `document`, `retrieve`, `transform`, `secure`, `govern`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.context` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.context` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `f9aadaddbd03327a648933a58b342fa83ec1583a43688d9b640aec064defa51d`
