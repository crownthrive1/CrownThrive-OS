---
title: "PentaStudios"
sidebarTitle: "PentaStudios"
description: "Registry-driven PentaDocs portal guide for PentaStudios."
...
<Info>
  ...
  **This page:** PentaStudios — Registry-driven PentaDocs portal guide for PentaStudios.
...
## Overview
**Mission / role:** Governed studio, podcast, recording, livestream and production assets, providers, routes and control surfaces.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.studios` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D2` |
| Execution eligible by registry | `false` |
| Strict readiness | `HOLD_MEMBER_MATURITY` |
| Family | [Penta Media, Studio & Publishing Family](/pentas/families/media-creative) |
| Family assignment | `registry_backed` |
| Operator route | `/io/pentas/studios` |
| Public status route | `/penta/studios` |
...
Primary family context: [Penta Media, Studio & Publishing Family](/pentas/families/media-creative). Sibling, upstream, downstream, and cross-family relationships resolve through the family topology and dependency graph rather than prose inference.

## Dependencies
`penta.docs`, `penta.mation`, `penta.media`
...
| --- | --- |
| Architectural layers | [Media, Creative & Publishing](/pentas/layers/media-creative), [Experience & Interface](/pentas/layers/experience-interface), [Data, Knowledge & Semantics](/pentas/layers/data-knowledge), [Routing, Interop & Transport](/pentas/layers/routing-transport) |
| Jobs / functions | [Publish](/pentas/jobs/publish), [Build](/pentas/jobs/build), [Communicate & Grow](/pentas/jobs/communicate-grow), [Document](/pentas/jobs/document), [Retrieve](/pentas/jobs/retrieve), [Route](/pentas/jobs/route) |
...
Route work to PentaStudios when the request matches its recorded role (Governed studio, podcast, recording, livestream and production assets, providers, routes and control surfaces.) and one of its job classifications: publish, build, communicate-grow, document, retrieve, route.
...
5. Verify declared dependencies before execution: penta.docs, penta.mation, penta.media.
...
1. Start from this exact identity and machine key: `penta.studios`.
2. Match the request to the documented jobs: `publish`, `build`, `communicate-grow`, `document`, `retrieve`, `route`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.studios` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.studios` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `ae58439d2a4645ada56534c2f71fd7d0c051291c0ef547693f83721f69e432f9`
