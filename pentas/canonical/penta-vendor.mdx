---
title: "PentaVendor"
sidebarTitle: "PentaVendor"
description: "Registry-driven PentaDocs portal guide for PentaVendor."
...
<Info>
  ...
  **This page:** PentaVendor — Registry-driven PentaDocs portal guide for PentaVendor.
...
## Overview
**Mission / role:** Govern vendor/provider onboarding, due diligence, ownership, risk, service levels, renewals, performance, dependencies, concentration and offboarding.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.vendor` |
| Kind | `system` |
...
| Operator route | `/io/pentas/vendor` |
| Public status route | `/penta/vendor` |
...
## Dependencies
`penta.certify`, `penta.contracts`, `penta.credentials`, `penta.procure`, `penta.risk`, `penta.security`
...
| --- | --- |
| Architectural layers | [Control & Governance](/pentas/layers/control-governance), [Security & Trust](/pentas/layers/security-trust), [Build, Test & Release](/pentas/layers/build-release), [Observability & Reliability](/pentas/layers/observability-reliability), [Experience & Interface](/pentas/layers/experience-interface) |
| Jobs / functions | [Govern](/pentas/jobs/govern), [Secure](/pentas/jobs/secure), [Document](/pentas/jobs/document), [Build](/pentas/jobs/build), [Test & Certify](/pentas/jobs/test-certify), [Publish](/pentas/jobs/publish) |
...
Route work to PentaVendor when the request matches its recorded role (Govern vendor/provider onboarding, due diligence, ownership, risk, service levels, renewals, performance, dependencies, concentration and offboarding.) and one of its job classifications: govern, secure, document, build, test-certify, publish.
...
5. Verify declared dependencies before execution: penta.certify, penta.contracts, penta.credentials, penta.procure, penta.risk, penta.security.
...
1. Start from this exact identity and machine key: `penta.vendor`.
2. Match the request to the documented jobs: `govern`, `secure`, `document`, `build`, `test-certify`, `publish`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.vendor` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.vendor` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `3b4e5a0384ce0952c2dcb5b769e91a5d82393975274d9c89c963871c374d24e9`
