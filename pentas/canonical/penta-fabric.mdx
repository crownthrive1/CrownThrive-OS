...
description: "Registry-driven PentaDocs portal guide for PentaFabric."
...
<Info>
  ...
  **This page:** PentaFabric — Registry-driven PentaDocs portal guide for PentaFabric.
...
## Overview
**Mission / role:** Production runtime and federation fabric compatibility surface connecting governed PENTA capabilities without collapsing their authority boundaries.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.fabric` |
| Kind | `system` |
| Maturity | `production` |
| Risk ceiling | `D2` |
| Execution eligible by registry | `true` |
| Strict readiness | `HOLD_UNCLASSIFIED_DEPENDENCY_CYCLE` |
| Family | [Penta System Architecture Family](/pentas/families/system-architecture) |
| Family assignment | `registry_backed` |
| Operator route | `/io/pentas/fabric` |
| Public status route | `/penta/fabric` |
...
Primary family context: [Penta System Architecture Family](/pentas/families/system-architecture). Sibling, upstream, downstream, and cross-family relationships resolve through the family topology and dependency graph rather than prose inference.

## Dependencies
`penta.federation`, `penta.mcp`, `penta.mesh`, `penta.status`
...
| --- | --- |
| Architectural layers | [Foundation & Kernel](/pentas/layers/foundation-kernel), [Control & Governance](/pentas/layers/control-governance), [Security & Trust](/pentas/layers/security-trust) |
| Jobs / functions | [Architect](/pentas/jobs/architect), [Govern](/pentas/jobs/govern), [Support & Operate](/pentas/jobs/support-operate), [Secure](/pentas/jobs/secure) |
...
Route work to PentaFabric when the request matches its recorded role (Production runtime and federation fabric compatibility surface connecting governed PENTA capabilities without collapsing their authority boundaries.) and one of its job classifications: architect, govern, support-operate, secure.
...
5. Verify declared dependencies before execution: penta.federation, penta.mcp, penta.mesh, penta.status.
...
1. Start from this exact identity and machine key: `penta.fabric`.
2. Match the request to the documented jobs: `architect`, `govern`, `support-operate`, `secure`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.fabric` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.fabric` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `9ed8ab4a34c48b73dbc371c1edfb7357ad67839748c08dd8b9fd8c533d9dfddd`
