---
title: "PentaNurture"
sidebarTitle: "PentaNurture"
description: "Registry-driven PentaDocs portal guide for PentaNurture."
...
<Info>
  ...
  **This page:** PentaNurture — Registry-driven PentaDocs portal guide for PentaNurture.
...
## Overview
**Mission / role:** Continuously nurse and maintain registered software, integrations, dependencies, health signals, sessions and lifecycle obligations.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.nurture` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D2` |
...
| Operator route | `/io/pentas/nurture` |
| Public status route | `/penta/nurture` |
...
## Dependencies
`penta.build`, `penta.credentials`, `penta.status`, `penta.triage`
...
| --- | --- |
| Architectural layers | [Observability & Reliability](/pentas/layers/observability-reliability), [Build, Test & Release](/pentas/layers/build-release), [Data, Knowledge & Semantics](/pentas/layers/data-knowledge) |
| Jobs / functions | [Recover](/pentas/jobs/recover), [Observe](/pentas/jobs/observe), [Support & Operate](/pentas/jobs/support-operate), [Build](/pentas/jobs/build), [Test & Certify](/pentas/jobs/test-certify), [Publish](/pentas/jobs/publish) |
...
Route work to PentaNurture when the request matches its recorded role (Continuously nurse and maintain registered software, integrations, dependencies, health signals, sessions and lifecycle obligations.) and one of its job classifications: recover, observe, support-operate, build, test-certify, publish.
...
5. Verify declared dependencies before execution: penta.build, penta.credentials, penta.status, penta.triage.
...
1. Start from this exact identity and machine key: `penta.nurture`.
2. Match the request to the documented jobs: `recover`, `observe`, `support-operate`, `build`, `test-certify`, `publish`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.nurture` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.nurture` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `72bb141d81d1e91320a1fed8626c91b2d20b9d0b837bf0a662df301eb1b1ac67`
