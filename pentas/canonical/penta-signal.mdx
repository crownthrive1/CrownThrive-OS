---
title: "PentaSignal"
sidebarTitle: "PentaSignal"
description: "Registry-driven PentaDocs portal guide for PentaSignal."
...
<Info>
  ...
  **This page:** PentaSignal — Registry-driven PentaDocs portal guide for PentaSignal.
...
## Overview
**Mission / role:** Early warning, weak-signal detection, anomaly sensing and strategic watch across internal telemetry and approved external sources.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.signal` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D1` |
...
| Operator route | `/io/pentas/signal` |
| Public status route | `/penta/signal` |
...
| --- | --- |
| Architectural layers | [Data, Knowledge & Semantics](/pentas/layers/data-knowledge), [Observability & Reliability](/pentas/layers/observability-reliability) |
| Jobs / functions | [Analyze & Research](/pentas/jobs/analyze-research), [Retrieve](/pentas/jobs/retrieve), [Observe](/pentas/jobs/observe), [Recover](/pentas/jobs/recover) |
...
Route work to PentaSignal when the request matches its recorded role (Early warning, weak-signal detection, anomaly sensing and strategic watch across internal telemetry and approved external sources.) and one of its job classifications: analyze-research, retrieve, observe, recover.
...
5. Verify declared dependencies before execution: penta.docs.
...
1. Start from this exact identity and machine key: `penta.signal`.
2. Match the request to the documented jobs: `analyze-research`, `retrieve`, `observe`, `recover`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.signal` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.signal` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `2675b7573067c5489e93cd6d2c17b1745fb3e791bb8c527038dc9d0cd92c0191`
