---
title: "PentaResolve"
sidebarTitle: "PentaResolve"
description: "Registry-driven PentaDocs portal guide for PentaResolve."
...
<Info>
  ...
  **This page:** PentaResolve — Registry-driven PentaDocs portal guide for PentaResolve.
...
| Operator route | `/io/pentas/primitives#resolve` |
| Public status route | `/penta/resolve` |
...
Route work to PentaResolve when the request matches its recorded role (Governed capability primitive under PentaRoute and the universal Penta OS contract.) and one of its job classifications: retrieve, transform, route.
...
1. Start from this exact identity and machine key: `penta.resolve`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.resolve` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.resolve` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `5632d8dafb48eabc4cfb089d78253e16c19ce2b7100424bf006784b269cc46cc`
