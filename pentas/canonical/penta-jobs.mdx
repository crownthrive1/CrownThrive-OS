...
<Info>
  ...
  **This page:** PentaJobs — Registry-driven PentaDocs portal guide for PentaJobs.
...
Route work to PentaJobs when the request matches its recorded role (Generation-61 PentaFabric job subcomponent.) and one of its job classifications: architect, govern, support-operate.
...
1. Start from this exact identity and machine key: `penta.jobs`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.jobs` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.jobs` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `ac915bc4a2d20515a1b5beaaf72f211907da63be607c67db0c96a3c7268a2ae4`
