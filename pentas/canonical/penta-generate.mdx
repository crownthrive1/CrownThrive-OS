...
Route work to PentaGenerate when the request matches its recorded role (Governed PentaRoute capability primitive.) and one of its job classifications: retrieve, transform, route.
...
1. Start from this exact identity and machine key: `penta.generate`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.generate` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.generate` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `495725cfd34e8b2105a734a35c93a0bfb4d40ca56c3e421f0f1a8274e1069d75`
