---
title: "PentaMedia"
sidebarTitle: "PentaMedia"
description: "Registry-driven PentaDocs portal guide for PentaMedia."
...
<Info>
  ...
  **This page:** PentaMedia — Registry-driven PentaDocs portal guide for PentaMedia.
...
## Overview
**Mission / role:** Governed media assets, channels, schedules, provider bindings, delivery, maintenance and continuity.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.media` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D2` |
...
| Operator route | `/io/pentas/media` |
| Public status route | `/penta/media` |
...
| --- | --- |
| Architectural layers | [Media, Creative & Publishing](/pentas/layers/media-creative), [Experience & Interface](/pentas/layers/experience-interface), [Routing, Interop & Transport](/pentas/layers/routing-transport) |
| Jobs / functions | [Publish](/pentas/jobs/publish), [Build](/pentas/jobs/build), [Communicate & Grow](/pentas/jobs/communicate-grow), [Route](/pentas/jobs/route), [Retrieve](/pentas/jobs/retrieve), [Transform](/pentas/jobs/transform) |
...
Route work to PentaMedia when the request matches its recorded role (Governed media assets, channels, schedules, provider bindings, delivery, maintenance and continuity.) and one of its job classifications: publish, build, communicate-grow, route, retrieve, transform.
...
5. Verify declared dependencies before execution: penta.docs, penta.mation.
...
1. Start from this exact identity and machine key: `penta.media`.
2. Match the request to the documented jobs: `publish`, `build`, `communicate-grow`, `route`, `retrieve`, `transform`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.media` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.media` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `fc51730741f3ac51a3c2cd1fcaa7e9234cac643ea13c0f77162102bd2152646b`
