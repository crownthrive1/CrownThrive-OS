---
title: "PentaQuality"
sidebarTitle: "PentaQuality"
description: "Registry-driven PentaDocs portal guide for PentaQuality."
...
<Info>
  ...
  **This page:** PentaQuality — Registry-driven PentaDocs portal guide for PentaQuality.
...
## Overview
**Mission / role:** Govern quality standards, acceptance criteria, nonconformances, corrective/preventive actions, root-cause analysis, quality plans and continuous-improvement evidence across software and operations.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.quality` |
| Kind | `system` |
...
| Operator route | `/io/pentas/quality` |
| Public status route | `/penta/quality` |
...
## Dependencies
`penta.assure`, `penta.audit`, `penta.build`, `penta.nurture`, `penta.status`
...
| --- | --- |
| Architectural layers | [Build, Test & Release](/pentas/layers/build-release), [Routing, Interop & Transport](/pentas/layers/routing-transport), [Experience & Interface](/pentas/layers/experience-interface) |
| Jobs / functions | [Build](/pentas/jobs/build), [Test & Certify](/pentas/jobs/test-certify), [Publish](/pentas/jobs/publish), [Route](/pentas/jobs/route), [Retrieve](/pentas/jobs/retrieve), [Transform](/pentas/jobs/transform) |
...
Route work to PentaQuality when the request matches its recorded role (Govern quality standards, acceptance criteria, nonconformances, corrective/preventive actions, root-cause analysis, quality plans and continuous-improvement evidence across software and operations.) and one of its job classifications: build, test-certify, publish, route, retrieve, transform.
...
5. Verify declared dependencies before execution: penta.assure, penta.audit, penta.build, penta.nurture, penta.status.
...
1. Start from this exact identity and machine key: `penta.quality`.
2. Match the request to the documented jobs: `build`, `test-certify`, `publish`, `route`, `retrieve`, `transform`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.quality` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.quality` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `ae66353554a52f4eade83919892cded32196671b860017eae1ca9d927b061a92`
