---
title: "PentaRead"
sidebarTitle: "PentaRead"
description: "Registry-driven PentaDocs portal guide for PentaRead."
...
<Info>
  ...
  **This page:** PentaRead — Registry-driven PentaDocs portal guide for PentaRead.
...
| Operator route | `/io/pentas/primitives#read` |
| Public status route | `/penta/read` |
...
Route work to PentaRead when the request matches its recorded role (Governed capability primitive under PentaRoute and the universal Penta OS contract.) and one of its job classifications: retrieve, transform, route.
...
1. Start from this exact identity and machine key: `penta.read`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.read` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.read` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `7b098979d3e2927fdb9dae70a016892938b991bd53d77cf78defebf65c24d598`
