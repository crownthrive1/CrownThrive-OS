...
<Info>
  ...
  **This page:** PentaPatch — Registry-driven PentaDocs portal guide for PentaPatch.
...
Route work to PentaPatch when the request matches its recorded role (certified partial-mutation primitive) and one of its job classifications: retrieve, transform, route.
...
1. Start from this exact identity and machine key: `penta.patch`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.patch` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.patch` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `4435a69508cddcc497ad914bd8d9fd7f74d27c1ef06f884f4df59a1ff1c821d6`
