---
title: "PentaIngest"
sidebarTitle: "PentaIngest"
description: "Registry-driven PentaDocs portal guide for PentaIngest."
...
<Info>
  ...
  **This page:** PentaIngest — Registry-driven PentaDocs portal guide for PentaIngest.
...
| Operator route | `/io/pentas/primitives#ingest` |
| Public status route | `/penta/ingest` |
...
Route work to PentaIngest when the request matches its recorded role (Governed capability primitive under PentaRoute and the universal Penta OS contract.) and one of its job classifications: retrieve, transform, route.
...
1. Start from this exact identity and machine key: `penta.ingest`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.ingest` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.ingest` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `73f155d9fef1b4719d1d9f1ad80f649c73d0825eaf833dc6534894e7ed3e634a`
