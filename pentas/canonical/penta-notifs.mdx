---
title: "PentaNotifs"
sidebarTitle: "PentaNotifs"
description: "Registry-driven PentaDocs portal guide for PentaNotifs."
...
<Info>
  ...
  **This page:** PentaNotifs — Registry-driven PentaDocs portal guide for PentaNotifs.
...
## Overview
**Mission / role:** Dedupe-aware incident notification and escalation with PentaMail as the delivery rail.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.notifs` |
| Kind | `system` |
...
| Operator route | `/io/pentas/notifs` |
| Public status route | `/penta/notifs` |
...
| --- | --- |
| Architectural layers | [Build, Test & Release](/pentas/layers/build-release), [Communications, Growth & Service](/pentas/layers/communications-growth) |
| Jobs / functions | [Build](/pentas/jobs/build), [Test & Certify](/pentas/jobs/test-certify), [Publish](/pentas/jobs/publish), [Communicate & Grow](/pentas/jobs/communicate-grow) |
...
| Risk ceiling | `D1` |
| Strict readiness | `HOLD_DEPENDENCY_INVENTORY_UNASSESSED` |
| Classification provenance | `docs_inferred` / `docs_inferred` |
...
Route work to PentaNotifs when the request matches its recorded role (Dedupe-aware incident notification and escalation with PentaMail as the delivery rail.) and one of its job classifications: build, test-certify, publish, communicate-grow.
...
1. Start from this exact identity and machine key: `penta.notifs`.
2. Match the request to the documented jobs: `build`, `test-certify`, `publish`, `communicate-grow`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.notifs` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.notifs` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `24388b32ee6418331a43604162977680a22f1f6340402483a7785e578f77229f`
