---
title: "PentaFederation"
sidebarTitle: "PentaFederation"
description: "Registry-driven PentaDocs portal guide for PentaFederation."
...
<Info>
  ...
  **This page:** PentaFederation — Registry-driven PentaDocs portal guide for PentaFederation.
...
## Overview
**Mission / role:** Cross-system, cross-repository and cross-provider bindings, trust relationships, events, proofs and federation state.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.federation` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D3` |
| Execution eligible by registry | `false` |
| Strict readiness | `HOLD_MEMBER_MATURITY` |
| Family | [Penta Routing & Interoperability Family](/pentas/families/routing-interoperability) |
| Family assignment | `registry_backed` |
| Operator route | `/io/pentas/federation` |
| Public status route | `/penta/federation` |
...
Primary family context: [Penta Routing & Interoperability Family](/pentas/families/routing-interoperability). Sibling, upstream, downstream, and cross-family relationships resolve through the family topology and dependency graph rather than prose inference.

## Dependencies
`penta.assure`, `penta.docs`, `penta.route`
...
| --- | --- |
| Architectural layers | [Routing, Interop & Transport](/pentas/layers/routing-transport), [Foundation & Kernel](/pentas/layers/foundation-kernel), [Security & Trust](/pentas/layers/security-trust), [Observability & Reliability](/pentas/layers/observability-reliability) |
| Jobs / functions | [Route](/pentas/jobs/route), [Orchestrate](/pentas/jobs/orchestrate), [Support & Operate](/pentas/jobs/support-operate), [Secure](/pentas/jobs/secure), [Govern](/pentas/jobs/govern), [Retrieve](/pentas/jobs/retrieve) |
...
Route work to PentaFederation when the request matches its recorded role (Cross-system, cross-repository and cross-provider bindings, trust relationships, events, proofs and federation state.) and one of its job classifications: route, orchestrate, support-operate, secure, govern, retrieve.
...
5. Verify declared dependencies before execution: penta.assure, penta.docs, penta.route.
...
1. Start from this exact identity and machine key: `penta.federation`.
2. Match the request to the documented jobs: `route`, `orchestrate`, `support-operate`, `secure`, `govern`, `retrieve`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.federation` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.federation` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `72671823c7b33d067a4faa65af4c81e5eb840efc28cc59d029a657c0c3874580`
