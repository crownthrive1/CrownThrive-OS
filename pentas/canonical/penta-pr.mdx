---
title: "PentaPR"
sidebarTitle: "PentaPR"
description: "Registry-driven PentaDocs portal guide for PentaPR."
...
<Info>
  ...
  **This page:** PentaPR — Registry-driven PentaDocs portal guide for PentaPR.
...
## Overview
**Mission / role:** Stack, tag, classify, nurture and govern pull-request lifecycle state so downstream merge and closer systems know dependencies, deadlines and dispositions.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.pr` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D2` |
...
| Operator route | `/io/pentas/pr` |
| Public status route | `/penta/pr` |
...
## Dependencies
`penta.build`, `penta.closer`, `penta.merge`, `penta.nurture`
...
Route work to PentaPR when the request matches its recorded role (Stack, tag, classify, nurture and govern pull-request lifecycle state so downstream merge and closer systems know dependencies, deadlines and dispositions.) and one of its job classifications: build, test-certify, publish, route, retrieve, transform.
...
5. Verify declared dependencies before execution: penta.build, penta.closer, penta.merge, penta.nurture.
...
1. Start from this exact identity and machine key: `penta.pr`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.pr` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.pr` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `b0b3e7d0239b5995f98a0236012b636b8ed5dba64902df93932a10b1640a2561`
