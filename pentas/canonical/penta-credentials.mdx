---
title: "PentaCredentials"
sidebarTitle: "PentaCredentials"
description: "Registry-driven PentaDocs portal guide for PentaCredentials."
...
<Info>
  ...
  **This page:** PentaCredentials — Registry-driven PentaDocs portal guide for PentaCredentials.
...
## Overview
**Mission / role:** Manage provider credential references, lifecycle/readiness, binding and credential health without exposing secret material.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.credentials` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D3` |
...
| Operator route | `/io/pentas/credentials` |
| Public status route | `/penta/credentials` |
...
## Dependencies
`penta.audit`, `penta.certify`, `penta.security`
...
| --- | --- |
| Architectural layers | [Security & Trust](/pentas/layers/security-trust), [Control & Governance](/pentas/layers/control-governance), [Routing, Interop & Transport](/pentas/layers/routing-transport), [Observability & Reliability](/pentas/layers/observability-reliability) |
| Jobs / functions | [Secure](/pentas/jobs/secure), [Govern](/pentas/jobs/govern), [Observe](/pentas/jobs/observe), [Route](/pentas/jobs/route), [Retrieve](/pentas/jobs/retrieve), [Transform](/pentas/jobs/transform) |
...
Route work to PentaCredentials when the request matches its recorded role (Manage provider credential references, lifecycle/readiness, binding and credential health without exposing secret material.) and one of its job classifications: secure, govern, observe, route, retrieve, transform.
...
5. Verify declared dependencies before execution: penta.audit, penta.certify, penta.security.
...
1. Start from this exact identity and machine key: `penta.credentials`.
2. Match the request to the documented jobs: `secure`, `govern`, `observe`, `route`, `retrieve`, `transform`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.credentials` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.credentials` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `3cef6927d4ab5583d8d2e0762a259fecdaef7f2f6a4ce2d7a1e0b6b72cac356d`
