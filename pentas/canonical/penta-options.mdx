---
title: "PentaOptions"
sidebarTitle: "PentaOptions"
description: "Registry-driven PentaDocs portal guide for PentaOptions."
...
<Info>
  ...
  **This page:** PentaOptions — Registry-driven PentaDocs portal guide for PentaOptions.
...
## Overview
**Mission / role:** certified OPTIONS/request primitive

| Field | Registry value |
| --- | --- |
| Machine key | `penta.options` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D3` |
...
| Operator route | `/io/pentas/options` |
| Public status route | `/penta/options` |
...
| --- | --- |
| Architectural layers | [Routing, Interop & Transport](/pentas/layers/routing-transport), [Observability & Reliability](/pentas/layers/observability-reliability) |
| Jobs / functions | [Retrieve](/pentas/jobs/retrieve), [Transform](/pentas/jobs/transform), [Route](/pentas/jobs/route), [Observe](/pentas/jobs/observe), [Recover](/pentas/jobs/recover) |
...
Route work to PentaOptions when the request matches its recorded role (certified OPTIONS/request primitive) and one of its job classifications: retrieve, transform, route, observe, recover.
...
1. Start from this exact identity and machine key: `penta.options`.
2. Match the request to the documented jobs: `retrieve`, `transform`, `route`, `observe`, `recover`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.options` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.options` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `e4fc0884c814e254f1ec5372a409817ee77ea6afa12660ea24e17036cd60a0d0`
