---
title: "PentaRelease"
sidebarTitle: "PentaRelease"
description: "Registry-driven PentaDocs portal guide for PentaRelease."
...
<Info>
  ...
  **This page:** PentaRelease — Registry-driven PentaDocs portal guide for PentaRelease.
...
## Overview
**Mission / role:** Version, package, publish and reconcile governed CrownThrive software releases and release evidence.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.release` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D3` |
...
| Operator route | `/io/pentas/release` |
| Public status route | `/penta/release` |
...
## Dependencies
`penta.assure`, `penta.build`, `penta.certify`, `penta.merge`
...
| --- | --- |
| Architectural layers | [Build, Test & Release](/pentas/layers/build-release), [Observability & Reliability](/pentas/layers/observability-reliability), [Media, Creative & Publishing](/pentas/layers/media-creative) |
| Jobs / functions | [Build](/pentas/jobs/build), [Test & Certify](/pentas/jobs/test-certify), [Publish](/pentas/jobs/publish), [Observe](/pentas/jobs/observe), [Recover](/pentas/jobs/recover) |
...
Route work to PentaRelease when the request matches its recorded role (Version, package, publish and reconcile governed CrownThrive software releases and release evidence.) and one of its job classifications: build, test-certify, publish, observe, recover.
...
5. Verify declared dependencies before execution: penta.assure, penta.build, penta.certify, penta.merge.
...
1. Start from this exact identity and machine key: `penta.release`.
2. Match the request to the documented jobs: `build`, `test-certify`, `publish`, `observe`, `recover`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.release` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.release` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `af8c242c65f46acb4f71b4993114d1206cd6656f72372be0d984b517086462b9`
