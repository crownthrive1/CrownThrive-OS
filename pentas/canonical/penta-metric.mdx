---
title: "PentaMetric"
sidebarTitle: "PentaMetric"
description: "Registry-driven PentaDocs portal guide for PentaMetric."
...
<Info>
  ...
  **This page:** PentaMetric — Registry-driven PentaDocs portal guide for PentaMetric.
...
## Overview
**Mission / role:** Thread-safe local counters, gauges, duration observations, and deterministic metric snapshots for PENTA runtime evidence.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.metric` |
| Kind | `system` |
| Maturity | `production` |
| Risk ceiling | `D1` |
...
| Operator route | `/io/pentas/metric` |
| Public status route | `/penta/metric` |
...
Route work to PentaMetric when the request matches its recorded role (Thread-safe local counters, gauges, duration observations, and deterministic metric snapshots for PENTA runtime evidence.) and one of its job classifications: observe, recover, support-operate, route, retrieve, transform.
...
1. Start from this exact identity and machine key: `penta.metric`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.metric` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.metric` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `f7acf36c842e3279fa5d4b2bc4e3998c0f70b63d2c8cff22d4ca73410872aadc`
