---
title: "PentaHealth"
sidebarTitle: "PentaHealth"
description: "Registry-driven PentaDocs portal guide for PentaHealth."
...
<Info>
  ...
  **This page:** PentaHealth — Registry-driven PentaDocs portal guide for PentaHealth.
...
## Overview
**Mission / role:** Track operational wellbeing and workload health of PENTA workers/systems, maintenance stress, burnout-like capacity indicators and support needs without acting as clinical care.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.health` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D2` |
...
| Operator route | `/io/pentas/health` |
| Public status route | `/penta/health` |
...
## Dependencies
`penta.benefits`, `penta.hr`, `penta.triage`
...
| --- | --- |
| Architectural layers | [Observability & Reliability](/pentas/layers/observability-reliability), [Build, Test & Release](/pentas/layers/build-release) |
| Jobs / functions | [Observe](/pentas/jobs/observe), [Recover](/pentas/jobs/recover), [Support & Operate](/pentas/jobs/support-operate), [Build](/pentas/jobs/build), [Test & Certify](/pentas/jobs/test-certify), [Publish](/pentas/jobs/publish) |
...
Route work to PentaHealth when the request matches its recorded role (Track operational wellbeing and workload health of PENTA workers/systems, maintenance stress, burnout-like capacity indicators and support needs without acting as clinical care.) and one of its job classifications: observe, recover, support-operate, build, test-certify, publish.
...
5. Verify declared dependencies before execution: penta.benefits, penta.hr, penta.triage.
...
1. Start from this exact identity and machine key: `penta.health`.
2. Match the request to the documented jobs: `observe`, `recover`, `support-operate`, `build`, `test-certify`, `publish`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.health` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.health` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `caaaff29dbd0b916254062a4ac87225a9801f21f06e430cb9f882dda7a14b093`
