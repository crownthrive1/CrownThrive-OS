...
description: "Registry-driven PentaDocs portal guide for PentaMail."
...
<Info>
  ...
  **This page:** PentaMail — Registry-driven PentaDocs portal guide for PentaMail.
...
## Overview
**Mission / role:** Govern institutional email composition, routing, provider bindings, delivery state, threading, templates, receipts and retention.
...
| Operator route | `/io/pentas/mail` |
| Public status route | `/penta/mail` |
...
## Dependencies
`penta.credentials`, `penta.docs`, `penta.route`, `penta.status`
...
| --- | --- |
| Architectural layers | [Communications, Growth & Service](/pentas/layers/communications-growth), [Experience & Interface](/pentas/layers/experience-interface), [Routing, Interop & Transport](/pentas/layers/routing-transport), [Observability & Reliability](/pentas/layers/observability-reliability) |
...
Route work to PentaMail when the request matches its recorded role (Govern institutional email composition, routing, provider bindings, delivery state, threading, templates, receipts and retention.) and one of its job classifications: communicate-grow, support-operate, analyze-research, route, retrieve, transform.
...
5. Verify declared dependencies before execution: penta.credentials, penta.docs, penta.route, penta.status.
...
1. Start from this exact identity and machine key: `penta.mail`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.mail` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.mail` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `2f5f3e36b7bfec8ca1964bc62183bc9446c1c5651d41e03cd668418e8197ce49`
