---
title: "PentaSync"
sidebarTitle: "PentaSync"
description: "Registry-driven PentaDocs portal guide for PentaSync."
...
<Info>
  ...
  **This page:** PentaSync — Registry-driven PentaDocs portal guide for PentaSync.
...
| Operator route | `/io/pentas/primitives#sync` |
| Public status route | `/penta/sync` |
...
Route work to PentaSync when the request matches its recorded role (Governed capability primitive under PentaRoute and the universal Penta OS contract.) and one of its job classifications: retrieve, transform, route.
...
1. Start from this exact identity and machine key: `penta.sync`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.sync` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.sync` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `7815861c1f9d4931200094617fae121ac0ba1b400f0c7927da0bf425da0c6fbb`
