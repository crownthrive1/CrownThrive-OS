---
title: "PentaHeartbeat"
sidebarTitle: "PentaHeartbeat"
description: "Registry-driven PentaDocs portal guide for PentaHeartbeat."
...
<Info>
  ...
  **This page:** PentaHeartbeat — Registry-driven PentaDocs portal guide for PentaHeartbeat.
...
## Overview
**Mission / role:** Whole-system heartbeat and readiness aggregation across PentaOD, PentaBeata, PentaMesh, PentaFabric, PentaStatus, CHLOM mesh health and the canonical ThriveBase heartbeat.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.heartbeat` |
| Kind | `system` |
| Maturity | `production` |
| Risk ceiling | `D1` |
...
| Operator route | `/io/pentas/heartbeat` |
| Public status route | `/penta/heartbeat` |
...
## Dependencies
`penta.beata`, `penta.fabric`, `penta.mesh`, `penta.od`, `penta.status`
...
| Lifecycle stages | [Design](/pentas/lifecycle/design), [Build](/pentas/lifecycle/build), [Verify](/pentas/lifecycle/verify), [Release](/pentas/lifecycle/release), [Operate](/pentas/lifecycle/operate), [Observe](/pentas/lifecycle/observe), [Recover](/pentas/lifecycle/recover), [Govern](/pentas/lifecycle/govern), [Evolve](/pentas/lifecycle/evolve) |
...
Route work to PentaHeartbeat when the request matches its recorded role (Whole-system heartbeat and readiness aggregation across PentaOD, PentaBeata, PentaMesh, PentaFabric, PentaStatus, CHLOM mesh health and the canonical ThriveBase heartbeat.) and one of its job classifications: recover, observe, support-operate, route, retrieve, transform.
...
5. Verify declared dependencies before execution: penta.beata, penta.fabric, penta.mesh, penta.od, penta.status.
...
1. Start from this exact identity and machine key: `penta.heartbeat`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.heartbeat` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.heartbeat` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `671a14382abd4dbf6392db50f60c5a2c4ef6f0b16ba9979267f529b5c577a2a6`
