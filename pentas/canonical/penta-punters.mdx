---
title: "PentaPunters"
sidebarTitle: "PentaPunters"
description: "Registry-driven PentaDocs portal guide for PentaPunters."
...
<Info>
  ...
  **This page:** PentaPunters — Registry-driven PentaDocs portal guide for PentaPunters.
...
## Overview
**Mission / role:** fail-closed routing of bounded action requests to eligible runners

| Field | Registry value |
| --- | --- |
| Machine key | `penta.punters` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D3` |
...
| Operator route | `/io/pentas/punters` |
| Public status route | `/penta/punters` |
...
Route work to PentaPunters when the request matches its recorded role (fail-closed routing of bounded action requests to eligible runners) and one of its job classifications: build, test-certify, publish.
...
1. Start from this exact identity and machine key: `penta.punters`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.punters` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.punters` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `832ca653760826962b91a8f49f11ecf4db4254e7c441d0e8329062971a68d798`
