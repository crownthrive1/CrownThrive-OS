---
title: "PentaRestore"
sidebarTitle: "PentaRestore"
description: "Registry-driven PentaDocs portal guide for PentaRestore."
...
<Info>
  ...
  **This page:** PentaRestore — Registry-driven PentaDocs portal guide for PentaRestore.
...
## Overview
**Mission / role:** Fail-closed restore planning gated by provenance, integrity, and restore readiness.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.restore` |
| Kind | `system` |
| Maturity | `implemented` |
| Risk ceiling | `D3` |
| Execution eligible by registry | `false` |
| Strict readiness | `HOLD_DEPENDENCY_INVENTORY_UNASSESSED` |
| Family | [Resilience Continuity](/pentas/families/resilience-continuity) |
| Family assignment | `docs_inferred` |
| Operator route | `/io/pentas/restore` |
| Public status route | `/penta/restore` |
...
Primary family context: [Resilience Continuity](/pentas/families/resilience-continuity). Sibling, upstream, downstream, and cross-family relationships resolve through the family topology and dependency graph rather than prose inference.
...
Route work to PentaRestore when the request matches its recorded role (Fail-closed restore planning gated by provenance, integrity, and restore readiness.) and one of its job classifications: recover, observe, support-operate, route, retrieve, transform.
...
1. Start from this exact identity and machine key: `penta.restore`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.restore` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.restore` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `2b12addb1e103a167400f072a2e5d1d86575e5b78c775bfd9f81ded4b6699224`
