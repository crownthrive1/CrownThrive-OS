---
title: "PentaMerge"
sidebarTitle: "PentaMerge"
description: "Registry-driven PentaDocs portal guide for PentaMerge."
...
<Info>
  ...
  **This page:** PentaMerge — Registry-driven PentaDocs portal guide for PentaMerge.
...
## Overview
**Mission / role:** Govern merge readiness, stacking, dependency order, conflict disposition and exact-head promotion into canonical branches.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.merge` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D3` |
...
| Operator route | `/io/pentas/merge` |
| Public status route | `/penta/merge` |
...
## Dependencies
`penta.assure`, `penta.closer`, `penta.docs`
...
Route work to PentaMerge when the request matches its recorded role (Govern merge readiness, stacking, dependency order, conflict disposition and exact-head promotion into canonical branches.) and one of its job classifications: build, test-certify, publish, route, retrieve, transform.
...
5. Verify declared dependencies before execution: penta.assure, penta.closer, penta.docs.
...
1. Start from this exact identity and machine key: `penta.merge`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.merge` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.merge` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `2fd66a7fd9f059900bdfddfba031987b4d358378eabf6cba1771f9679f376d7e`
