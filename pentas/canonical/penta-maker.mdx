...
<Info>
  ...
  **This page:** PentaMaker — Registry-driven PentaDocs portal guide for PentaMaker.
...
Route work to PentaMaker when the request matches its recorded role (Selects the registered Penta responsible for authoring an artifact without transport or certification authority.) and one of its job classifications: orchestrate, build, support-operate, secure, govern.
...
1. Start from this exact identity and machine key: `penta.maker`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.maker` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.maker` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `a80b78eb59b30a3e1b2febb1f0118705e9a977d3eafed104021b2263e444bb85`
