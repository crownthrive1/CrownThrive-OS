---
title: "PentaSuite"
sidebarTitle: "PentaSuite"
description: "Registry-driven PentaDocs portal guide for PentaSuite."
...
<Info>
  ...
  **This page:** PentaSuite — Registry-driven PentaDocs portal guide for PentaSuite.
...
## Overview
**Mission / role:** Build bounded autonomous agent laboratories and capability suites only from approved PentaRFA requests, including TTL, tools, adapters, policy, rollback and job contract.

| Field | Registry value |
| --- | --- |
| Machine key | `penta.suite` |
| Kind | `system` |
| Maturity | `specified` |
| Risk ceiling | `D3` |
...
| Operator route | `/io/pentas/suite` |
| Public status route | `/penta/suite` |
...
## Dependencies
`penta.certify`, `penta.credentials`, `penta.factory`, `penta.rfa`
...
| --- | --- |
| Architectural layers | [Automation & Agentic](/pentas/layers/automation-agentic), [Build, Test & Release](/pentas/layers/build-release) |
| Jobs / functions | [Orchestrate](/pentas/jobs/orchestrate), [Build](/pentas/jobs/build), [Support & Operate](/pentas/jobs/support-operate), [Test & Certify](/pentas/jobs/test-certify), [Publish](/pentas/jobs/publish) |
...
Route work to PentaSuite when the request matches its recorded role (Build bounded autonomous agent laboratories and capability suites only from approved PentaRFA requests, including TTL, tools, adapters, policy, rollback and job contract.) and one of its job classifications: orchestrate, build, support-operate, test-certify, publish.
...
5. Verify declared dependencies before execution: penta.certify, penta.credentials, penta.factory, penta.rfa.
...
1. Start from this exact identity and machine key: `penta.suite`.
2. Match the request to the documented jobs: `orchestrate`, `build`, `support-operate`, `test-certify`, `publish`.
...
2. Resolve the canonical implementation and contracts from machine key `penta.suite` and the evidence paths below.
...
Agents and Pentas should not scrape prose as their primary contract. Consume the machine-readable record keyed by `penta.suite` from `data/penta/agent-knowledge.v1.json`, then use this page for explanation and examples.
...
- This record hash: `5b0e7dfbecece736310994cd369e2b44c0cee98122ba9c678130816f01dd61e0`
